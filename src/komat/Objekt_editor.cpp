...
{ 
  ...
  if(k == K_CHYBA) {
    ...
    *p_cnf->p_invcam = p_cnf->kamery[k].p_objekt->m;
    ...
    projection_matrix(p_cnf->p_project,p_cnf->kamery[k].fov,(float)OXRES/(float)OYRES,p_cnf->kam.near_plane,p_cnf->kam.far_plane);
                                  ...
                                  byte spol_bodu)
{
 ...
 /*
   Naplnim face 1
 */
 k1 = p_list[f1].k; o1 = p_list[f1].o; f = p_list[f1].v;
 ...
 /*
   Naplnim face 2
 */
 k2 = p_list[f2].k; o2 = p_list[f2].o; f = p_list[f2].v;
 ...
 */
       ...
       if(p_cnf->p_kont[k]->p_obj[o]) {
         for(f = p_cnf->p_kont[k]->p_obj[o]->facenum-1; f > 0; f -= 3) {
           ...
           calc_rovinu(p_cnf->p_kont[k]->p_obj[o]->p_vertex+(f-2),
                       ...
                       p_cnf->p_kont[k]->p_obj[o]->p_vertex+f, 
                       p_rov+facu);
 ...
 for(i = 0; i < facu; i++) {
   if(p_rov[i].e == 1.0f) {
     b.x = p_rov[i].x; b.y = p_rov[i].y; b.z = p_rov[i].z;
...
{   
  ...
  oe_smaz_scenu(p_cnf);
  oe_nahraj_projekt(p_cnf, "ray_zaloha.bpr", p_cnf->dir.data_dir, FALSE, TRUE);
}

ZDRCADLO_DESC_POLY * oe_najdi_podzrcadlo(ZDRCADLO_DESC_POLY *p_first, int kont_id)
{
  while(p_first) {
...
int oe_scena_to_poly(K_EDITOR *p_cnf, EDIT_MESH_POLY **p_list)
{
  ...
  */
  for(i = 0, p = 0; i < p_cnf->max_kont; i++) {
    if(p_cnf->p_kont[i] && p_cnf->p_kont[i]->kflag&KONT_STATIC && !(p_cnf->p_kont[i]->kflag&KONT_VERTEX_LIGHT)) {
      p++;
    }
  }
  ...
  */
  for(i = 0, t = 0; i < p_cnf->max_kont; i++) {
    if(p_cnf->p_kont[i]) {
      ...
      if(kflag&KONT_PRVEK && kflag&KONT_STATIC && !(kflag&KONT_VERTEX_LIGHT)) {
    ...
    if(p_cnf->p_kont[i]) {
      ...
      if(!(kflag&KONT_PRVEK) && kflag&KONT_STATIC && !(kflag&KONT_VERTEX_LIGHT)) {
        ...
        if(p_cnf->p_kont[i]->kflag&KONT_ZRCADLO) { // oflaguj zrcadlo
          p_zrc = oe_najdi_podzrcadlo(p_cnf->zrcadlo.p_poly, p_cnf->p_kont[i]->kontejner_ID);
          if(p_zrc) {
            p_zrc->poly = t;
          }
        }  
...
void oe_scena_to_game(K_EDITOR *p_cnf)
{
  ...
  */  
  ...
  p_cnf->kontraynum = oe_scena_to_raykont(p_cnf, p_cnf->p_kont_raytracer, KONT_NUM);
  ...
  */
  ...
  kd_strom_vyrob(p_cnf->p_poly, p_cnf->polynum,
                 p_cnf->p_kont_raytracer, p_cnf->kontraynum,
                 &p_cnf->kdtree);
...
void oe_updatuj_scenu_stat(K_EDITOR *p_cnf)
{
  ...
  for(k = 0; k < KONT_NUM; k++) {
    if(p_cnf->p_kont[k]) {
      ...
      if(p_cnf->p_kont[k]->kflag&KONT_STATIC && !(p_cnf->p_kont[k]->kflag&KONT_VERTEX_LIGHT)) {
...
*/
  ...
  for(i = 0; i < p_cnf->max_kont; i++) {
    if(p_cnf->p_kont[i]) {
      ...
      cti_velikost_kontejneru(p_cnf->p_kont[i], &p_stat->kontsize, &p_stat->objnum);
    }
  }
  p_stat->facu /= 3;
  ...
  for(i = 0; i < MAX_BERUSKY_KONTEJNERU; i++) {
    if(p_cnf->bnf.p_kont[i]) {
      p_stat->lev_kontnum++;
      cti_velikost_kontejneru(p_cnf->bnf.p_kont[i], &p_stat->lev_kontsize, &p_stat->lev_objnum);
    }
  }
...
float oe_stredni_vzdalenost_ve_scene(K_EDITOR *p_cnf)
{
 ...
 if(akt_editor == GAME_EDITOR) {
   ROVINA   r = {0.0f,1.0f,0.0f,(float)(-p_bnf->grid.akt_patro*Y_PRVEK-p_bnf->y_start)};
   transf_2d_3d((float)OXRES/2, (float)OYRES/2, 0,
     &a.x, &a.y, &a.z,
     p_cnf->p_camera,
     p_cnf->p_project, 
     OXRES, OYRES, OXSTART, OYSTART);
   transf_2d_3d((float)OXRES/2, (float)OYRES/2, 1.0f,
     &b.x, &b.y, &b.z,
     p_cnf->p_camera,
     p_cnf->p_project, 
     OXRES, OYRES, OXSTART, OYSTART);
   ...
   for(k = 0; k < kolik; k++) {
     ...
     if(!p_kont || !p_kont->kreslit || p_kont->kflag&KONT_NO_CALC) continue;
     ...
     for(ob = 0; ob < kolik_ob; ob++) {
       ...
       if(!p_obj || !p_obj->kreslit) continue;
...
void oe_konfiguruj_editor(K_EDITOR *p_cnf, byte *p_file, int renderovac)
{
 ...
 GetPrivateProfileString("editor","k_far_plane","1000000.0f",pom,99,p_file);
...
//#define AMBIENT 0x90909090
 ...
 p_cnf->cnf.precizni_export_obb = GetPrivateProfileInt("editor","precizni_export_obb",0,p_file);
 p_cnf->cnf.mazat_materialy = GetPrivateProfileInt("editor","mazat_materialy",1,p_file);

 p_cnf->ray.ambient = GetPrivateProfileInt("ray-tracer","ambient",p_cnf->ray.ambient,p_file);
 GetPrivateProfileString("ray-tracer","faktor","5.0",pom,99,p_file); 
 p_cnf->ray.faktor = (float)atof(pom); 
 p_cnf->ray.stupen_filtrace = GetPrivateProfileInt("ray-tracer","stupen_filtrace",p_cnf->ray.stupen_filtrace,p_file);
 p_cnf->ray.disk = GetPrivateProfileInt("ray-tracer","disk",p_cnf->ray.disk,p_file);
 p_cnf->ray.stiny = GetPrivateProfileInt("ray-tracer","stiny",p_cnf->ray.stiny,p_file);
 p_cnf->ray.max_x_surface = GetPrivateProfileInt("ray-tracer","max_x_surface",p_cnf->ray.max_x_surface,p_file);
 p_cnf->ray.max_y_surface = GetPrivateProfileInt("ray-tracer","max_y_surface",p_cnf->ray.max_y_surface,p_file);
 p_cnf->ray.max_x_lepic = GetPrivateProfileInt("ray-tracer","max_x_lepic",p_cnf->ray.max_x_lepic,p_file);
 p_cnf->ray.max_y_lepic = GetPrivateProfileInt("ray-tracer","max_x_lepic",p_cnf->ray.max_x_lepic,p_file);
 p_cnf->ray.disk_mezi = GetPrivateProfileInt("ray-tracer","disk_mezi",p_cnf->ray.disk_mezi,p_file);
 p_cnf->ray.stinovani = GetPrivateProfileInt("ray-tracer","stinovani",p_cnf->ray.stinovani,p_file);
 p_cnf->ray.supersampl = GetPrivateProfileInt("ray-tracer","supersampl",p_cnf->ray.supersampl,p_file);
 p_cnf->ray.filtrace = GetPrivateProfileInt("ray-tracer","filtrace",p_cnf->ray.filtrace,p_file);

 p_cnf->ray.sup_hloubka = GetPrivateProfileInt("ray-tracer","sup_hloubka",1,p_file);
 p_cnf->ray.sup_velikost_x = GetPrivateProfileInt("ray-tracer","sup_velikost_x",2,p_file);
 p_cnf->ray.sup_velikost_y = GetPrivateProfileInt("ray-tracer","sup_velikost_y",2,p_file);

 p_cnf->ray.okraj = GetPrivateProfileInt("ray-tracer","okraj",4,p_file);
 p_cnf->ray.text_min_x = GetPrivateProfileInt("ray-tracer","text_min_x",16,p_file);
 p_cnf->ray.text_min_y = GetPrivateProfileInt("ray-tracer","text_min_y",16,p_file);

 p_cnf->cnf.mlhokostku_kreslit = GetPrivateProfileInt("editor","mlhokostku_kreslit",0,p_file);
 p_cnf->cnf.mlhokostka_aktivni = GetPrivateProfileInt("editor","mlhokostka_aktivni",0,p_file);
 p_cnf->cnf.mlhokostka_testy = GetPrivateProfileInt("editor","mlhokostka_testy",0,p_file);

 p_cnf->cnf.editor_priorita = GetPrivateProfileInt("editor","editor_priorita",0,p_file); 
 ...
 GetPrivateProfileString("game","prvky_dir","work_dir",p_cnf->dir.prvky_dir,99,p_file);
 if(oe_work_dir(p_cnf->dir.prvky_dir)) kprintf(TRUE,"work dir \"%s\" = \"%s\"","prvky_dir",work_dir);
 
 GetPrivateProfileString("game","level_dir","work_dir",p_cnf->dir.level_dir,99,p_file);
 if(oe_work_dir(p_cnf->dir.level_dir)) kprintf(TRUE,"work dir \"%s\" = \"%s\"","level_dir",work_dir);
 strcpy(p_cnf->dir.edit_level_dir,p_cnf->dir.level_dir);
 
 GetPrivateProfileString("game","out_dir","work_dir",p_cnf->dir.out_dir,99,p_file);
 if(oe_work_dir(p_cnf->dir.out_dir)) kprintf(TRUE,"work dir \"%s\" = \"%s\"","out_dir",work_dir);
 
 // Load texture dir
 for(i = 0; i < TEXT_DIRS; i++) {
   sprintf(pom,"texture_dir%d",i);
   GetPrivateProfileString("game",pom,"",p_cnf->tdir.texture_dir[i],99,p_file);
   if(p_cnf->tdir.texture_dir[i][0] == '.' && !p_cnf->tdir.texture_dir[i][1])
 ...
 for(i = 0; i < TEXT_DIRS; i++) {
   ...
   GetPrivateProfileString("game",pom,"",p_cnf->tdir.texture_file[i],99,p_file);
   if(p_cnf->tdir.texture_file[i][0] == '.' && !p_cnf->tdir.texture_file[i][1])
     p_cnf->tdir.texture_file[i][0] = 0;
 }
 
 GetPrivateProfileString("game","material_dir","work_dir",p_cnf->dir.material_dir,99,p_file);
 if(oe_work_dir(p_cnf->dir.material_dir)) kprintf(TRUE,"work dir \"%s\" = \"%s\"","material_dir",work_dir);
 
 GetPrivateProfileString("game","data_dir","work_dir",p_cnf->dir.data_dir,99,p_file);
 if(oe_work_dir(p_cnf->dir.data_dir)) kprintf(TRUE,"work dir \"%s\" = \"%s\"","data_dir",work_dir);
 
 GetPrivateProfileString("game","scene_dir","work_dir",p_cnf->dir.scene_dir,99,p_file);
 if(oe_work_dir(p_cnf->dir.scene_dir)) kprintf(TRUE,"work dir \"%s\" = \"%s\"","scene_dir",work_dir);
 
 GetPrivateProfileString("game","game_level_dir","work_dir",p_cnf->dir.game_level_dir,99,p_file);
 if(oe_work_dir(p_cnf->dir.game_level_dir)) kprintf(TRUE,"work dir \"%s\" = \"%s\"","game_level_dir",work_dir);
   
 GetPrivateProfileString("game","game_data_dir","work_dir",p_cnf->dir.game_data_dir,99,p_file);
 if(oe_work_dir(p_cnf->dir.game_data_dir)) kprintf(TRUE,"work dir \"%s\" = \"%s\"","game_data_dir",work_dir);
 ...
 p_cnf->bnf.default_x = GetPrivateProfileInt("editor","default_x",10,p_file);
 p_cnf->bnf.default_y = GetPrivateProfileInt("editor","default_y",3,p_file);
 p_cnf->bnf.default_z = GetPrivateProfileInt("editor","default_z",10,p_file);

 p_cnf->lp.aktivni = GetPrivateProfileInt("editor","lista_prvku",0,p_file);
 p_cnf->cnf.load_textur = GetPrivateProfileInt("editor","load_textury",1,p_file);
 hlasit_kolize = GetPrivateProfileInt("editor","kolize_materialu",0,p_file);
 GetPrivateProfileString("editor","berusky_hra","berusky.exe",p_cnf->berusky_hra,199,p_file);
 GetPrivateProfileString("editor","berusky_hra_dir",".",p_cnf->berusky_hra_dir,199,p_file);

 
 GetPrivateProfileString("editor","editor","1",pom,19,p_file);
 akt_editor = atoi(pom);

 p_cnf->cnf.mazat_dummy = GetPrivateProfileInt("editor","mazat_dummy",1,p_file);
 
 p_cnf->cnf.obalky_kontejner = GetPrivateProfileInt("editor","obalky_kontejner",0,p_file);
 p_cnf->cnf.obalky_objekt = GetPrivateProfileInt("editor","obalky_objekt",1,p_file);

 p_cnf->drobic_hloubka = GetPrivateProfileInt("editor","drobic_hloubka",1,p_file);
 
 p_cnf->deka.aktivni = GetPrivateProfileInt("editor","deka_aktivni",0,p_file);
...
*/
 ...
 p_cnf->bar.set_krok = GetPrivateProfileInt("editor","krok_barvy",1,p_file)*0.001f;
 p_cnf->bar.oboustrany_stinovac = GetPrivateProfileInt("editor","oboustrany_stinovac",0,p_file);
 p_cnf->bar.multi_mesh = GetPrivateProfileInt("editor","multi_mesh",1,p_file);
...
*/
 if(GetPrivateProfileInt("editor","load_system_materialy",0,p_file)) {
   ...
   GetPrivateProfileString("editor","system_materialy","syssada.b2t",file,99,p_file); 
   chdir(p_cnf->dir.material_dir);
   lo_nahraj_materialy(p_cnf->p_mat,MAX_EDIT_MATERIALU,p_cnf->p_text,MAX_EDIT_TEXTUR,file);
 }
...
*/
 if(GetPrivateProfileInt("editor","load_default_materialy",0,p_file)) {
   ...
   GetPrivateProfileString("editor","default_materialy","defsada.b2t",file,99,p_file); 
   chdir(p_cnf->dir.material_dir);     
   lo_nahraj_materialy(p_cnf->p_mat,MAX_EDIT_MATERIALU,p_cnf->p_text,MAX_EDIT_TEXTUR,file);
 }
 ...
 */
 p_cnf->cnf.load_prvku = GetPrivateProfileInt("editor","load_prvky",0,p_file);
...
void oe_ini_mod(K_EDITOR *p_cnf, int mod, int styl)
{
  ...
  if(!styl) {
    ...
    calc_camera(&p_cnf->viewports[mod].camera,&p_cnf->viewports[mod].r_bod,p_cnf->viewports[mod].vzdal,p_cnf->viewports[mod].fi);
    invert_matrix(&p_cnf->viewports[mod].camera,&p_cnf->viewports[mod].invcam);
  } else {
    int md = p_cnf->mod;
    ...
    p_m->_11 = 0; p_m->_21 = 0; p_m->_31 = 0; // x
    p_m->_12 = 0; p_m->_22 = 0; p_m->_32 = 0; // y 
    p_m->_13 = 0; p_m->_23 = 0; p_m->_33 = 0; // z
    ...
    calc_r_bod(&p_cnf->viewports[mod].camera, &p_cnf->viewports[mod].r_bod, p_cnf->viewports[mod].vzdal, p_cnf->viewports[mod].fi);
    invert_matrix(&p_cnf->viewports[mod].camera,&p_cnf->viewports[mod].invcam);
...
void oe_prikaz_posun_sceny_xz(K_EDITOR *p_cnf)
{
 ...
 nuluj_mys();
 while(stisk_mys()) {
   translate_matrix(p_cnf->p_r_bod, -(float)mi.dx*0.02f,0,(float)mi.dy*0.02f);
 ...
 *p_cnf->p_invcam = m;
 ...
 while(stisk_mys()) {
   ...
   *p_cnf->p_fi += ((float)PI/180)*(float)mi.dy*0.5f;
   rotate_matrix_y(p_cnf->p_r_bod, -((float)PI/180)*(float)mi.dx*0.5f);
   calc_camera(p_cnf->p_camera,p_cnf->p_r_bod,*p_cnf->p_vzdal,*p_cnf->p_fi);
   invert_matrix(p_cnf->p_camera,p_cnf->p_invcam);
...
void oe_nastav_kameru(K_EDITOR *p_cnf, BOD *p_bod, float r, float fi, float vzdal)
{       
 *p_cnf->p_vzdal = vzdal;
 *p_cnf->p_fi = fi;
...
void oe_vybrani_ctverce(K_EDITOR *p_cnf, dword barva)
{
 ...
 if(p_cnf->x1 > p_cnf->x2) {v = p_cnf->x2; p_cnf->x2 = p_cnf->x1; p_cnf->x1 = v;}
 if(p_cnf->y1 > p_cnf->y2) {v = p_cnf->y2; p_cnf->y2 = p_cnf->y1; p_cnf->y1 = v;}
...
*/
   ...
   if(key[K_P]) {
     ...
     if(p_cnf->lc.hDlg) {
       if(p_cnf->lc.extra_svetlo)         
         doe_proved_prikaz_menu(key[K_CTRL]?ID_K_EFEKTOVESVETLA_POSUNKOPIE:ID_K_EFEKTOVESVETLA_POSUN,TRUE,TRUE);
       else         
         doe_proved_prikaz_menu(key[K_CTRL]?ID_K_DYNAMICKESVETLA_POSUNOUTKOPII:ID_K_DYNAMICKESVETLA_POSUNOUT,TRUE,TRUE);
     } else {
       doe_proved_prikaz_menu(key[K_CTRL]?ID_K_SVETLA_POSUNOUTKOPII:ID_K_SVETLA_POSUNOUT,TRUE,TRUE);
     }
   }
/* ----
*/
...
void oe_pridej_dvertex(K_EDITOR *p_cnf, BOD *p, int k, int o, int f, byte pozice, byte d2)
{
 if(!p_cnf->dvertnum) { // maloc - groupnum = 0
   ...
   if(!(p_cnf->p_dvert = (D_VERTEX *) realloc(p_cnf->p_dvert,sizeof(D_VERTEX)*(p_cnf->dvertnum+1))))
     chyba("Pamet");
 }
...
void oe_transformuj_2d_texture_tagy(K_EDITOR *p_cnf, int k, int o, byte textura)
{
  ...
  x_c = p_cnf->wxres/2; y_c = p_cnf->wyres/2;
  xs = p_cnf->wx; ys = p_cnf->wy;
...
{
  BOD p0 = {0,0,0}, p1 = {1,2.89f,0}, p2 = {1,2.89f,1};
  BOD orig = {0,3,0}, dir = {2,-3,1};
  BOD a = {0,3,0}, b = {2,0,1}, p;
  int   vysl;
  ROVINA r;
...
void oe_test_text_resident(K_EDITOR *p_cnf)
{
 ...
 for(i = 0; i < MAX_EDIT_TEXTUR; i++) {
   if(p_cnf->p_text[i].load) { // textura je loadnuta
     ...
     kprintf(1,"Textura %d,%s je %d",p_cnf->p_text[i].text,p_cnf->p_text[i].jmeno,rez);
   }
 }
}
...
void oe_klavesy_editace_kontejneru(K_EDITOR *p_cnf)
{
 if(key[K_K] && key[K_CTRL]) {
   ...
   if(key[K_V]&&key[K_CTRL]) {
     key[K_V] = FALSE;
     doe_proved_prikaz_menu(ID_K_KONTEJNER_NORMALOVEVEKTORYNAVSECHNO,TRUE,TRUE);
   }
   if(key[K_X]) {
     key[K_X] = FALSE;
  ...
  for(k = 0; k < p_cnf->max_kont; k++) {
    if(p_cnf->p_kont[k]) {
      ...
      for(o = 0; o < p_kont->max_objektu; o++) {
        if(p_kont->p_obj[o]) p_kont->p_obj[o]->kreslit = TRUE;
      }
    }
  }
}
...
void oe_prikaz_zmena_kresleni_kamer(K_EDITOR *p_cnf)
{
 ...
 p_cnf->p_kont[KAMERA_4]->kreslit=
 !p_cnf->p_kont[R_BOD]->kreslit;
}
                         ...
                         int x2, int y2, I_VERTEX *p_index)
{
 ...
 for(k = 0; k < kontnum; k++) {
    p_kont = p_cnf->p_kont[k];
    if(!p_kont || !p_kont->kreslit) continue;
 ...
 *p_t = FLT_MAX;
 ...
 for(o = 0; o < p_kont->max_objektu; o++) {
   ...
   for(f = 0; f < facenum; f+=3) {
     p0.x = p_vert[p_face[f]].x;   p0.y = p_vert[p_face[f]].y;   p0.z = p_vert[p_face[f]].z;
     p1.x = p_vert[p_face[f+1]].x; p1.y = p_vert[p_face[f+1]].y; p1.z = p_vert[p_face[f+1]].z;
     p2.x = p_vert[p_face[f+2]].x; p2.y = p_vert[p_face[f+2]].y; p2.z = p_vert[p_face[f+2]].z;
...
int oe_intersect_scena(K_EDITOR *p_cnf, BOD *p_orig, BOD *p_dir, float *p_t, int *p_k, int *p_o, int *p_f)
{
  ...
  *p_t = FLT_MAX;
  ...
  for(k = 0; k < p_cnf->max_kont; k++) {
    if(p_cnf->p_kont[k] /*&& p_cnf->p_kont[k]->kreslit/* && intersect_kont_fast(p_cnf->p_kont[k],p_orig,p_dir)*/) {
...
int oe_nejblizsi_bod_ve_scene(K_EDITOR *p_cnf, BOD *p_bod, int *p_k, int *p_o, int *p_f)
{
 ...
 for(k = 0; k < kontnum; k++) {
    ...
    _1t.x = _1.x; _1t.y = _1.y; _1t.z = _1.z;
    _2t.x = _2.x; _2t.y = _2.y; _2t.z = _2.z;
    ...
    for(ob = 0; ob < objnum; ob++) {
      ...
      for(f = 0; f < facenum; f+=3) {
        if(prusek_face_editor(&_1, &_2, &p, p_vert+p_face[f], p_vert+p_face[f+1], p_vert+p_face[f+2])) {
...
int oe_prikaz_vyber_mysi_mesh(K_EDITOR *p_cnf)
{
 ...
 OBJ_VERTEX      _1,_2,p,
                *p_vert; 
 BOD             _1t,_2t,A,Q;
 float           t, max_t = FLT_MAX,
                 *p_P,*p_Q,*p_A;
 word            f,n_f,*p_face;
 int             kontnum = p_cnf->max_kont,
                 objnum,
                 facenum,
                 k,ob,n_k,n_ob,hit = 0,o;
 ...
 for(k = 0; k < kontnum; k++) {
    ...
    if(!(p_kont->kflag&KONT_KEYFRAME)) {
      ...
      // vektor 1->2
      A.x = _1t.x; A.y = _1t.y; A.z = _1t.z;
      Q.x = _2t.x - _1t.x; Q.y = _2t.y - _1t.y; Q.z = _2t.z - _1t.z;       
    }
    ...
    for(ob = 0; ob < objnum; ob++) {
      ...
      for(f = 0; f < facenum; f+=3) {
        if(prusek_face_editor(&_1, &_2, &p, p_vert+p_face[f], p_vert+p_face[f+1], p_vert+p_face[f+2])) {
          transformuj_bod_matici(&p.x, &p.y, &p.z, p_world);
          if(Q.x > Q.y && Q.x > Q.z) {
            p_P = &p.x; p_Q = &Q.x; p_A = &A.x;
          } else if(Q.y > Q.x && Q.y > Q.z) {
            p_P = &p.y; p_Q = &Q.y; p_A = &A.y;
          } else {
            p_P = &p.z; p_Q = &Q.z; p_A = &A.z;
          }
          t = (*p_P - *p_A)/(*p_Q);
          if( t > 0.0f && t < max_t) {
            max_t = t;
 ...
 if(hit) {
   ...
   if(key[K_CTRL]) {
     ...
     p_cnf->vybr_face_group = najdi_group_face_vertex(p_cnf->p_group_face,p_cnf->groupfacenum,&face,p_cnf->groupflag);
   ...
   kprintf(0,"Kontejner: %s Objekt %s",p_cnf->p_kont[k]->jmeno,p_cnf->p_kont[k]->p_obj[o]->jmeno);
   if(f != K_CHYBA && p_cnf->p_mat[f]) {
     kprintf(0,"Mat %d,%s, Txt %s",f,p_cnf->p_mat[f]->jmeno,p_cnf->p_mat[f]->textfile[0]);
...
//   p_kobj = oe_cti_joint_kont(p_cnf);
   ...
   p_cnf->jcf.k = k;
   p_cnf->jcf.o = o;
   if(p_kobj != p_obj || !p_cnf->jcf.p_joint_akt || !p_cnf->jcf.p_joint_animace_akt) {
     p_cnf->jcf.p_joint_animace_akt = p_cnf->p_kont[k]->p_obj[o]->p_joit_animace;
     p_cnf->jcf.p_joint_akt = p_cnf->jcf.p_joint_animace_akt ? p_cnf->jcf.p_joint_animace_akt->p_child : NULL;
   }
...
{
  if(p_obj == oe_cti_joint_objekt(p_cnf) && p_cnf->jcf.k == p_cnf->vybr_kont && p_cnf->jcf.o == p_cnf->vybr_objekt) {
...
{  
  return((vzdal_bodu_a_primky(p_pivot,_1,_2) < RADIUS_KOSTI) ? vzdal_bodu_bod(p_pivot,_1) : FLT_MAX);
}

JOINT * oe_najdi_nejlepsi_joint(JOINT *p_joint, float *p_nej, BOD *_1, BOD *_2)
{
...
int oe_prikaz_vyber_mysi_kosti(K_EDITOR *p_cnf)
{
 ...
 if(handle) {
   EDIT_OBJEKT    *p_obj = oe_vybr_obj_get_ind(&handle,&p_cnf->jcf.k,&p_cnf->jcf.o);
   ...
   transf_2d_3d_world((float)mi.x, (float)mi.y, 0, &_1.x, &_1.y, &_1.z,
     p_world, p_cnf->p_camera, p_cnf->p_project, OXRES, OYRES, OXSTART, OYSTART);
   transf_2d_3d_world((float)mi.x, (float)mi.y, 0.5f, &_2.x, &_2.y, &_2.z,
     p_world, p_cnf->p_camera, p_cnf->p_project, OXRES, OYRES, OXSTART, OYSTART);
...
{ 
 ...
 for(i = 0; i < MAX_EDIT_SVETEL; i++) {
   if(p_cnf->light[i].akt) {
     ...
     t = (vzdal_bodu_a_primky(p_pos,&_1,&_2) < RADIUS_SVETLA) ? vzdal_bodu_bod(p_pos,&_1) : FLT_MAX;
     if(t < tmin) {
       tmin = t;
       best = i;       
     }
...
void oe_prikaz_prirad_material_objekt(K_EDITOR *p_cnf)
{
 ...
 p_cnf->p_kont[k]->m1flag |= (p_cnf->p_kont[k]->p_obj[o]->m1flag = p_cnf->p_mat[p_cnf->km.vybr_mat]->flag);
 p_cnf->p_kont[k]->m2flag |= (p_cnf->p_kont[k]->p_obj[o]->m2flag = p_cnf->p_mat[p_cnf->km.vybr_mat]->flag2);
...
// int x,y;
 ...
 for(o = 0; o < p_kont->max_objektu; o++) {
   if(p_kont->p_obj[o]) {
     ...
     p_kont->m1flag |= (p_kont->p_obj[o]->m1flag = p_cnf->p_mat[p_cnf->km.vybr_mat]->flag);
     p_kont->m2flag |= (p_kont->p_obj[o]->m2flag = p_cnf->p_mat[p_cnf->km.vybr_mat]->flag2);
   }
 } 
...
void oe_mys_editace(K_EDITOR *p_cnf)
{
 if(mi.t1) {
   ...
   if(key[K_SLASH]&&key[K_CTRL]) {
     key[K_SLASH] = FALSE; 
     doe_proved_prikaz_menu(ID_PRIKAZ_PRIRAD_MATERIAL_KONTEJNER,TRUE,FALSE);     
     mi.t1 = FALSE;
   }
 }
}
...
void go_object_editor(char *p_edit_kont, char *p_text_dir)
{
 ...
 if(p_edit_kont[0] && strstr(p_edit_kont,KONCOVKA_MESH)) {
   ...
   if((cnf.p_kont[k] = oe_nahraj_kontejner_bez_dialog(&cnf, p_edit_kont, TRUE)) == NULL) {
 ...
 if(p_edit_kont[0] && strstr(p_edit_kont,".3ds")) {
   ...
   strcpy(pom,cnf.dir.out_dir);
   strcpy(cnf.dir.out_dir,work_dir);     
   if((cnf.p_kont[k] = oe_nahraj_kontejner_bez_dialog_3ds(&cnf, p_edit_kont, TRUE)) == NULL) {
     ddw("Sakra, nejaka chyba loadu ! file (%s)",p_edit_kont);
   }
   else {
     strcpy(cnf.p_kont[k]->jmeno,p_edit_kont);
...
void ddw(byte *p_text,...)
{
 ...
 MessageBox(hwnd_aplikace,text,"DDW:",MB_ICONASTERISK|MB_OK|MB_SYSTEMMODAL);
}
...
void oe_prikaz_vypis_face(K_EDITOR *p_cnf)
{
  ...
  for(i = 0; i < p_cnf->groupfacenum; i++) {
    kprintf(TRUE,"%d k[%d] o[%d] f[%d]",i,
                 ...
                 p_cnf->p_group_face[i].o,
                 p_cnf->p_group_face[i].f);
  }   
...
{
  float scale;
  if(doe_dialog_scale("Scale sceny","Validni je prvni cislo - symetricky scale", &scale)) {
...
void oe_prikaz_posun_scenu(K_EDITOR *p_cnf)
{
  ...
  calc_transf_3d_2d_matrix(&p_cnf->init_matrix,p_cnf->p_camera,p_cnf->p_project,&mat);
  *p_cnf->p_vzdal = oe_stredni_vzdalenost_ve_scene(p_cnf);
...
{  
  return(p_cnf->groupflag&GROUP_KONT && !(p_cnf->groupflag&(GROUP_OBJ|GROUP_FACE)));
}
