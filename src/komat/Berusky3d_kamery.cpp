...
void ber_kamera_zpruhledni_mesh(GAME_MESH_OLD * p_mesh, BOD * p_cam,
  BOD * p_stred)
{
  BODRGBA *p_d, *p_dm;
  OBB_OLD *p_obb;
  ...
  if (in) {
    ...
    if (p_mesh->p_data->m1flag & MAT_PRUHLEDNY
      || p_mesh->p_data->m2flag & MAT2_MASKA) {
      ber_reset_mesh(p_mesh->p_vertex_diff, p_mesh->p_vertex_diff_material,
        p_mesh->vertexnum);
      p_mesh->p_data->k2flag |= KONT2_UPDATE_DIFF;
    }
  }
...
void ber_kamera_zpruhledni_objekty(G_KONFIG * p_ber)
{
  ...
  while ((p_mesh =
      ber_mesh_render_list_next_flag(p_ber, KONT_VIDITELNY, KONT_PRVEK))) {
    if (p_mesh->p_vertex_diff && (pohyb_kamery
  ...
  if (pohyb_kamery) {
    ...
    while ((p_poly =
        ber_poly_render_list_next_flag(p_ber, KONT_VIDITELNY, KONT_PRVEK))) {
...
void ber_kamera_zpruhledni_objekty_reset(G_KONFIG * p_ber)
{
  ...
  for (i = 0; i < p_ber->meshnum; i++) {
    ...
    if (p_mesh && !(p_mesh->p_data->kflag & KONT_PRVEK)) {
      if ((p_mesh->p_data->m1flag & MAT_PRUHLEDNY
          || p_mesh->p_data->m2flag & MAT2_MASKA) && p_mesh->p_vertex_diff) {
        ber_reset_mesh(p_mesh->p_vertex_diff, p_mesh->p_vertex_diff_material,
          p_mesh->vertexnum);
        p_mesh->p_data->k2flag |= KONT2_UPDATE_DIFF;
      }
  ...
  int korekce_vzdal)
{
  ...
  BOD p, pos;
  GLMATRIX *p_cam = p_ber->p_camera, *p_invcam = p_ber->p_invcam;
  float t;


  calc_camera_bod(p_cam, p_invcam, p_ber->p_target, *p_ber->p_vzdal,
    *p_ber->p_fi, *p_ber->p_rfi);
  ...
  if (!korekce) {
    *p_ber->p_vzdal_kor = *p_ber->p_vzdal;
     ...
     */
    if (kd_je_bod_v_kostce((BOD *) & (p_invcam->_41), &p_kam->min,
        &p_kam->max)
      ...
      if (p_ber->kam.min_vzdal > *p_ber->p_vzdal) {
        *p_ber->p_vzdal_kor = p_ber->kam.min_vzdal;
        calc_camera_bod(p_cam, p_invcam, p_ber->p_target, *p_ber->p_vzdal_kor,
          *p_ber->p_fi, *p_ber->p_rfi);
      }
      else {
        *p_ber->p_vzdal_kor = *p_ber->p_vzdal;
      ...
      kd_intersect_kostku_bod_inter(p_ber->p_target, &pos, &p_kam->min,
        &p_kam->max, &p, &t);
...
inline void kam_set_mod(int mod)
{
  ...
  if (p_ber->kamera.aktivni & GAME_KAMERA_3DS) {
    ...
    polar2kar(p_ber->kamera.r, p_ber->kamera.fi, p_ber->kamera.vzdal,
      &p_ber->kamera.p);
...
naslo_se_to2:;
  key_kamera_vyrob_indir(p_ber->kamery + kam, pos_keys, 0, vzdal_keys, 0,
    rot_keys);
  p_ber->kamery[kam].flag =
    (korekce) ? GAME_KAMERA_KOREKCE | GAME_KAMERA_POLAR : GAME_KAMERA_POLAR;
  ...
  float fi, int frame)
{
  if (handle >= p_ber->kamnum || klic >= p_ber->kamery[handle].quat_keys)
    return (K_CHYBA);
  kam_angle_to_quat(DEG2RAD(r), DEG2RAD(fi),
    p_ber->kamery[handle].p_quat + klic);
...
naslo_se_to2:;
  key_kamera_vyrob_indir(p_ber->kamery + kam, pos_keys, target_keys,
    roll_keys, fov_keys, 0);
...
RunHandle kam_start(AnimHandle a_handle, int *p_flag, int flag, int start, int stop)
{
  if (p_ber->kamery[a_handle].endtime) {
    ...
    p_ber->kamera.aktivni =
      p_ber->kamery[a_handle].flag | GAME_KAMERA_ANIMACE;
  ...
  int *p_flag, int flag, int framenum, float tension)
{
  ...
  kam_pol_vloz_klic_all(ah, 0, &p_kam->t, RAD2DEG(p_kam->r),
    RAD2DEG(p_kam->fi), p_kam->vzdal, 0, tension);
  ...
  int *p_flag, int flag, int framenum, float tension)
{
  ...
  kam_pol_vloz_klic_all(ah, 1, vektor_add(&p_kam->t, p_p, &pp),
    RAD2DEG(p_kam->r) + r, RAD2DEG(p_kam->fi) + fi, p_kam->vzdal + vzdal,
    framenum - 1, tension);
  ...
  int flag, int framenum, float tension)
{
  ...
  kam_3ds_vloz_klic_all(ah, 1, p_p, p_t, DEG2RAD(roll), framenum - 1,
    tension);
  ...
  int flag, int framenum, float tension)
{
  ...
  kam_3ds_vloz_klic_all(ah, 1, vektor_add(&p_kam->p, p_p, &p),
    ...
    p_kam->roll + DEG2RAD(roll), framenum - 1, tension);
  ...
  for (i = 0; i < p_ber->meshnum; i++) {
    ...
    if (p_mesh) {
      if (p_mesh->p_data->kflag & KONT_PRVEK &&
        (p_mesh->p_data->m1flag & MAT_PRUHLEDNY
          || p_mesh->p_data->m2flag & MAT2_MASKA) && p_mesh->p_vertex_diff) {
        ber_reset_mesh(p_mesh->p_vertex_diff, p_mesh->p_vertex_diff_material,
          p_mesh->vertexnum);
        p_mesh->p_data->k2flag |= KONT2_UPDATE_DIFF;
      ...
      if (!bod_mezi(&p, &t, &i)) {
        if ((p_mesh->p_data->m1flag & MAT_PRUHLEDNY
          ...
          ber_reset_mesh(p_mesh->p_vertex_diff,
            p_mesh->p_vertex_diff_material, p_mesh->vertexnum);
          p_mesh->p_data->k2flag |= KONT2_UPDATE_DIFF;
        }
        p_mesh->p_data->kflag &= ~KONT_DRAW_CAMERA;
      }
