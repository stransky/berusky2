...
static int Read_Skip (FILE *f) {if (f) {}; return OK;}
...
{
  short v;
  if (fread(&v, sizeof(v), 1, f) != 1) return ERROR_READER;
  v++;
...
{
  float c[3];
  if (fread(c, sizeof(c), 1, f) != 1) return ERROR_READER;
...
{
  word a;
  if (fread (&a, sizeof (a), 1, f) != 1) return ERROR_READER;
...
{
  float s;
  if (fread (&s, sizeof (s), 1, f) != 1) return ERROR_READER;
...
static int Read_VertFlags (FILE *f)
{
  ...
  if (fread(&nv, sizeof(nv), 1, f) != 1) return ERROR_READER;
//  spaces(ind);
//  printf("Vertices flags: %d\n", nv);

  while (nv-- > 0)
  {
    if (fread(&fl, sizeof(fl), 1, f) != 1) return ERROR_READER;
//    spaces(ind+4);
//    printf("Flags: %d\n", fl);
  }
  return OK;
...
// najdu si material pro akt objekt
  ...
  if (fread(&n, sizeof(n), 1, f) != 1) return ERROR_READER;
//  spaces(ind);
//  printf("Faces with this material: %d\n", n);

  while (n-- > 0) {
    if (fread(&nf, sizeof(nf), 1, f) != 1) return ERROR_READER;
//    spaces(ind+4);
//    printf("Face %d\n", nf);
  }
  return OK;
...
static int Read_BoxMap(FILE *f)
{
  ...
  if (ASCIIZReader(f)!=OK) return ERROR_READER;
  if (ASCIIZReader(f)!=OK) return ERROR_READER;
  if (ASCIIZReader(f)!=OK) return ERROR_READER;
  if (ASCIIZReader(f)!=OK) return ERROR_READER;
  if (ASCIIZReader(f)!=OK) return ERROR_READER;
...
static int Read_SpotLight(FILE *f)
{
  float c[5];
  if (fread(c, sizeof(c), 1, f) != 1) return ERROR_READER;
  oe_wrlog(p_cnf,"Spotlight - Target X: %f, Y: %f, Z: %f; Hotspot %f, Falloff %f",
          c[0], c[1], c[2], c[3], c[4]);
  return OK;
}
...
static int Read_MatName (FILE *f)
{
  ...
  if((akt_matros = lo_najdi_prazdny_material(p_cnf->p_mat,MAX_EDIT_MATERIALU)) == K_CHYBA) {
  ...
  kprintf(TRUE,"Material %d '%s'",akt_matros,p_cnf->p_mat[akt_matros]->jmeno);
  return OK;
}
...
{
  float w;
  if (fread(&w, sizeof(w), 1, f) != 1) return ERROR_READER;
...
{
  word flags;
  if (fread (&flags, sizeof (flags), 1, f) != 1) return ERROR_READER;
...
{
  float blur;
  if (fread (&blur, sizeof (blur), 1, f) != 1) return ERROR_READER;
...
{
  float u;
  if (fread(&u, sizeof (u), 1, f) != 1) return ERROR_READER;
...
static int Read_TrackObjName(FILE *f)
{
  ...
  if (ASCIIZReader_muj(f,pom,50) != OK) return ERROR_READER;
  if (fread(w, sizeof(w), 1, f) != 1) return ERROR_READER;
  if (fread(&parent, sizeof(parent), 1, f) != 1) return ERROR_READER;
  
  kprintf(TRUE,"Track object name %s",pom);
  kprintf(TRUE,"Object name data: Flags 0x%X, 0x%X, Parent %d", w[0], w[1], parent);
...
{
  byte jmeno[500];
  if (ASCIIZReader_muj(f,jmeno,500)!=OK) return ERROR_READER;
...
static int Read_PivotPoint(FILE *f)
{
  ...
  if (fread(pos, sizeof(pos), 1, f) != 1) return ERROR_READER;
...
static int SplineFlagsReader(FILE *f, int ind, word * nf, KEY_POINT_3DS *p_key)
{
  unsigned int i;
  float dat;
  word unknown;
  ...
  if (fread(nf, sizeof(word), 1, f) != 1) return ERROR_READER;
  if (fread(&unknown, sizeof(unknown), 1, f) != 1) return ERROR_READER;
  if (fread(&flags, sizeof(flags), 1, f) != 1) return ERROR_READER;
//  spaces(ind+2);
//  printf("Frame %3d: Flags 0x%X\n", *nf, flags);
  ...
  for (i = 0; i < 16; i++)
  {
    ...
    if (flags & (1 << i))
    {
      if (fread(&dat, sizeof(dat), 1, f) != 1) return ERROR_READER;
...
{  
  if(fread (p_flags, sizeof(word)*7, 1, f) != 1)
    return ERROR_READER;
  if ((p_flags[0] & 0x02) == 0x02) oe_wrlog(p_cnf,"repeat");
  if ((p_flags[0] & 0x03) == 0x03) oe_wrlog(p_cnf,"loop");
  *n = p_flags[5];
  return OK;
}
...
static int Read_TrackPos(FILE *f)
{
  ...
  if(n) {
    ...
    if(!check_track(p_key,n,&klic)) {
      kprintf(TRUE,"**** Vicenasobny frame na klici cislo %d frame %d",klic,p_key[klic]);
      //assert(0);
    }
  }
  return OK;
...
static int Read_TrackRot(FILE *f)
{
  ...
  if(n) {    
    ...
    for(i = 0; i < n; i++) {
      ...
      oe_wrlog(p_cnf,"Angle: %f§, X: %f, Y: %f, Z: %f", v[0]*180.0/PI, v[1], v[3], v[2]);
      key_rotkey_to_quat(p_track->p_rot+i, &q);
      oe_wrlog(p_cnf,"Prevod - Quat: X: %f, Y: %f, Z: %f W: %f", q.x, q.y, q.z, q.w);
      quat_to_angle(&q,&osa,&uhel);
      oe_wrlog(p_cnf,"Prevod - Angle: %f§ (%f), X: %f, Y: %f, Z: %f", uhel*180.0/PI,uhel, osa.x, osa.y, osa.z);
    }
    
    if(!check_track(p_track->p_rkeys,n,&klic)) {
      kprintf(TRUE,"**** Vicenasobny frame na klici cislo %d frame %d",klic,p_track->p_rkeys[klic]);
    }
  }
  return OK;
}
...
{  
  ...
  if(n) {
    ...
    for(i = 0; i < n; i++) {
      ...
      if(fread(&fov, sizeof(kamery[akt_kamera].tracky[0].p_fov[0]), 1, f) != 1)
        return ERROR_READER;
...
static int Read_TrackRoll(FILE *f)
{
  ...
  if(n) {
    ...
    for(i = 0; i < n; i++)  {
      ...
      if(fread(&roll, sizeof(kamery[akt_kamera].tracky[0].p_roll[0]), 1, f) != 1)
        return ERROR_READER;
...
static int Read_TrackScale(FILE *f)
{
  ...
  if(n) {    
    ...
    if(!check_track(p_track->p_skeys,n,&klic)) {
      kprintf(TRUE,"**** Vicenasobny frame na klici cislo %d frame %d",klic,p_track->p_skeys[klic]);
      //assert(0);
    }
  }
  return OK;
 ...
 if(p_track) {
   ...
   while(p_track) {
     ...
     if(!strcmp(p_track->jmeno,DUMMY_OBJEKT)) {
       ...
       ddw("Importovany soubor obsahuje DUMMY objekt (cislo %d) !!\n Objekty se muzou spatne importovat a animovat !!",o);
       ...
       if((o = lo_najdi_objekt_kont(&k, p_track->jmeno)) != K_CHYBA) {
         if(p_track->objekt == o) {
           ...
           // je to kopie
           kprintf(TRUE,"Kopiruji objekt '%s' z %d na %d",p_track->jmeno,o,p_track->objekt);
...
void *ds_vyrob_chunk_material_shiness_strenght(EDIT_MATERIAL *p_mat, int *p_velikost)
{
  byte *p_tmp;
  word c = 0;
  TChunkHeader head;
