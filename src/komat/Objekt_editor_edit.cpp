...
EDIT_OBJEKT * oe_vybr_obj_get_ind(int *p_handle, int *p_k, int *p_o)
{
 ...
 if(p_cnf->groupfacenum && 
   ...
   if(h < p_cnf->groupfacenum) {
     ...
     if(p_kont) {
       p_obj = p_kont->p_obj[p_cnf->p_group_face[h].o];
       if(p_k) *p_k = p_cnf->p_group_face[h].k;
       if(p_o) *p_o = p_cnf->p_group_face[h].o;
       (*p_handle)++;
     }
   }
 } else {
   if(!h) {
     p_kont = p_cnf->p_kont[p_cnf->vybr_kont];
     p_obj = p_kont->p_obj[p_cnf->vybr_objekt];
     if(p_k) *p_k = p_cnf->vybr_kont;
     if(p_o) *p_o = p_cnf->vybr_objekt;
     (*p_handle)++;
   }
 }
 return(p_obj);
...
void oe_prikaz_kopiruj_kontejner_vyber(K_EDITOR *p_cnf)
{
  ...
  if(doe_kopie_kontejneru(NULL,&flag)) {
    kprintf(TRUE,"Kopiruji z '%s' do '%s'...",p_cnf->p_kont[k2]->jmeno,p_cnf->p_kont[k]->jmeno);
...
EDIT_KONTEJNER * oe_slep_kontejnery_vsechny(K_EDITOR *p_cnf)
{
 ...
 for(i = 0; i < KONT_NUM; i++) {
   ...
   if(p_kont) {
     p_kont = kopiruj_kontejner(p_kont,NULL,NULL,0,p_cnf->light,MAX_EDIT_SVETEL,
                                p_cnf->p_dlight,MAX_FLARE_SVETEL,p_cnf->p_edlight,
...
void oe_prikaz_rotace_kontejneru_osove(K_EDITOR *p_cnf, int x, int y, int z)
{
 ...
 while(stisk_mys()) {
   ...
   if(x) {
     rotate_matrix_x(&p_kont->world,((float)PI/180)*(float)mi.dx*0.5f);
   }
   if(y) {
     rotate_matrix_y(&p_kont->world,((float)PI/180)*(float)mi.dx*0.5f);
   }
   if(z) {
     rotate_matrix_z(&p_kont->world,((float)PI/180)*(float)mi.dx*0.5f);
   }
   key_kontejner_sim_reanimuj(p_kont);
   nuluj_dif();
   oe_renderuj_scenu_pre(p_cnf);
...
void oe_prikaz_transformace_kontejneru(K_EDITOR *p_cnf)
{
 ...
 if(p_kont->kflag&KONT_KEYFRAME && ddwqueto(hwnd_aplikace,"Transformovat key-frame?")) { 
...
********************************************************************************
*/
void scale_vec(OBJ_VERTEX *p_vert, word pocet, float t_x, float t_y, float t_z)
{
 GLMATRIX m;
 dword     i;
 float     x,y,z;
...
void oe_prikaz_scale_objektu(K_EDITOR *p_cnf)
{
 ...
 while(stisk_mys()) {    
   ...
   scale_vec(p_cnf->p_kont[k]->p_obj[o]->p_vertex, p_cnf->p_kont[k]->p_obj[o]->vertexnum, f1, f1, f1);
...
void oe_prikaz_rotace_objektu(K_EDITOR *p_cnf)
{
 ...
 while(stisk_mys()) {
   ...
   for(i = 0; i < p_obj->vertexnum; i++) {
     transformuj_bod_matici(&p_obj->p_vertex[i].x, &p_obj->p_vertex[i].y, &p_obj->p_vertex[i].z, &v);
   }
...
void oe_prikaz_rotace_objektu_osove(K_EDITOR *p_cnf)
{
 ...
 while(stisk_mys()) {  
   ...
   for(i = 0; i < p_obj->vertexnum; i++)
     transformuj_bod_matici(&p_obj->p_vertex[i].x, &p_obj->p_vertex[i].y, &p_obj->p_vertex[i].z, &m);
...
void oe_prikaz_posun_animace_kosti_rec(JOINT *p_child, float dx, float dy, float dz)
{ 
  while(p_child) {
    p_child->pivot.x += dx;
    p_child->pivot.y += dy;
...
void oe_prikaz_posun_animace_kosti(EDIT_OBJEKT *p_obj, float dx, float dy, float dz)
{
...
// a tento objekt vybere
 ...
 p_cnf->p_kont[k]->p_obj[p_cnf->vybr_objekt] = kopiruj_objekt(p_cnf->p_kont[k]->p_obj[o]);
 strcpy(p_cnf->p_kont[k]->p_obj[p_cnf->vybr_objekt]->jmeno,p_cnf->p_kont[k]->p_obj[o]->jmeno);
 updatuj_kontejner_statistika(p_cnf->p_kont[k],TRUE);
}
...
{
 system_kurzor = 1;
 if(ddwqueto(hwnd_aplikace,"Chceö smazat celou scÈnu ? PeËlivÏ tento riskatnÌ postoj zvaû, neb jiû nelze vzÌti zpÏt!\nNechù tv˘j monitor nepozn· sÌlu tvÈno hnÏvu a aù tvoji kl·vesnici neskr·pÌ krokod˝lÌ slzy!")) {
   oe_smaz_scenu(p_cnf);     
 }
 system_kurzor = 0;
}
...
*/
__inline static void face_korekce(F_VERTEX *p_list, int num, int k, int o, int f)
{
  int i;
  for(i = 0; i < num; i++) {
...
void oe_smaz_vertex(K_EDITOR *p_cnf, int k, int o, int v)
{
 ...
 for(i = 0; i < p_cnf->p_kont[k]->p_obj[o]->facenum; i +=3 ) {
   if( p_face[i] == v || p_face[i+1] == v || p_face[i+2] == v ) {
     ...
     if(p_face[i] > v) p_face[i]--;
     if(p_face[i+1] > v) p_face[i+1]--;
     if(p_face[i+2] > v) p_face[i+2]--;
   }
 }
...
static void pridej_vertex(GROUP_MAZ_POM_OBJ *p_grp, int v)
{
  if(!p_grp->p_vertexlist) {
    ...
    p_grp->vertexakt = 0;
    p_grp->p_vertexlist = mmalloc(sizeof(p_grp->p_vertexlist)*p_grp->vertexnum);
  }

  if(p_grp->vertexakt+1 >= p_grp->vertexnum) {
    p_grp->vertexnum += SEGMENT;    
    p_grp->p_vertexlist = realloc(p_grp->p_vertexlist,sizeof(p_grp->p_vertexlist)*p_grp->vertexnum);
  }

  p_grp->p_vertexlist[p_grp->vertexakt++] = v;  
}

static GROUP_MAZ_POM_OBJ * oe_pridej_vertex_do_seznamu_obj(GROUP_MAZ_POM_OBJ *p_klo, int o, int v)
{  
...
static GROUP_MAZ_POM_KONT * oe_pridej_vertex_do_seznamu(GROUP_MAZ_POM_KONT *p_kls, int k, int o, int v)
{ 
...
static void oe_vymaz_vertexy_objektu(EDIT_OBJEKT *p_obj, int *p_list, int num)
{
  OBJ_VERTEX *p_tmp;
  int        *p_ind;
  FACE       *p_fc;
...
static void oe_group_vymaz_seznam_obj(K_EDITOR *p_cnf, GROUP_MAZ_POM_OBJ *p_klo, int k)
{
  // vymazu vertexy aktualni objektu
  oe_vymaz_vertexy_objektu(p_cnf->p_kont[k]->p_obj[p_klo->o], p_klo->p_vertexlist, p_klo->vertexakt);
  if(p_klo->p_obj) {
    oe_group_vymaz_seznam_obj(p_cnf, p_klo->p_obj, k);
  }
}

static void oe_group_vymaz_seznam_kont(K_EDITOR *p_cnf, GROUP_MAZ_POM_KONT *p_kls)
{
  if(!p_kls)
    return;
...
void prusecik_normalbod(NORMALBOD *p_a, NORMALBOD *p_b, NORMALBOD *p_p, NORMALBOD *p_I)
{
  float t_i,vz;
  BOD   q;
...
{  
  ...
  for(i = 0; i < p_cnf->groupnum; i++) {
    ...
    for(j = i+1; j < p_cnf->groupnum; j++) {
      ...
      if(stejny_vertex_point_delta((BOD *)(p_obj->p_vertex+v),(BOD *)(p_obj->p_vertex+nv),DELTA_SLICKA)) {
...
//    while(stisk_mys());
    ...
    for(i = 0; i < p_cnf->groupnum; i++) {
      if(k != p_cnf->p_group[i].k) continue;
      if(i == g) continue;
      o = p_cnf->p_group[i].o;
      v = p_cnf->p_group[i].v;
...
{
 p_cnf->groupnum = oe_vrat_body_ve_scene(p_cnf,p_cnf->x1,p_cnf->y1,p_cnf->x2,p_cnf->y2,NULL);
 p_cnf->vybr_group = 0;
 if(p_cnf->p_group) {
   free(p_cnf->p_group);
   p_cnf->p_group = NULL;
 ...
 if(p_cnf->groupnum) {
   if((p_cnf->p_group = (I_VERTEX *) malloc(sizeof(I_VERTEX)*p_cnf->groupnum)) == NULL) {
     chyba("Pamet");
   }
   oe_vrat_body_ve_scene(p_cnf,p_cnf->x1,p_cnf->y1,p_cnf->x2,p_cnf->y2,p_cnf->p_group);
 }
}

void oe_vrat_vertexy_ze_ctverce(K_EDITOR *p_cnf, I_VERTEX **p_group, int *p_groupnum)
{
 *p_groupnum = oe_vrat_body_ve_scene(p_cnf,p_cnf->x1,p_cnf->y1,p_cnf->x2,p_cnf->y2,NULL);
 if((*p_group = (I_VERTEX *) malloc(sizeof(I_VERTEX)*(*p_groupnum) )) == NULL) {
   chyba("Pamet");
 }
 oe_vrat_body_ve_scene(p_cnf,p_cnf->x1,p_cnf->y1,p_cnf->x2,p_cnf->y2,*p_group);
}
...
int oe_je_vertex_v_seznamu(K_EDITOR *p_cnf, int k, int o, int v)
{
 ...
 for(i = 0; i < n; i++) {
   if(p_cnf->p_group[i].v == v && p_cnf->p_group[i].o == o && p_cnf->p_group[i].k == k ) {
     return(i+1);
   }
 }
 return(0);
...
void oe_pridej_vertexy_ze_ctverce(K_EDITOR *p_cnf)
{
 ...
 novych = oe_vrat_body_ve_scene(p_cnf,p_cnf->x1,p_cnf->y1,p_cnf->x2,p_cnf->y2,NULL);
 if((p_grp = (I_VERTEX *) malloc(sizeof(I_VERTEX)*novych)) == NULL) {
   chyba("Pamet");
 }
 
 oe_vrat_body_ve_scene(p_cnf,p_cnf->x1,p_cnf->y1,p_cnf->x2,p_cnf->y2,p_grp);
 ...
 if((p_cnf->p_group = (I_VERTEX *) realloc(p_cnf->p_group,sizeof(I_VERTEX)*(p_cnf->groupnum+j))) == NULL)
   chyba("Pamet");
...
void oe_pridej_groupvertex(K_EDITOR *p_cnf, int k, int o, int v)
{
  ...
  if(p_cnf->groupnum) {
    ...
    p_cnf->p_group = realloc(p_cnf->p_group,(p_cnf->groupnum+1)*sizeof(p_cnf->p_group[0]));
    mtest(p_cnf->p_group);
  } else {
    p_cnf->p_group = malloc((p_cnf->groupnum+1)*sizeof(p_cnf->p_group[0]));
    mtest(p_cnf->p_group);
  }
...
void oe_prikaz_pridej_vertex_face(K_EDITOR *p_cnf)
{
 ...
 for(i = 0; i < n; i++) {
   if(p_cnf->p_group[i].v == v1 && p_cnf->p_group[i].o == o && p_cnf->p_group[i].k == k ) {
     v1 = 0xfffff; d++;
   }
   if(p_cnf->p_group[i].v == v2 && p_cnf->p_group[i].o == o && p_cnf->p_group[i].k == k ) {
     v2 = 0xfffff; d++;
   }
   if(p_cnf->p_group[i].v == v3 && p_cnf->p_group[i].o == o && p_cnf->p_group[i].k == k ) {
     v3 = 0xfffff; d++;
   }
 }
 ...
 p_cnf->groupnum += 3-d;
 if((p_cnf->p_group = (I_VERTEX *) realloc(p_cnf->p_group,sizeof(I_VERTEX)*p_cnf->groupnum)) == NULL)
   chyba("Pamet");
...
void oe_ungroup_vertex(K_EDITOR *p_cnf, int cislo)
{
 if(p_cnf->groupnum) {
   if(cislo+1 < p_cnf->groupnum)
     memmove(p_cnf->p_group+cislo,p_cnf->p_group+cislo+1,(p_cnf->groupnum-cislo-1)*sizeof(p_cnf->p_group[0]));
   p_cnf->groupnum--;
 }
 if(!p_cnf->groupnum) {
   free(p_cnf->p_group);
...
void oe_prikaz_ungroup_group_vertex(K_EDITOR *p_cnf)
{
 ...
 for(i = 0; i < groupnum; i++) {
   v = oe_je_vertex_v_seznamu(p_cnf, p_group[i].k, p_group[i].o, p_group[i].v);
   if(v)
...
void oe_edit_vertex_uv(K_EDITOR *p_cnf, int i, int t, byte *p_flag, float u, float t2)
{
 ...
 if(p_flag[0]) p_fl[0+t*2] = u;
 if(p_flag[1]) p_fl[1+t*2] = t2;
}
...
void oe_add_vertex_uv(K_EDITOR *p_cnf, int i, int t, byte *p_flag, float u, float t2)
{
 float *p_fl;
 int    k,o,v;
...
void oe_prikaz_nasobeni_vertex(K_EDITOR *p_cnf, float *p_f, byte *p_flag)
{
 ...
 if(p_flag[3] || p_flag[4]) {
   oe_nasob_vertex_uv(p_cnf, p_cnf->vybr_group, 0, p_flag+3, p_f[3], p_f[4]);
 }
 if(p_flag[5] || p_flag[6]) {
   oe_nasob_vertex_uv(p_cnf, p_cnf->vybr_group, 1, p_flag+5, p_f[5], p_f[6]);
 }
}
...
void oe_prikaz_posun_group_vertex(K_EDITOR *p_cnf)
{
 ...
 if((p_mat = (GLMATRIX *) malloc(sizeof(GLMATRIX)*p_cnf->groupnum)) == NULL)
   chyba("Nedostatek pameti."); 

 for(i = 0; i < p_cnf->groupnum; i++)
   calc_transf_3d_2d_matrix(kont_world_matrix(p_cnf->p_kont[p_cnf->p_group[i].k]),
                            p_cnf->p_camera,p_cnf->p_project,p_mat+i);
 ...
 *p_cnf->p_vzdal = oe_stredni_vzdalenost_ve_scene(p_cnf);
 ...
 while(stisk_mys()) {
   ...
   oe_renderuj_scenu_pre(p_cnf);
   oe_kresli_editacni_stred(p_cnf,(BOD *)&p_cnf->p_kont[k]->p_obj[o]->p_vertex[b].x,flag);
   oe_renderuj_scenu_po(p_cnf);
...
void oe_prikaz_posun_flare(K_EDITOR *p_cnf)
{
 ...
 calc_transf_3d_2d_matrix(&mat,p_cnf->p_camera,
                          p_cnf->p_project,&mat);
 *p_cnf->p_vzdal = oe_stredni_vzdalenost_ve_scene(p_cnf);
 transf_2d_3d_z_matrix((float)mi.x, (float)mi.y, *p_cnf->p_vzdal,
     &v1.x, &v1.y, &v1.z, &mat,OXRES, OYRES, OXSTART, OYSTART,
...
void oe_prikaz_posun_staticke_svetlo(K_EDITOR *p_cnf)
{
 ...
 if(key[K_CTRL]) {
   ...
   sprintf(p_cnf->light[p_cnf->lightakt].jmeno,"Svetlo %d",p_cnf->lightakt);
   key[K_CTRL] = FALSE;
 }
...
{ 
  ...
  if(!p_cnf->p_mlha_akt) {
    ...
    while(stisk_mys()) {
      ...
      scale = mi.dx*0.002f*vektor_velikost(vektor_sub(&p_mlha->max,&p_mlha->min,&v));
...
{ 
  ...
  if(!p_cnf->cnf.kamera_kostka_kreslit) {
    ...
    while(stisk_mys()) {
      ...
      scale = mi.dx*0.002f*vektor_velikost(vektor_sub(&p_kam->max,&p_kam->min,&v));
...
{
 BOD p = {0,0,0}, 
     d = {X_OKOLI_CUBE/KILOMETR,
          Y_OKOLI_CUBE/KILOMETR,
          Z_OKOLI_CUBE/KILOMETR};
 int k = lo_najdi_volny_kontejner(p_cnf->p_kont,p_cnf->max_kont);

 if(doe_uprava_okoli(&d, &p)) {
   d.x *= KILOMETR; d.y *= KILOMETR; d.z *= KILOMETR;
   p.x *= KILOMETR; p.y *= KILOMETR; p.z *= KILOMETR;
...
void oe_tiskni_akt_objekt(K_EDITOR *p_cnf)
{
 ...
 for(v = 0; v < p_obj->vertexnum; v++) {
   fprintf(f,"V %.3d u = %.3f v = %.3f\n", v, p_obj->p_vertex[v].tu1, p_obj->p_vertex[v].tv1);
 }
 fclose(f);
}
...
void oe_pridej_group_face_list_list(F_VERTEX **p_group_face, int *p_groupnum, F_VERTEX *p_new, int num, int flag)
{
...
{
  oe_pridej_group_face_list_list(&p_cnf->p_group_face, &p_cnf->groupfacenum, p_new, num, flag);
}
...
void oe_ungroup_face_seznam(K_EDITOR *p_cnf, F_VERTEX *p_list, int num)
{
  ...
  for(i = 0; i < num; i++) {
    n = najdi_group_face_vertex(p_cnf->p_group_face,p_cnf->groupfacenum,p_list+i,p_cnf->groupflag);
    if(n != K_CHYBA) {
...
{
  smaz_group_face_vertex(p_cnf->p_group_face, &p_cnf->groupfacenum, p_cnf->vybr_face_group);
  p_cnf->vybr_face_group = 0;
}
...
__inline static F_VERTEX * najdi_face_k_vertexu(FACE *p_list, int fnum, int face, int *p_naslych, int k, int o, int flag)
{
  F_VERTEX *p_ret;
  int i,j,naslych;
  int f;
...
F_VERTEX * oe_vrat_face_ze_ctverce(K_EDITOR *p_cnf, int *p_num)
{
 ...
 for(i = 0; i < novych; i++) {
   ...
   p_face = najdi_face_k_vertexu(p_cnf->p_kont[k]->p_obj[o]->p_face,
                                 p_cnf->p_kont[k]->p_obj[o]->facenum,
                                 v,&fnum,k,o,p_cnf->groupflag);
   if(p_face) {
     oe_pridej_group_face_list_list(&p_ret,&retnum,p_face,fnum,p_cnf->groupflag);
     free(p_face);
   }
 }
 free(p_grp);
...
void triangle_div(TRIANGLE tri, int hloubka)
{
  ...
  if(hloubka <= 0) {
    if(_tri_posledni < MAX_LIST_TRI)
      ...
      ddw("Prekrocena max pocet !!! posledni = %d max = %d",_tri_posledni,MAX_LIST_TRI);
    return;
  }
...
int oe_serad_tri_compare(const int *p_vert1, const int *p_vert2)
{
  ...
  if(cmp != 0.0f) return(cmp > 0.0f ? 1 : -1);
...
{
  qsort((void *)p_list,(size_t)num,(size_t)sizeof(OBJ_VERTEX), oe_serad_tri_compare);
}
...
int oe_najdi_vertex(OBJ_VERTEX *p_list, int num, OBJ_VERTEX *p_hled)
{
 ...
 while((res = oe_serad_tri_compare((const int *)p_hled, (const int *)(p_list+s))) 
        && l <= p) {
   if(res > 0)
     l = s+1;
   else
...
int oe_smaz_stejne_vertexy(OBJ_VERTEX *p_list, int num)
{
  ...
  for(v = 1; v < num; v++) {
    // jsou stejne
    if(!oe_serad_tri_compare((const int *)(p_list+v-1), (const int *)(p_list+v))) {
...
void oe_prikaz_osamostatnit_objekt(K_EDITOR *p_cnf)
{
 ...
 if(handle) {
   if((kn = lo_najdi_volny_kontejner(p_cnf->p_kont,p_cnf->max_kont)) == K_CHYBA) {
     ddw("Neni volny kontejner!");
     return;
   }
   ...
   kprintf(TRUE,"Novy kontejner je %s na pozici %d",p_cnf->p_kont[kn]->jmeno,kn);
 }   
}
...
void kamera_vloz_do_klice(KAMERA *p_kam, int flag)
{
  ...
  if(flag&POSUN_POZICI) {
    if((klic = key_je_time_klic(p_track->p_pkeys,p_track->pos_keys,time)) == K_CHYBA) {
      // zaloz novy klic
  ...
  if(flag&POSUN_TARGET) {
    if((klic = key_je_time_klic(p_track->p_tkeys,p_track->trg_keys,time)) == K_CHYBA) {
      // zaloz novy klic
      memset(&nklic,0,sizeof(nklic));
      nklic.time = time;
      p_track->trg_keys = key_track_klic_vloz((byte **)&p_track->p_trg, &p_track->p_tkeys, 
                           p_track->trg_keys,sizeof(p_track->p_trg[0]),
                           (byte *)&p_kam->t, &nklic);
      doe_hs_setmark(frame);
    } else {
  ...
  if(flag&POSUN_ROLL) {
    if((klic = key_je_time_klic(p_track->p_rlkeys,p_track->roll_keys,time)) == K_CHYBA) {
      // zaloz novy klic
      memset(&nklic,0,sizeof(nklic));
      nklic.time = time;
      p_track->roll_keys = key_track_klic_vloz((byte **)&p_track->p_roll, &p_track->p_rlkeys, 
  ...
  if(flag&POSUN_FOV) {
    if((klic = key_je_time_klic(p_track->p_fkeys,p_track->fov_keys,time)) == K_CHYBA) {
      // zaloz novy klic
      memset(&nklic,0,sizeof(nklic));
      nklic.time = time;
      p_track->fov_keys = key_track_klic_vloz((byte **)&p_track->p_fov, &p_track->p_fkeys, 
                           p_track->fov_keys,sizeof(p_track->p_fov[0]),
                           (byte *)&p_kam->fov, &nklic);
      doe_hs_setmark(frame);
    } else {
...
{ 
 ...
 calc_transf_3d_2d_matrix(init_matrix(&mrt),p_cnf->p_camera,p_cnf->p_project,&mat);
 *p_cnf->p_vzdal = oe_stredni_vzdalenost_ve_scene(p_cnf);
 transf_2d_3d_z_matrix((float)mi.x, (float)mi.y, *p_cnf->p_vzdal,&v1.x, &v1.y,
                     &v1.z, &mat,OXRES, OYRES, OXSTART, OYSTART,
                     p_cnf->kam.near_plane,p_cnf->kam.far_plane);
 ...
 while(stisk_mys()) {
   ...
   transf_2d_3d_z_matrix((float)mi.x,(float)mi.y,*p_cnf->p_vzdal,&v2.x, &v2.y, 
                        &v2.z, &mat,OXRES, OYRES, OXSTART, OYSTART,
                        p_cnf->kam.near_plane,p_cnf->kam.far_plane);
...
{
  KAMERA *p_kam = oe_kamera_najdi_volnou(p_cnf->kamery, MAX_EDIT_KAMER, &p_cnf->kamnum);
  BOD  p,t;
  byte pom[200];
  int knum;
...
void oe_prikaz_kamera_nahrat(K_EDITOR *p_cnf)
{
  ...
  if(doe_dialog_load(hwnd_aplikace, pom, 200, "Vyber soubor s kamerou", "*.sik","Soubor s animacema kamer (*.sik)")) {
    memset(p_kam,0,sizeof(*p_kam));
    p_kam->cislo = K_CHYBA;
...
{  
  ...
  if(p_kam->cislo == K_CHYBA)
    ...
    if(!p_track->endtime)
      ...
      // ulozeni kamery
      chdir(p_cnf->dir.game_data_dir);
      if(doe_dialog_save(pom, 200, "Ulozit kameru jako", "*.sik","Soubor s animacema kamer (*.sik)")) {
...
{  
  ...
  kprintf(1,"Ukladam vsechny animace kamery do adresare %s...",p_cnf->dir.game_data_dir);
  chdir(p_cnf->dir.game_data_dir);
...
int oe_spol_prusecik(BOD *p1, BOD *p2, BOD *p3, BOD *v1, BOD *v2, BOD *v3)
{
  int num = 0;
...
void oe_oznac_placate_plosky(K_EDITOR *p_cnf, EDIT_OBJEKT *p_obj, int k, int o)
{
  F_VERTEX face;
  int f,f1;
  ...
  for(f = 0; f < p_obj->facenum; f += 3) {
    for(f1 = 0; f1 < p_obj->facenum; f1 += 3) {
      ...
      if(oe_spol_face(p_obj->p_face+f, p_obj->p_face+f1)) {
        ...
        face.o = o;
        face.f = f;
        oe_pridej_group_face_list(p_cnf,&face,1,GROUP_KONT|GROUP_OBJ|GROUP_FACE);
      }
    }
  }
}
...
{
  BOD norm;
  if(podobny_bod(p_p1,p_p2) || podobny_bod(p_p1,p_p3)||podobny_bod(p_p2,p_p3))
    return(TRUE);
...
void oe_decimace_objektu(K_EDITOR *p_cnf, int k, int o, ELIMINATOR *p_elim)
{
  ...
  if(p_elim->flag&(ELIM_INTELIGENT_1|ELIM_INTELIGENT_2)) {
    ...
    while((p_ktmp = oe_klist_next_maska(p_cnf,&tk,maska,flag))) {
      if(p_ktmp != p_kont && vzdal_bodu_bod(&p_ktmp->obb.obb_stred,&p_obj->obb.obb_stred) < 3.0f) {
        oe_olist_reset(&to);
        while((p_tmp = oe_olist_next(p_ktmp,&to))) {
          if(vzdal_bodu_bod(&p_tmp->obb.obb_stred,&p_obj->obb.obb_stred) < 3.0f) {    
  ...
  */
  if(p_elim->flag&ELIM_INTELIGENT_2) {
    ...
    while(p_list) {
      ...
      for(i = 0; i < vnum1; i++)        
        transformuj_bod_matici_bod((BOD *)(p_obj->p_vertex+i),&world,p_transf_1+i);
      for(i = 0; i < vnum2; i++)        
        transformuj_bod_matici_bod((BOD *)(p_list->p_vertex+i),&wlow,p_transf_2+i);

      spol = 0;
      for(i = 0; i < vnum1; i++) {
        for(j = 0; j < vnum2; j++) {
          if(stejny_vertex_point_delta(p_transf_1+i, p_transf_2+j, p_elim->delta_vzdalenost)) {
...
void oe_prikaz_decimace_vyberu(K_EDITOR *p_cnf)
{
  ...
  if(p_cnf->groupfacenum) {
    ...
    memcpy(p_vyber,p_cnf->p_group_face,sizeof(p_vyber[0])*p_cnf->groupfacenum);
    ...
    */
    if(k&&!o) { // vyber podle kontejneru      
      for(v = 0; v < vyber; v++) {
        k = p_vyber[v].k;
        p_kont = p_cnf->p_kont[k];          
        if(p_kont && ((p_kont->kflag&KONT_STATIC && p_cnf->elm.flag&ELIM_STATIC)||
          ((!(p_kont->kflag&KONT_STATIC) && p_cnf->elm.flag&ELIM_DYNAMIC)))) {
          for(o = 0; o < MAX_KONT_OBJEKTU; o++) {
            if(p_kont->p_obj[o])
              oe_decimace_objektu(p_cnf, k, o, &p_cnf->elm);
          }
...
void oe_prikaz_decimace_trsu(K_EDITOR *p_cnf)
{
  ...
  */
  ...
  calc_face_normal((BOD *)(p_obj->p_vertex+p_obj->p_face[f]),
                   ...
                   (BOD *)(p_obj->p_vertex+p_obj->p_face[f+2]),
                   &n);
  vektor_norm(&n);
  ...
  p_obj->facenum += 3;
  p_obj->p_face = realloc(p_obj->p_face,p_obj->facenum*sizeof(p_obj->p_face[0]));
  mtest(p_obj->p_face);
...
void oe_prikaz_decimace_pruhu_body(K_EDITOR *p_cnf)
{
  ...
  */
  ...
  /* Podle vektoroveho soucinu zjistim
     ctverec
  */  
  vektor_sub((BOD *)(p_obj->p_vertex+vl[2]),(BOD *)(p_obj->p_vertex+vl[0]),&b);
  vektor_sub((BOD *)(p_obj->p_vertex+vl[2]),(BOD *)(p_obj->p_vertex+vl[1]),&c);
...
void oe_objekt_izolovane_vertexy(K_EDITOR *p_cnf, int k, int o)
{
  ...
  for(i = 0; i < p_obj->vertexnum; i++) {
    for(j = i+1; j < p_obj->vertexnum; j++) {
      if(stejny_vertex_point_delta((BOD *)(p_obj->p_vertex+i),(BOD *)(p_obj->p_vertex+j),DELTA_SLICKA)) {
        p_vhit[i]++;
        p_vhit[j]++;
      }
    }
      ...
      for(vf = 0; vf < p_cnf->groupfacenum; vf++) {
        if(vf != v) {
          for(fv = p_cnf->p_group_face[vf].f, mfv = fv+3; fv < mfv; fv++) {
            j = p_obj->p_face[fv];
            if(i != j && stejny_vertex_point_delta((BOD *)(p_obj->p_vertex+i),
                                                   (BOD *)(p_obj->p_vertex+j),
                                                   DELTA_SLICKA)) {
              p_vhit[i]++;
              p_vhit[j]++;
            }
...
/*
*/
void oe_prikaz_rozmnoz_texture_koord_objekt(EDIT_OBJEKT *p_obj, int src, int dest)
{
  BODUV *p_src,*p_dest;
  int i;
  for(i = 0; i < p_obj->vertexnum; i++) {
...
int najdi_vertex_koord(MUJ_BOD *p_vertexy, int vertexnum, MUJ_BOD *p_bod)
{
  ...
  for(i = 0; i < vertexnum; i++) {
    if(p_vertexy[i].x   == p_bod->x && 
       ...
       p_vertexy[i].tu1 == p_bod->tu1 &&
       p_vertexy[i].tv1 == p_bod->tv1) {
      return(i);
    }
  }
  return(K_CHYBA);
...
void oe_calc_rel_vzdal(MUJ_BOD *p_vertex, int num, float *p_vzdal, BOD *p_str, float max_vzdal)
{
  float vzd;
  int   i;
...
void oe_prikaz_kopiruj_objekt_geometrie(K_EDITOR *p_cnf)
{
 int  k = p_cnf->vybr_kont,
      o = p_cnf->vybr_objekt;
 int lk = p_cnf->vybr_kont_last,
     lo = p_cnf->vybr_objekt_last;

 n_objekt(k,o);   
 if(p_cnf->p_kont[lk]->p_obj[lo])
   kopiruj_objekt_geometrie(p_cnf->p_kont[lk]->p_obj[lo],p_cnf->p_kont[k]->p_obj[o]); 
}
...
int oe_kopiruj_texturu(byte *p_file, byte *p_srcdir, byte *p_outdir, byte *p_outfile)
{
  byte  fullpath[500];
  byte  srcfile[500];
 //byte  koncovka[500];
...
*/
void oe_zkrat_materialy(K_EDITOR *p_cnf, byte *p_prefix, byte *p_strcdir, byte *p_outdir, int dos)
{
  EDIT_TEXT *p_text;
  byte file[256], *p_pom;  
  int  m,dl,d,v;
  ...
  for(m = 0, dl = 0; m < MAX_EDIT_TEXTUR; m++) {
    if(p_cnf->p_text[m].load && p_cnf->p_text[m].flag2&TEXT2_POUZITA) {
      if(dos) {        
        sprintf(file,"%s%.4d%s",p_prefix,dl,cti_koncovku(p_cnf->p_text[m].jmeno));
      } else {
...
void oe_prikaz_exportuj_scenu(K_EDITOR *p_cnf)
{
  ...
  if(p_cnf->cnf.druh_sceny == SCENA_POLY) {
    if(!ddwqueto(hwnd_aplikace,"Level je preveden do hry. Ulozeni do standartni sceny nemusi byt nejlepsi. Ulozit ?"))
      return;
  }

  chdir(p_cnf->dir.scene_dir);
  if(doe_dialog_save(file,49,"Ulozit scenu jako",MASKA_SCENY,"Scena")) {
    if(efile(file) && (MessageBox(hwnd_aplikace,"Tento soubor jiz existuje ! Prepsat ?","KZH",MB_YESNO|MB_ICONASTERISK|MB_DEFBUTTON2) == IDNO)) 
      return;
    else {
      if(doe_export_sceny(hwnd_aplikace, prefix, &dos)) {
        oe_zkrat_materialy(p_cnf, prefix, p_cnf->tdir.scene_texture_dir, work_dir, dos);
        oe_oznac_pouzite_materialy(p_cnf);
...
__inline static int oe_kontroluj_objekt(K_EDITOR *p_cnf, EDIT_OBJEKT *p_obj, int k, int o)
{
  return(p_cnf->p_kont[k] ? p_cnf->p_kont[k]->p_obj[o] ? p_cnf->p_kont[k]->p_obj[o] == p_obj : FALSE : FALSE);
}

JOINT * oe_kost_vyrob(K_EDITOR *p_cnf, EDIT_OBJEKT *p_obj, int *p_k, int *p_o)
{    
  JOINT *p_ja;
  int  i,k,o,v,b = 0;
...
void oe_kost_pridej_vertexy(K_EDITOR *p_cnf, JOINT *p_joint, EDIT_OBJEKT *p_obj)
{    
  int  i,k,o,v,b = 0;  
  ...
  p_joint->p_vertexlist = realloc(p_joint->p_vertexlist,(b+p_joint->vertexnum)*sizeof(p_joint->p_vertexlist[0]));
...
void oe_kost_uber_vertexy(K_EDITOR *p_cnf, JOINT *p_joint, EDIT_OBJEKT *p_obj)
{
  int  *p_ind;
  int   i,k,o,v,b = 0;  
  ...
  for(i = 0, b = 0; i < p_cnf->groupnum; i++) {
    ...
    if(oe_kontroluj_objekt(p_cnf, p_obj, k, o)) {
      v = p_cnf->p_group[i].v;
      if(p_ind = oe_je_v_seznamu(p_joint->p_vertexlist, p_joint->vertexnum, v)) {
        *p_ind = -1;
        b++;
      }
    }
  ...
  p_ind = mmalloc((p_joint->vertexnum-b)*sizeof(p_joint->p_vertexlist[0]));
...
void oe_prikaz_kost_pridej_vertexy(K_EDITOR *p_cnf)
{
  if(p_cnf->jcf.p_joint_akt) {
    oe_kost_pridej_vertexy(p_cnf, p_cnf->jcf.p_joint_akt, p_cnf->p_kont[p_cnf->jcf.k]->p_obj[p_cnf->jcf.o]);
  }
}
...
{
  if(p_cnf->jcf.p_joint_akt) {
    oe_kost_uber_vertexy(p_cnf, p_cnf->jcf.p_joint_akt, p_cnf->p_kont[p_cnf->jcf.k]->p_obj[p_cnf->jcf.o]);
  } 
}
...
JOINT_ANIMACE * oe_kost_animaci_smaz(K_EDITOR *p_cnf, EDIT_OBJEKT *p_obj, JOINT_ANIMACE *p_maz)
{
...
void oe_prikaz_kost_smaz(K_EDITOR *p_cnf)
{
  ...
  if(p_conf->p_joint_animace_akt->p_child == p_conf->p_joint_akt) { // smaz celou animaci
    p_conf->p_joint_animace_akt = oe_kost_animaci_smaz(p_cnf, p_obj, p_conf->p_joint_animace_akt);
    if(p_conf->p_joint_animace_akt)
      p_conf->p_joint_akt = p_conf->p_joint_animace_akt->p_child;
  } else {    
    p_prev = oe_kost_najdi_rec(p_conf->p_joint_animace_akt->p_child, p_conf->p_joint_akt);
    if(!p_prev)
      return;
...
void joint_vloz_do_klice(JOINT *p_joint, int flag)
{
  ...
  if(flag&POSUN_ROT) {
    if((klic = key_je_time_klic(p_track->p_rkeys,p_track->rot_keys,time)) == K_CHYBA) {
      memset(&nklic,0,sizeof(nklic));
      nklic.time = time;
  ...
  if(flag&POSUN_SCALE) {
    if((klic = key_je_time_klic(p_track->p_skeys,p_track->scs_keys,time)) == K_CHYBA) {
      memset(&nklic,0,sizeof(nklic));
      nklic.time = time;
...
{  
  ...
  for(f = 0; f < p_obj->facenum; f+=3) {
    ...
    if(v == 3) {
      vzd[0] = vzdal_bodu(p_obj->p_vertex+p_flist[f],  p_obj->p_vertex+p_flist[f+1]);
      vzd[1] = vzdal_bodu(p_obj->p_vertex+p_flist[f+1],p_obj->p_vertex+p_flist[f+2]);
      vzd[2] = vzdal_bodu(p_obj->p_vertex+p_flist[f+2],p_obj->p_vertex+p_flist[f]);
...
int oe_vyrob_material_flare(K_EDITOR *p_cnf, FLARE_GEN *p_gen)
{
  ...
  p_text->p_bmp = oe_vyrob_light_texturu(128, p_gen->start_color, p_gen->konec_color);
  ...
  p_mat->flag2 &= ~(MAT2_DIFFUSE|MAT2_SPECULAR|MAT2_DIFFUSE_RAY|MAT2_SPECULAR_RAY);
  return(mat);
}
...
void oe_prikaz_generuj_flare(K_EDITOR *p_cnf)
{
  ...
  if(doe_cti_flare_gen(hwnd_aplikace, &gen)) {
    ...
    p_new->kflag |= (KONT_STATIC|KONT_VERTEX_LIGHT|KONT_NO_SELF_SHADOW|KONT_NO_DRAW_SHADOW);
    p_new->p_obj[0] = p_obj;
...
{  
  ...
  for(f = 0; f < p_obj->facenum; f += 3) {
    ...
    calc_rovinu(p_obj->p_vertex+f0, p_obj->p_vertex+f1, p_obj->p_vertex+f2, &r);
...
void calc_equad_hrana(OBJ_VERTEX *p_vert, GLMATRIX *p_mat, HRAN *p_hran, int *p_vmask)
{
  ...
  if(p_vmask) {    
    ...
    vd2 = (p_vmask[v1]&&p_vmask[v2]) ? calc_equad_vert(p_q, (BOD *)(&vn)) : -1;
    if(vd0 != -1 && (vd0 <= vd1 || vd1 == -1) && (vd0 <= vd2 || vd2 == -1)) {
      *p_new = p_vert[v1];
      p_hran->err_uhel = p_hran->err = vd0;
    } else if(vd1 != -1 && (vd1 <= vd0 || vd0 == -1) && (vd1 <= vd2 || vd2 == -1)) {
      *p_new = p_vert[v2];
      p_hran->err_uhel = p_hran->err = vd1;
    } else if(vd2 != -1 && (vd2 < vd0 || vd0 == -1) && (vd2 < vd1 || vd1 == -1)) {
      *p_new = vn;
...
int oe_hran_vloz_hrany_stack(HRAN_STACK *p_stack, EDIT_OBJEKT *p_obj)
{
  ...
  for(f = 0; f < p_obj->facenum; f += 3) {
    ...
    f1 = p_obj->p_face[f+1];
    f2 = p_obj->p_face[f+2];    
    calc_face_normal((BOD *)(p_obj->p_vertex+f0),(BOD *)(p_obj->p_vertex+f1),(BOD *)(p_obj->p_vertex+f2),&norm);
    if(maska) {
...
int oe_hran_zrus_hranu(HRAN_STACK *p_stack, HRAN *p_hrana, EDIT_OBJEKT *p_obj, int *p_vmask)
{
  ...
  if(v2+1 < p_obj->vertexnum) {
    memmove(p_obj->p_vertex+v2,p_obj->p_vertex+v2+1,sizeof(p_obj->p_vertex[0])*(p_obj->vertexnum-v2-1));
  }
  ...
  if(p_vmask) {
    ...
    if(v2+1 < p_obj->vertexnum)
      memmove(p_vmask+v2,p_vmask+v2+1,sizeof(p_vmask[0])*(p_obj->vertexnum-v2-1));
    p_vmask[v1] = vr;
  }
...
int oe_hran_vloz(HRAN_STACK *p_stack, int ind1, int ind2, BOD *p_norm)
{
  ...
  if(p_stack->listakt+1 >= p_stack->listnum) {
    ...
    p_stack->p_list = p_stack->p_list ? realloc(p_stack->p_list, sizeof(p_stack->p_list[0])*p_stack->listnum):
                                        mmalloc(sizeof(p_stack->p_list[0])*p_stack->listnum);
...
{
  float err1 = (float)fabs(((HRAN *)p_h1)->err), err2 = (float)fabs(((HRAN *)p_h2)->err);
...
void oe_hran_vyhodnot(HRAN_STACK *p_stack, EDIT_OBJEKT *p_obj, int *p_vmask)
{
  ...
  // Ohodnot vertexy
  for(i = 0; i < p_stack->listakt; i++) {
    calc_equad_hrana(p_obj->p_vertex, p_stack->p_mat, p_stack->p_list+i, p_vmask);
  }

  // Setrid podle chyb
  qsort(p_stack->p_list,p_stack->listakt,sizeof(p_stack->p_list[0]),oe_hran_setrid_sort);
}
...
BOOL CALLBACK doe_callback_decimator_plosek( HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam )
{
  switch(msg)  {
    case WM_COMMAND:
...
int oe_stack_vertexy_pridej(K_EDITOR *p_cnf, EDIT_OBJEKT *p_obj, HRAN_STACK *p_stack, int k, int o, int flag)
{
  ...
  if(flag&VERT_NECHEJ) {
    memset(p_stack->p_vmask,0xffffffff,sizeof(p_stack->p_vmask[0])*p_obj->vertexnum);
    for(vnum = 0, v = 0; v < p_cnf->groupnum; v++) {
      if(p_cnf->p_group[v].k == k && p_cnf->p_group[v].o == o) {
...
{ 
  ...
  p_cnf->ds.hDlg = CreateDialog(hinst,MAKEINTRESOURCE(IDD_DECIMATOR),hwnd_plocha,doe_callback_decimator_plosek);
  ...
  while((p_obj = oe_vybr_obj_get_ind(&handle,&k,&o))) {
    ...
    if(flag&(VERT_NECHEJ|VERT_SMAZ)) {
      ...
      vertexu = p_cnf->ds.vertexy ? p_stack->vertexu : (p_obj->vertexnum - ftoi((float)(p_obj->vertexnum)*p_stack->stupen));
    }
...
int oe_sjednoceni_vertexu_obj(EDIT_OBJEKT *p_obj)
{
  ...
  for(v = 0; v < p_obj->vertexnum; v++) {
    for(vv = v+1; vv < p_obj->vertexnum; vv++) {      
      if(p_obj->p_vertex[vv].x != FLT_MAX && sjednoc_stejny_vertex(p_obj->p_vertex[v],p_obj->p_vertex[vv])) {
...
void oe_generator_animaci_frame(JOINT *p_joint, JOINT_ANIM_GENERATOR *p_gen, int level, int frame)
{
...
void oe_generator_animaci_rady(JOINT_ANIM_GENERATOR *p_gen)
{
  ...
  */
  ...
  for(i = 0; i < p_gen->framu; i++)
    key_track_interpolace_float(p_gen->p_udr_all+i,p_gen->p_udr,p_gen->p_ukeys,
    i*SIM_KONSTI,endtime,p_gen->udr_keys,TRUE);
  ...
  */
  ...
  for(i = 0; i < p_gen->framu; i++)
    key_track_interpolace_float(p_gen->p_vtr_all+i,p_gen->p_vtr,p_gen->p_vkeys,
    i*SIM_KONSTI,endtime,p_gen->vtr_keys,TRUE);
...
void oe_generator_animaci(K_EDITOR *p_cnf)
{
  ...
  p_gen->framu = p_gen->framu_min+ftoi((float)(p_gen->framu_max-p_gen->framu_min)*randf());
...
void oe_prikaz_prevadec_bmp(K_EDITOR *p_cnf)
{
  ...
  if(doe_dialog_load(hwnd_aplikace,pom,199,"Vyber prvni prevadenou bmp","*.bmp","Bitmapy")) {
