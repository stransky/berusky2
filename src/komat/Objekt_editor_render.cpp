...
void oe_kresli_edit_flare(K_EDITOR *p_cnf, LENS_FLARE *p_flare, int num)
{
 ...
 for(f = 0; f < num; f++) {
   if(p_flare[f].akt) {
     ...
     if(f == p_cnf->flareakt) {
       oe_test_kriz_stred_bod(p_cnf,VELIKOST_STREDU,&p,POPIS_OS_ON,STRED_ON,p_flare[f].rflag);
     } 
 ...
 */
 for(f = 0; f < num; f++) {   
   ...
   p_flare[f].vid = (0 < p.z && p.z < w && -w < p.x && p.x <= w && -w < p.y && p.y <= w);
   ...
   if(p.z > 0.0f && (p_flare[f].vid || p_flare[f].p_sloz)) {
     ...
     if(p_flare[f].vid) {
       glReadPixels(ftoi(xw),ftoi(yw),1,1,GL_DEPTH_COMPONENT,GL_FLOAT,&pixel_z);       
 ...
 */
 ...
 for(f = 0; f < num; f++) {
   if(p_flare[f].vid) {
     ...
     text_set(p_flare[f].p_mat->p_text[0]->text,p_flare[f].p_mat->p_text[0]->typ);
   ...
   if(p_flare[f].vid && p_flare[f].p_sloz/* && p_flare[f].wz > 0.0f*/) {
     ...
     for(s = 0; s < SLOZ_FLARE; s++) {
       if(p_sloz[s].vid) {         
         ...
         text_set(p_sloz[s].p_mat->p_text[0]->text,p_sloz[s].p_mat->p_text[0]->typ);
...
{ 
  kresli_kosoctverec(p_cnf->p_project, p_cnf->p_camera, p_world, p_p, r,  dr, dg, db);
}
...
*/
void oe_kresli_staticke_svetlo(K_EDITOR *p_cnf, STATIC_LIGHT *p_light, int akt)
{
  BOD s;
  if(p_light->akt) {
    oe_cti_pozici_static_light(p_light,&s);
    if(p_light->index == akt) {
      oe_test_kriz_stred_bod(p_cnf,VELIKOST_STREDU,&s,POPIS_OS_ON,STRED_ON,p_light->flag);
    } 
...
*/
void oe_kresli_dynamicke_svetlo(K_EDITOR *p_cnf, DYN_LIGHT *p_light, int akt)
{
  BOD s;
  if(p_light->akt) {
    if(p_light->flag&SDL_MESH) {
      transformuj_bod_matici_bod(&p_light->np,kont_world_matrix(p_light->p_mesh_data),&s);
    } else {
      s = p_light->np;
    }
    if(akt) {
      oe_test_kriz_stred_bod(p_cnf,VELIKOST_STREDU,&s,POPIS_OS_ON,STRED_ON,p_light->pflag);
    } 
    oe_kresli_kosoctverec(p_cnf,&p_cnf->init_matrix,&s,0.5f,1,1,0);
  }
}
...
/*
  Kresli extra svetla
*/
void oe_kresli_extra_dynamicke_svetlo(K_EDITOR *p_cnf, EXTRA_DYN_LIGHT *p_light, int akt)
{
  BOD s;
  if(p_light->akt) {
    ...
    oe_kresli_kosoctverec(p_cnf,&p_cnf->init_matrix,&s,0.5f,113.0f/255.0f,250.0f/255.0f,88.0f/255.0f);
  }
}
...
{
  oe_test_kriz_stred_bod(p_cnf,VELIKOST_STREDU,&p_cnf->elm.p,POPIS_OS_ON,STRED_ON,p_cnf->elm.flag);
  oe_kresli_kosoctverec(p_cnf,&p_cnf->init_matrix,&p_cnf->elm.p,0.7f,1,0,0);
}
 ...
 for(i = 0; i < p_cnf->groupnum; i++) {
   ...
   if(transf_3d_2d_matrix(p_cnf->p_kont[k]->p_obj[o]->p_vertex[v].x,
     ...
     if(p_cnf->vybr_group == i)
       kresli_ctverec_2d_plnej(x-VEL_CTVERCE,y-VEL_CTVERCE,x+VEL_CTVERCE,y+VEL_CTVERCE,1,0,0);
     else
       kresli_ctverec_2d(x-VEL_CTVERCE,y-VEL_CTVERCE,x+VEL_CTVERCE,y+VEL_CTVERCE,1,0,0);     
   }
 }
}
...
void oe_kresli_mlho_kostky(K_EDITOR *p_cnf)
{
  ...
  while(p_mlha) {    
    kresli_oktanovou_bunku_minmax(&p_mlha->min, &p_mlha->max,float_rgba(&p_mlha->r));
    ...
    oe_test_kriz_stred_bod(p_cnf,VELIKOST_STREDU,&s,POPIS_OS_ON,STRED_ON,p_cnf->p_mlha->flag);
...
{ 
  ...
  init_matrix(&world);  
  kresli_oktanovou_bunku_minmax(&p_cnf->kam.min, &p_cnf->kam.max, float_rgba(r));
  kd_stred_bunky(&p_cnf->kam.min, &p_cnf->kam.max, &s);
  oe_kresli_kosoctverec(p_cnf, &world, &s, 0.5f, 0.0f, 0.0f, 1.0f);
  oe_test_kriz_stred_bod(p_cnf,VELIKOST_STREDU,&s,POPIS_OS_ON,STRED_ON,flag);
}
  ...
  if(flag&GROUP_KONT && !(flag&(GROUP_OBJ|GROUP_FACE))) {
    for(i = 0; i < p_cnf->groupfacenum; i++) {
      ...
      if((p_kont = p_cnf->p_kont[k])) {
        obb_kresli_obalku_kont(p_kont,kont_world_matrix(p_kont),DDRGB(0,1,0));
      } 
    }          
  } 
    ...
    for(i = 0; i < p_cnf->groupfacenum; i++) {
      ...
      if(p_kont && p_kont->p_obj[o]) {        
        obb_kresli_obalku_obj(p_kont->p_obj[o],kont_world_matrix(p_kont),DDRGB(0,1,0));
      }
    }
  } 
  else if(flag&GROUP_KONT && flag&GROUP_OBJ && flag&GROUP_FACE) {
 ...
 for(i = 0; i < p_cnf->dvertnum; i++) {
   if(p_cnf->p_dvert[i].d2) {
     ...
     if(p_cnf->vybr_dvert == i)
       kresli_ctverec_2d_plnej(x-VEL_CTVERCE,y-VEL_CTVERCE,x+VEL_CTVERCE,y+VEL_CTVERCE,0,1,0);
     else
       kresli_ctverec_2d(x-VEL_CTVERCE,y-VEL_CTVERCE,x+VEL_CTVERCE,y+VEL_CTVERCE,0,1,0);
   } else {
     if(transf_3d_2d_matrix(p_cnf->p_dvert[i].x,p_cnf->p_dvert[i].y,p_cnf->p_dvert[i].z, &x, &y, &z, &m,OXRES, OYRES, OXSTART, OYSTART,p_cnf->kam.near_plane,p_cnf->kam.far_plane)) {
       if(p_cnf->vybr_dvert == i)
         kresli_ctverec_2d_plnej(x-VEL_CTVERCE,y-VEL_CTVERCE,x+VEL_CTVERCE,y+VEL_CTVERCE,0,1,0);
       else
         kresli_ctverec_2d(x-VEL_CTVERCE,y-VEL_CTVERCE,x+VEL_CTVERCE,y+VEL_CTVERCE,0,1,0);
...
void oe_animuj_frame_material(dword akt_time, EDIT_MATERIAL *p_mat, EDIT_TEXT *p_text)
{  
  ...
  if(p_anim->last_time+p_anim->p_frame[next_frame].time < akt_time && !(flag&FRAME_EVENT)) { 
  ...
  if(!event) {
    for(m = 0; m < MAX_EDIT_MATERIALU; m++) {
      if(p_cnf->p_mat[m] && p_cnf->p_mat[m]->flag&MAT_ANIM_FRAME)
        oe_animuj_frame_material(p_cnf->TimeEndLastFrame,p_cnf->p_mat[m], p_cnf->p_text);
    }
  } else {
    for(m = 0; m < MAX_EDIT_MATERIALU; m++) {
      if(p_cnf->p_mat[m] && p_cnf->p_mat[m]->flag&MAT_ANIM_FRAME)
        oe_animuj_frame_material_event(p_cnf->TimeEndLastFrame,p_cnf->p_mat[m], p_cnf->p_text);
    }
  }
}
...
void oe_kresli_objekt_jednoduse(EDIT_OBJEKT *p_obj, int flag, int kflag)
{
  ...
  int norm =  glstav_pn_triangles || flag&(MAT2_CALC_MAP1|MAT2_CALC_MAP2|MAT2_CALC_MAP3|MAT2_CALC_MAP4);
  ...
  for(i = 0; i < p_obj->facenum; i+=3) {
    ...
    if((ind = *p_int) != K_CHYBA)
      glMultiTexCoord2fvARB(GL_TEXTURE0_ARB, ((float *)(&p_obj->p_vertex[f1].tu1))+((ind)<<1));
    p_int++;
    if((ind = *p_int) != K_CHYBA)
      glMultiTexCoord2fvARB(GL_TEXTURE1_ARB, ((float *)(&p_obj->p_vertex[f1].tu1))+((ind)<<1));
    p_int++;
    if((ind = *p_int) != K_CHYBA)
      glMultiTexCoord2fvARB(GL_TEXTURE2_ARB, ((float *)(&p_obj->p_vertex[f1].tu1))+((ind)<<1));
    p_int++;
    if((ind = *p_int) != K_CHYBA)
      glMultiTexCoord2fvARB(GL_TEXTURE3_ARB, ((float *)(&p_obj->p_vertex[f1].tu1))+((ind)<<1));
    
    if(norm) glNormal3fv(&p_obj->p_vertex[f1].nx);
    glColor4fv(&p_obj->p_vertex[f1].dr);
    glSecondaryColor3fvEXT(&p_obj->p_vertex[f1].sr);
    glVertex3fv(p_obj->p_vertex_kosti ? (float *)(p_obj->p_vertex_kosti+f1) : &p_obj->p_vertex[f1].x);

    p_int = glstav_text_map_indicie;
    if((ind = *p_int) != K_CHYBA)
      glMultiTexCoord2fvARB(GL_TEXTURE0_ARB, ((float *)(&p_obj->p_vertex[f2].tu1))+((ind)<<1));
    p_int++;
    if((ind = *p_int) != K_CHYBA)
      glMultiTexCoord2fvARB(GL_TEXTURE1_ARB, ((float *)(&p_obj->p_vertex[f2].tu1))+((ind)<<1));
    p_int++;
    if((ind = *p_int) != K_CHYBA)
      glMultiTexCoord2fvARB(GL_TEXTURE2_ARB, ((float *)(&p_obj->p_vertex[f2].tu1))+((ind)<<1));
    p_int++;
    if((ind = *p_int) != K_CHYBA)
      glMultiTexCoord2fvARB(GL_TEXTURE3_ARB, ((float *)(&p_obj->p_vertex[f2].tu1))+((ind)<<1));
    if(norm) glNormal3fv(&p_obj->p_vertex[f2].nx);
    glColor4fv(&p_obj->p_vertex[f2].dr);
    glSecondaryColor3fvEXT(&p_obj->p_vertex[f2].sr);        
    glVertex3fv(p_obj->p_vertex_kosti ? (float *)(p_obj->p_vertex_kosti+f2) : &p_obj->p_vertex[f2].x);

    p_int = glstav_text_map_indicie;
    if((ind = *p_int) != K_CHYBA)
      glMultiTexCoord2fvARB(GL_TEXTURE0_ARB, ((float *)(&p_obj->p_vertex[f3].tu1))+((ind)<<1));
    p_int++;
    if((ind = *p_int) != K_CHYBA)
      glMultiTexCoord2fvARB(GL_TEXTURE1_ARB, ((float *)(&p_obj->p_vertex[f3].tu1))+((ind)<<1));
    p_int++;
    if((ind = *p_int) != K_CHYBA)
      glMultiTexCoord2fvARB(GL_TEXTURE2_ARB, ((float *)(&p_obj->p_vertex[f3].tu1))+((ind)<<1));
    p_int++;
    if((ind = *p_int) != K_CHYBA)
      glMultiTexCoord2fvARB(GL_TEXTURE3_ARB, ((float *)(&p_obj->p_vertex[f3].tu1))+((ind)<<1));
    if(norm) glNormal3fv(&p_obj->p_vertex[f3].nx);
    glColor4fv(&p_obj->p_vertex[f3].dr);
    glSecondaryColor3fvEXT(&p_obj->p_vertex[f3].sr);
    glVertex3fv(p_obj->p_vertex_kosti ? (float *)(p_obj->p_vertex_kosti+f3) : &p_obj->p_vertex[f3].x);
  }
  glEnd();
}
...
void oe_kresli_objekt_jednoduse_normal(EDIT_OBJEKT *p_obj, float dv)
{
  ...
  for(i = 0; i < p_obj->facenum; i+=3) {
    ...
    glVertex3f(-p_obj->p_vertex[f1].nx*dv+v.x,
               -p_obj->p_vertex[f1].ny*dv+v.y,
               -p_obj->p_vertex[f1].nz*dv+v.z);
    ...
    glVertex3f(-p_obj->p_vertex[f2].nx*dv+v.x,
               -p_obj->p_vertex[f2].ny*dv+v.y,
               -p_obj->p_vertex[f2].nz*dv+v.z);
    ...
    glVertex3f(-p_obj->p_vertex[f3].nx*dv+v.x,
               -p_obj->p_vertex[f3].ny*dv+v.y,
               -p_obj->p_vertex[f3].nz*dv+v.z);
  }
  glEnd();
}
...
void oe_kresli_objekt_kosti_rec_body(K_EDITOR *p_cnf, EDIT_KONTEJNER *p_kont, EDIT_OBJEKT *p_obj, JOINT *p_joint)
{
  GLMATRIX m;
  ...
  calc_3d_2d_matrix(p_kont->kflag&KONT_KEYFRAME ? &p_obj->m : kont_world_matrix(p_kont), p_cnf->p_camera, p_cnf->p_project, &m);

  for(v = 0; v < p_joint->vertexnum; v++) {    
    p_vert = (BOD *)(p_obj->p_vertex+p_ind[v]);
    transformuj_bod_matici_bod(p_vert,&p_joint->m,&p);
    if(transf_3d_2d_matrix(p.x,p.y,p.z, &x, &y, &z, &m, OXRES, OYRES, OXSTART, OYSTART,p_cnf->kam.near_plane,p_cnf->kam.far_plane)) {
      kresli_ctverec_2d_plnej(x-VEL_CTVERCE_KOST,y-VEL_CTVERCE_KOST,x+VEL_CTVERCE_KOST,y+VEL_CTVERCE_KOST,1,0,0);
    }
  }  
...
void oe_kresli_objekt_kosti_rec(K_EDITOR *p_cnf, BOD *p_bod, EDIT_KONTEJNER *p_kont, EDIT_OBJEKT *p_obj, JOINT *p_joint)
{   
  BOD p;
  ...
  kresli_kosoctverec_word(&p,RADIUS_KOSTI,p_cnf->jcf.p_joint_akt == p_joint ? BARVA_KOSTI_AKT : BARVA_KOSTI);
...
void oe_kresli_objekt_kosti(K_EDITOR *p_cnf, EDIT_KONTEJNER *p_kont, EDIT_OBJEKT *p_obj)
{
  EDIT_OBJEKT *p_jobj = oe_cti_joint_objekt(p_cnf);
  
  if(p_jobj == p_obj && p_cnf->jcf.p_joint_animace_akt) {
    oe_kresli_objekt_kosti_rec(p_cnf, NULL, p_kont, p_obj, p_cnf->jcf.p_joint_animace_akt->p_child);
  } else if(p_obj->p_joit_animace) {    
    oe_kresli_objekt_kosti_rec(p_cnf, NULL, p_kont, p_obj, p_obj->p_joit_animace->p_child);
  }
}
...
{  
  GLMATRIX *p_word = p_kont->kflag&KONT_KEYFRAME ? &p_obj->m : kont_world_matrix(p_kont);
  int flag2;
  ...
  */
  ...
  /* Nastav material
  */
  oe_nastav_material_single(p_cnf->p_smat ? *(p_cnf->p_smat) : NULL, p_mat[p_obj->material], p_obj, p_word);
...
EDIT_OBJEKT * oe_cti_joint_objekt(K_EDITOR *p_cnf)
{
  int k = p_cnf->jcf.k;
  int o = p_cnf->jcf.o;
  return((p_cnf->p_kont[k] && p_cnf->p_kont[k]->p_obj[o]) ? p_cnf->p_kont[k]->p_obj[o] : NULL);  
}
...
void oe_kresli_kontejner(K_EDITOR *p_cnf, EDIT_MATERIAL **p_mat, EDIT_KONTEJNER *p_kont, int akt_objekt)
{
 ...
 for(i = 0 ; i < k; i++) {
   ...
   if(p_obj && p_obj->kreslit) {
     if(p_kont->kflag&KONT_KEYFRAME) {
       p_world = (p_cnf->mod_matrix_akt) ? mat_mult_dir(&p_obj->m,&p_cnf->mod_matrix,&conv) : &p_obj->m;
       set_matrix_world(p_world);
     }
     if(akt_objekt == i && p_cnf->cnf.objekt_stred) {
       oe_test_kriz_stred(p_cnf,VELIKOST_STREDU,p_obj->pivot.x,
       p_obj->pivot.y,p_obj->pivot.z,
       POPIS_OS_ON,STRED_OFF,POSUN_VSE);
     }

     oe_kresli_objekt(p_cnf, p_kont, p_obj, p_mat, i == akt_objekt, akt_objekt != K_CHYBA);
...
void oe_kresli_objekt_dratove(K_EDITOR *p_cnf, EDIT_MATERIAL **p_mat, EDIT_KONTEJNER *p_kont, EDIT_OBJEKT *p_obj, int akt_objekt, int akt_kont)
{
 ...
 if(!p_cnf->cnf.textury && p_cnf->cnf.obalky_objekt && akt_kont && akt_objekt) {
...
void oe_kresli_kontejner_dratove(K_EDITOR *p_cnf, EDIT_MATERIAL **p_mat, EDIT_KONTEJNER *p_kont, int akt_objekt)
{
 ...
 if(!p_cnf->cnf.textury && p_cnf->cnf.obalky_kontejner && akt_objekt != K_CHYBA) {
 ...
 for(i = 0; i < k; i++) {
   ...
   if(p_obj && p_obj->kreslit) {
     ...
     if(p_cnf->cnf.objekt_stred)
       oe_test_kriz_stred(p_cnf,VELIKOST_STREDU,p_obj->pivot.x,p_obj->pivot.y,p_obj->pivot.z,POPIS_OS_ON,STRED_OFF,POSUN_VSE);
     
     oe_kresli_objekt_dratove(p_cnf,p_mat,p_kont,p_obj,i == akt_objekt, akt_objekt != K_CHYBA);
     oe_kresli_objekt_kosti(p_cnf, p_kont, p_obj);
   }
 } 
...
void oe_renderuj_scenu_system_3d(K_EDITOR *p_cnf)
{
  ...
  /* Kresli znacku modu
  */
  transf_2d_3d_z(p.x,p.y,p.z,&d.x,&d.y,&d.z,p_cnf->p_camera,p_cnf->p_project,
               OXRES, OYRES, OXSTART, OYSTART,
               p_cnf->kam.near_plane,p_cnf->kam.far_plane);
  glDisable(GL_DEPTH_TEST);
  oe_test_kriz_stred(p_cnf,2.0f,d.x,d.y,d.z,POPIS_OS_ON,STRED_ON,POSUN_VSE);  
  glEnable(GL_DEPTH_TEST);
  ...
  */
  if(p_cnf->cnf.svetla_kreslit) {
    for(i = 0; i < MAX_FLARE_SVETEL; i++)
      oe_kresli_dynamicke_svetlo( p_cnf, p_cnf->p_dlight+i, i == p_cnf->dl_akt);
  }
  ...
  */
  if(p_cnf->cnf.svetla_kreslit) {
    for(i = 0; i < MAX_FLARE_SVETEL; i++)
      oe_kresli_extra_dynamicke_svetlo(p_cnf, p_cnf->p_edlight+i, i == p_cnf->edl_akt);
  }
...
void oe_renderuj_scenu_editace(K_EDITOR *p_cnf,EDIT_KONTEJNER *p_kont[KONT_NUM], int max_kont)
{
  GLMATRIX *p_world;
  dword x,y;
  int   akt,i;
...
void oe_kresli_kamery(K_EDITOR *p_cnf)
{
  ...
  for(i = 0; i < MAX_EDIT_KAMER; i++) {
    ...
    if(p_kam->cislo != K_CHYBA) {      
      ...
      oe_kresli_kameru(p_kam,!p_cnf->cnf.kamera_aktivni && p_cnf->kamakt == i);
...
*/
void oe_renderuj_scenu_kdtree_rec(K_EDITOR *p_cnf, KD_BUNKA *p_prvni, GLMATRIX *p_m, int kreslit)
{    
  ...
  if(p_cnf->cnf.kdtree_bunka == p_prvni->cislo || p_cnf->cnf.kdtree_kreslit_vse || kreslit) {
 ...
 */
 if(p_cnf->polynum) {
   ...
   polygonu = 0;
   facu = 0;
   calc_3d_2d_matrix(&p_cnf->init_matrix, p_cnf->p_camera, p_cnf->p_project, &m); 
   set_matrix_world(&p_cnf->init_matrix);
 ...
 */
 if(p_cnf->polynum) {
   ...
   for(i = 0; i < p_cnf->polynum; i++) {
     if(p_cnf->p_poly[i].kreslit && !(p_cnf->p_poly[i].kflag&KONT_ZRCADLO)) {
       if(p_cnf->p_poly[i].p_lightnum)
         oe_kresli_poly_3d_game(p_cnf->p_poly+i, p_cnf->p_mat, p_cnf->cnf.light_maps);
       else
...
{    
 return(oe_test_kriz_stred(p_cnf,vzdal,p_bod->x,p_bod->y,p_bod->z,popis,stred,akt_osa));
}
...
{  
  ...
  oe_nastav_material_single_poly(p_cnf->p_smat ? *(p_cnf->p_smat) : NULL, p_mat[p_poly->material],p_poly);
  text_off(GL_TEXTURE_1D);
  text_off(GL_TEXTURE_2D);
...
void oe_kresli_poly_3d_game(EDIT_MESH_POLY *p_poly, EDIT_MATERIAL **p_mat, int light)
{
  ...
  if(!light) {
    textur = oe_nastav_material_single_poly(p_cnf->p_smat ? *(p_cnf->p_smat) : NULL, p_mat[p_poly->material],p_poly);
    text_on(GL_TEXTURE_2D);
  } else {
    text_set_num(1);
    text_off(GL_TEXTURE_1D);
    ...
    glBegin(GL_TRIANGLES);
      for(i = 0; i < p_poly->p_lightnum[l]; i++) {
        if(!light) {
          ...
          glMultiTexCoord2fvARB(GL_TEXTURE0_ARB,&p_poly->p_koord[last+i].tul);
        }
...
void oe_kresli_listu_prvku(K_EDITOR *p_cnf)
{
  ...
  nr = 1.0f;
  projection_matrix(&prj, DEG2RAD(30.0f), (float)dx/(float)dy, nr, 100.0f);
  ...
  for(i = 0; i < oken; i++) {
    ...
    if(k < p_lista->prvnum) {
      ...
      if(k != K_CHYBA && p_bnf->p_prv_dat[k]) {
        oe_kresli_kontejner(p_cnf,p_cnf->bnf.p_mat,p_bnf->p_prv_dat[k]->p_kont,K_CHYBA);
      }
      x += LISTA_X;
    }
  }
