...
int (*ber_nastav_material_single) (EDIT_MATERIAL * p_smat,
  EDIT_MATERIAL * p_mat, dword kflag);
int (*ber_nastav_material_single_poly) (EDIT_MATERIAL * p_smat,
  EDIT_MATERIAL * p_mat, dword kflag);
...
void ber_nahod_render_funkce(void)
{
  ...
  ber_nastav_material_single_poly =
    ber_nastav_material_single_poly_multi_spec;
...
void ber_viditelnost_flare(G_KONFIG * p_ber, LENS_FLARE * p_flare)
{
  ...
  while (p_flare) {
    ...
    p_flare->vid = (0 < p.z && p.z < w && -w < p.x && p.x <= w && -w < p.y
      && p.y <= w);
    ...
    if (p.z > 0.0f && (p_flare->vid || p_flare->p_sloz)) {
      ...
      if (!(p_flare->zflag & FLR_NO_ZTEST) && p_flare->vid) {
        glReadPixels(ftoi(xw), ny + ftoi(yw), 1, 1, GL_DEPTH_COMPONENT,
          GL_FLOAT, &pixel_z);
  ...
  EDIT_MATERIAL * p_mat, dword kflag)
{
   ...
   */
  if (p_mat->flag2 & (MAT2_SPECULAR | MAT2_ENV_SPEC)
...
void ber_kresli_mesh_array(GAME_MESH_OLD * p_mesh, EDIT_MATERIAL ** p_mt)
{
  ...
  for (i = 0; i < o; i++, p_vflag++) {  // kresli to po objektech
    if ((*p_vflag) & KONT_VIDITELNY) {
      if (ber_nastav_material_single(p_ber->p_smat, p_mt[p_mesh->p_mat[i]],
          kflag) || !i)
...
void ber_kresli_mesh_vertex(GAME_MESH_OLD * p_mesh, EDIT_MATERIAL ** p_mt)
{
  ...
  p_vertex_pos = p_mesh->p_vertex_pos;
  p_vertex_normal = glstav_pn_triangles
    || p_mesh->p_data->m2flag & MAT2_CALC_MAP1 ? p_mesh->p_vertex_norm : NULL;
...
void ber_kresli_poly_array(EDIT_MESH_POLY * p_poly, EDIT_MATERIAL ** p_mat)
{
  ...
  ber_nastav_material_single_poly(p_ber->p_smat, p_mat[p_poly->material],
    p_poly->kflag);
...
inline void ber_kresli_poly(EDIT_MESH_POLY * p_poly, EDIT_MATERIAL ** p_mat)
{
  ...
  if (p_poly->m2flag & MAT2_ENV_SPEC && kamera_zmena)
    poly_env_maping_spec(p_poly, p_mat[p_poly->material],
      get_matrix_camera_point());
...
void ber_kresli_fleky(G_KONFIG * p_ber, FLEK * p_topflek)
{
  ...
  while (p_flek) {
    if (!(p_flek->flag & FLEK_NEKRESLIT)) {
      ber_nastav_material_single(p_ber->p_smat,
        p_ber->p_mat[p_flek->material], FALSE);
      ...
      glVertex3f(p_pos->x - p_nx->x - p_nz->x,
        p_pos->y - p_nx->y - p_nz->y, p_pos->z - p_nx->z - p_nz->z);
...
*/
static int ber_renderuj_scenu_pruhledne_compare(const void *p_r1,
  const void *p_r2)
{
  return (ftoi((((PRUHLEDNY_OBJEKT *) p_r2)->vzdal -
        ((PRUHLEDNY_OBJEKT *) p_r1)->vzdal) * 1000.0f));
}
...
void ber_renderuj_scenu_pruhledne(G_KONFIG * p_ber)
{
  ...
  if (p_ber->prhlnum) {
    qsort(p_ber->prhl, p_ber->prhlnum, sizeof(p_ber->prhl[0]),
    ...
    for (i = 0; i < p_ber->prhlnum; i++) {      // render pruhlednych objektu odzadu
      switch (p_ber->prhl[i].typ) {
        case PRUHLEDNY_MESH:
          ...
          ber_kresli_mesh((GAME_MESH_OLD *) p_ber->prhl[i].p_objekt,
            p_ber->p_mat);
          break;
        case PRUHLEDNY_POLY:
          ...
          ber_kresli_poly((EDIT_MESH_POLY *) p_ber->prhl[i].p_objekt,
            p_ber->p_mat);
          break;
        default:
          break;
...
void ber_renderuj_scenu_obyc(int zrc)
{
  ...
  while ((p_mesh =
      ber_mesh_render_list_next_flag(p_ber, flag, KONT_VIDITELNY_PRUHL))) {
  ...
  while ((p_poly =
      ber_poly_render_list_next_flag(p_ber, flag, KONT_VIDITELNY_PRUHL))) {
...
#ifdef DEBUG_MOD_OBALKY
    kresli_oktanovou_bunku_minmax(&p_hnizdo->min, &p_hnizdo->max, DDRGBA(0, 0,
        0, 1));
#endif
...
void pe_renderuj(G_KONFIG * p_ber)
{
  ...
  while (p_par) {
    ...
    if (p_par->flag & TPAR_STREP) {
      if (p_par->pnum
        && kd_visibility(&p_par->min, &p_par->max, p_camera_project))
...
void ber_renderuj_mlhu(G_KONFIG * p_ber)
{
  ...
  if (kamera_zmena) {
    while (p_tmp) {
      if (!p_tmp->p_kont
        && kd_je_bod_v_kostce(p_cbod, &p_tmp->min, &p_tmp->max) && (!p_top
          || p_tmp->priorita >= p_top->priorita)) {
        p_top = p_tmp;
      }
      p_tmp = p_tmp->p_next;
    }
...
static int ber_mesh_render_list_setrid_compare(const void *p_m1,
  const void *p_m2)
{
...
static int ber_poly_render_list_setrid_compare(const void *p_m1,
  const void *p_m2)
{
...
{
  qsort(p_ber->p_mesh_renderlist, p_ber->mesh_rendernum,
    sizeof(p_ber->p_mesh_renderlist[0]), ber_mesh_render_list_setrid_compare);
  qsort(p_ber->p_poly_renderlist, p_ber->poly_rendernum,
    sizeof(p_ber->p_poly_renderlist[0]), ber_poly_render_list_setrid_compare);
}

inline int obb_visibility_flag(OBB_OLD * p_obb, GLMATRIX * p_mat, int viz,
  int flag)
{
...
inline void ber_render_list_vyrob_poly(G_KONFIG * p_ber,
  EDIT_MESH_POLY * p_poly, int add, GLMATRIX * p_mat, int viditelny_flag,
  int zmena_flag, int mail)
{
  int kflag;
...
void ber_render_list_vyrob_rec(G_KONFIG * p_ber, OBB_TREE_OLD * p_prvni,
  GLMATRIX * p_m, GLMATRIX * p_zrc, int kamera_zmena, int mail)
{
  EDIT_MESH_POLY *p_poly;
  ...
  for (i = 0; i < p_prvni->itnum; i++, p_item++) {
    ...
    if (p_item->viditelny) {
      ...
      if (p_poly) {
        ber_render_list_vyrob_poly(p_ber, p_poly, TRUE, p_m, KONT_VIDITELNY,
          KONT_VIDITELNY_ZMENA, mail);
      }
      else {
        ber_render_list_vyrob_mesh(p_ber, TRUE, p_ber->p_mesh + p_item->mesh,
          TRUE, p_m, KONT_VIDITELNY, KONT_VIDITELNY_ZMENA, mail,
          kamera_zmena);
      }
    }
    ...
    if (p_zrc) {
      ...
      if (p_item->zrc_viditelny) {
        p_poly = p_item->p_poly;
        if (p_poly) {
          ber_render_list_vyrob_poly(p_ber, p_poly, !p_item->viditelny, p_zrc,
            KONT_VIDITELNY_ZRC, KONT_VIDITELNY_ZMENA, mail);
        }
        else {
          ber_render_list_vyrob_mesh(p_ber, TRUE,
            p_ber->p_mesh + p_item->mesh, !p_item->viditelny, p_zrc,
            KONT_VIDITELNY_ZRC, KONT_VIDITELNY_ZMENA, mail, kamera_zmena);
        }
      }
    }
  }
  ...
  for (i = 0; i < p_prvni->nextnum; i++, p_next++) {
    if (obb_visibility(&p_next->obb, p_m) || (p_zrc
        && obb_visibility(&p_next->obb, p_zrc)))
      ber_render_list_vyrob_rec(p_ber, p_next, p_m, p_zrc, kamera_zmena,
        mail);
  }
}
...
void ber_render_list_vyrob(G_KONFIG * p_ber, int zrcadlo, int kamera_zmena)
{
   ...
   */
  p_zrc =
    zrcadlo ? mat_mult_dir(__mat_top_matrix + __mat_top_matrix_akt,
    p_camera_project, &camera_project_zrc) : NULL;

  if (!p_ber->p_poly_renderlist) {
    p_ber->p_poly_renderlist =
      (EDIT_MESH_POLY **) mmalloc(sizeof(p_ber->p_poly_renderlist[0]) *
      p_ber->polynum * 2);
   ...
   */
  if (obb_visibility(&p_ber->obbtree.obb, p_camera_project) || (p_zrc
      && obb_visibility(&p_ber->obbtree.obb, p_zrc))) {
    ber_render_list_vyrob_rec(p_ber, &p_ber->obbtree, p_camera_project, p_zrc,
      kamera_zmena, mail);
  }
  ...
  for (i = 0; i < p_ber->dyn_meshlistnum; i++, p_src++) {
    if (**p_src && !((**p_src)->nekreslit)) {
      ...
      viditelny =
        ber_render_list_vyrob_mesh(p_ber, FALSE, *p_src, TRUE,
...
void ber_render_list_vyrob_pruhlist(G_KONFIG * p_ber)
{
  ...
  while ((p_mesh = ber_mesh_render_list_next(p_ber))) {
    if (p_mesh->p_data->m1flag & MAT_PRUHLEDNY
      || p_mesh->p_data->kflag & (KONT_DRAW_PRUHL | KONT_DRAW_CAMERA)) {
  ...
  while ((p_poly = ber_poly_render_list_next(p_ber))) {
    if (p_poly->m1flag & MAT_PRUHLEDNY
      || p_poly->kflag & (KONT_DRAW_PRUHL | KONT_DRAW_CAMERA)) {
      PRUHLEDNY_OBJEKT *p_pruhl = p_ber->prhl + p_ber->prhlnum++;
