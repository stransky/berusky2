...
__inline GLMATRIX * set_matrix_world_top(GLMATRIX *p_top_world)
{
  if(__mat_top_matrix_akt+1 < MAX_WORD_MATRIX_STACK) {    
    ...
    mat_mult_dir(&__mat_world,__mat_top_matrix+__mat_top_matrix_akt,&__mat_tmp);
...
__inline void ret_matrix_world_top(void)
{
  if(__mat_top_matrix_akt > 0) {
    __mat_top_matrix_akt--;
    if(__mat_top_matrix_akt) {
      mat_mult_dir(&__mat_world,__mat_top_matrix+__mat_top_matrix_akt-1,&__mat_tmp);
      mat_mult_dir(&__mat_tmp,&__mat_kamera,&__mat_vysl);
    } else {
...
{   
 ...
 if(__mat_top_matrix_akt) {
   mat_mult_dir(p_world,__mat_top_matrix+__mat_top_matrix_akt-1,&__mat_tmp);
   mat_mult_dir(&__mat_tmp,&__mat_kamera,&__mat_vysl);
 } else {
...
__inline void text_set(GLuint text, GLenum typ)
{
  if(typ == GL_TEXTURE_2D) {
    ...
    if(glstav_textury_1d[glstav_text_akt] && glstav_last_text_1d[glstav_text_akt] != text) {
