...
long CALLBACK WindowProc_master(HWND hwnd, UINT message, WPARAM wParam,
  LPARAM lParam)
{
  switch (message) {
/*
...
HWND otevri_okno_master(HINSTANCE handle_aplikace)
{
  ...
  hwnd = CreateWindow("3D Editor",
    ...
    NULL, handle_menu[MENU_MESH_EDITOR], handle_aplikace, NULL);
...
/*
  Obsluha plochy
*/
long CALLBACK WindowProc_plocha(HWND hwnd, UINT message, WPARAM wParam,
  LPARAM lParam)
{
  switch (message) {
    case WM_MOUSEMOVE:{
    ...
    case WM_RBUTTONUP:{
        ...
        if (wParam & MK_SHIFT)
          ...
          nacti_tlacitka_mysi(wParam, lParam);
          if (akt_editor == 1)
            TrackPopupMenuEx(GetSubMenu(handle_menu[MENU_MESH_POPUP], 0),
              TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON, mi.x, mi.y,
              hwnd, NULL);
          else
            TrackPopupMenuEx(GetSubMenu(handle_menu[MENU_GAME_POPUP], 0),
              TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON, mi.x, mi.y,
              hwnd, NULL);
          system_kurzor = 0;
        }
...
HWND otevri_okno_plocha(HINSTANCE handle_aplikace, HWND hwnd_otec)
{
  ...
  hwnd = CreateWindowEx(FALSE,
    "Plocha",
    "Plocha",
    WS_CHILD | WS_VISIBLE,
    x, y, dx, dy, hwnd_otec, NULL, handle_aplikace, NULL);
...
/*
  Bocni lista
*/
long CALLBACK WindowProc_bocni_lista(HWND hwnd, UINT message, WPARAM wParam,
  LPARAM lParam)
{
  switch (message) {
    case WM_CLOSE:
...
long CALLBACK WindowProc_bocni_lista_dialog(HWND hwnd, UINT message,
...
HWND otevri_bocni_listu(HINSTANCE handle_aplikace, HWND hwnd_otec)
{
  ...
  hwnd = CreateWindowEx(FALSE,
    "Bocni lista",
    "Bocni lista",
    WS_CHILD, x, y, dx, dy, hwnd_otec, NULL, handle_aplikace, NULL);
...
HWND otevri_konzoli(HINSTANCE handle_aplikace, HWND hwnd_otec)
{
  ...
  hwnd_konzole_dialog =
    CreateDialog(hinst, MAKEINTRESOURCE(IDD_KONZOLE), hwnd,
    doe_callback_konzole);
  return (hwnd);
}
...
/*
  Dolni lista
*/
long CALLBACK WindowProc_dolni_lista(HWND hwnd, UINT message, WPARAM wParam,
  LPARAM lParam)
{
  switch (message) {
    case WM_CLOSE:
...
HWND otevri_dolni_listu(HINSTANCE handle_aplikace, HWND hwnd_otec)
{
  ...
  hwnd = CreateWindowEx(FALSE,
    "Dolni lista", "Dolni lista", WS_CHILD | WS_VISIBLE /*|WS_DLGFRAME */ ,
    x, y, dx, dy, hwnd_otec, NULL, handle_aplikace, NULL);
  ...
  //hwnd to je to kam budu sipat ten dialog
  hwnd_dolni_lista_dialog =
    CreateDialog(hinst, MAKEINTRESOURCE(IDD_HORNI_LISTA), hwnd,
    DefWindowProc);
  return (hwnd);
}
  ...
  int nCmdShow)
{
  ...
  if (!ren_typ) {
    ...
    handle_menu[MENU_GAME_EDITOR] =
      LoadMenu(hi, MAKEINTRESOURCE(IDR_MENU_GAME_EDIT));
    handle_menu[MENU_MESH_EDITOR] =
      LoadMenu(hi, MAKEINTRESOURCE(IDR_MENU_MESH_EDIT));
    handle_menu[MENU_GAME_POPUP] =
      LoadMenu(hi, MAKEINTRESOURCE(IDR_POPUP_GAME));
    handle_menu[MENU_MESH_POPUP] =
      LoadMenu(hi, MAKEINTRESOURCE(IDR_POPUP_MESH));
  ...
  GetPrivateProfileString("editor", "log_file", "c:\\berusky2_editor.txt",
    pom, 500, ini_file);
  if (!(cnf.bnf.debug_file = fopen(pom, "a")))
    ddw("Nepodarilo se otevrit log. soubor !");
  kprintf(TRUE,
    "-----------------------Start editor - datum %s cas %s-----------------",
...
{
  system_kurzor = 1;
  if (MessageBox(hwnd_aplikace,
      "Opravdu, ale opravdu si pøeješ skonèit ? Vážnì jsi peèlive zvážil vsechny negativní dùsledky unáhleného rozhodnutí ?",
      "Èubèí synu !", MB_ICONASTERISK | MB_YESNO | MB_SYSTEMMODAL) == IDYES) {
    exit(0);
  }
  system_kurzor = 0;
}
