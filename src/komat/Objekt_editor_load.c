 ...
 ****************************************
*/
EDIT_KONTEJNER * oe_nahraj_kontejner_bez_dialog(K_EDITOR *p_cnf, byte *p_jmeno, int mat)
{
 EDIT_KONTEJNER *p_kont = NULL;
 
 chdir(p_cnf->dir.out_dir);
 kprintf(TRUE,"Load kontejner '%s'...",p_jmeno);
 p_kont = lo_nahraj_kontejner(p_cnf->p_mat, MAX_EDIT_MATERIALU,p_cnf->p_text,
                              MAX_EDIT_TEXTUR, p_jmeno, mat);
 lo_preved_flare_do_sceny(p_kont->p_slight, p_cnf->flare, MAX_EDIT_SVETEL);
 lo_preved_svetla_do_sceny(p_kont,p_cnf->light,MAX_EDIT_SVETEL,p_cnf->p_dlight,MAX_FLARE_SVETEL,p_cnf->p_edlight,MAX_FLARE_SVETEL);
...
EDIT_KONTEJNER * oe_nahraj_kontejner_bez_dialog_3ds(K_EDITOR *p_cnf, byte *p_jmeno, int mat)
{
 EDIT_KONTEJNER *p_kont = NULL;
   
 chdir(p_cnf->dir.out_dir);
 doe_3ds_importer(&p_cnf->cnf.keyframe_load,&p_cnf->cnf.keyframe_static,&p_cnf->cnf.keyframe_kont,&p_cnf->cnf.kamera_load);
 if((p_kont = ds_import_kontejner(p_jmeno,mat,p_cnf->cnf.keyframe_load,TRUE,TRUE,p_cnf->cnf.kamera_load)) == NULL) {
...
EDIT_KONTEJNER * oe_nahraj_kontejner_dialog(K_EDITOR *p_cnf,byte *p_jmeno, byte max_jmeno, int mat)
{
 EDIT_KONTEJNER *p_kont = NULL;
 byte pom[100];
 int  i = 0;
 ...
 if(doe_dialog_load(hwnd_aplikace,pom, 100, "Nahrani kontejneru...",MASKA_MESH,"Mesh soubory")) {
...
EDIT_KONTEJNER * oe_nahraj_kontejner_dialog_3ds(K_EDITOR *p_cnf,byte *p_jmeno, byte max_jmeno, int mat)
{
 EDIT_KONTEJNER *p_kont = NULL;
 byte pom[100];
 int  i = 0;
 ...
 chdir(p_cnf->dir.out_dir);
 if(doe_dialog_load(hwnd_aplikace, pom, 99, "Import z 3D studia", "3D Studio project file (*.prj)\1*.prj","3D Mesh (*.3ds)\1*.3ds\1 3D Studio Material library (*.mli)\1*.mli")) {
   doe_3ds_importer(&p_cnf->cnf.keyframe_load,&p_cnf->cnf.keyframe_static,&p_cnf->cnf.keyframe_kont,&p_cnf->cnf.kamera_load);
   if((p_kont = ds_import_kontejner(pom,mat,p_cnf->cnf.keyframe_load,TRUE,p_cnf->cnf.keyframe_static,p_cnf->cnf.kamera_load)) == NULL) {
...
EDIT_KONTEJNER * oe_nahraj_kontejner_dialog_ase(K_EDITOR *p_cnf, byte *p_jmeno, byte max_jmeno)
{
 EDIT_KONTEJNER *p_kont = NULL;
 byte pom[100];
 int  i = 0;
 ...
 chdir(p_cnf->dir.out_dir);
 if(doe_dialog_load(hwnd_aplikace, pom, 99, "Import z ASCII", "*.ase","MAX ASCII soubory(*.ASE)")) {   
...
void oe_prikaz_nahraj_kontejner(K_EDITOR *p_cnf, int soubor, int mat)
{
 ...
 if(soubor == LOAD_KONT_BTX) {
   if((p_cnf->p_kont[k] = oe_nahraj_kontejner_dialog(p_cnf,string,29,mat)) == NULL) {     
     p_cnf->vybr_kont = k1;
   } else {
     strcpy(p_cnf->p_kont[k]->jmeno,string);
   }
 } else if(soubor == LOAD_KONT_3DS) {
   if((p_cnf->p_kont[k] = oe_nahraj_kontejner_dialog_3ds(p_cnf,string,29,mat)) == NULL) {
     p_cnf->vybr_kont = k1;
   }
   else {
      strcpy(p_cnf->p_kont[k]->jmeno,string);
   ...
   if((p_cnf->p_kont[k] = oe_nahraj_kontejner_dialog_ase(p_cnf,string,29)) == NULL) {
     p_cnf->vybr_kont = k1;
   } else {
     strcpy(p_cnf->p_kont[k]->jmeno,string);
   }
...
void oe_prikaz_nahraj_3ds_animaci_dialog(K_EDITOR *p_cnf)
{
 ...
 if((p_anim_kont = oe_nahraj_kontejner_dialog_3ds(p_cnf,string,29,FALSE))) {
   ...
   key_sim_root_to_sim_root_indir(p_kont, p_kont->sim+va,p_anim_kont->sim);
...
void oe_prikaz_nahraj_sim_animaci_dialog(K_EDITOR *p_cnf)
{
 ...
 if(doe_dialog_load(hwnd_aplikace, pom, 200, "Load animace...", "*.sim","Soubor s sim animaci")) {
   if(p_kont->sim[va].keynum)
...
void oe_prikaz_uloz_kontejner(K_EDITOR *p_cnf, int obalky)
{
 ...
 if(p_cnf->bnf.p_lev) {
   if(!ddwqueto(hwnd_aplikace,"Je aktivni level. Ulozit vybrany prvek levelu jako mesh?"))
     return;
 }
 ...
 if(p_cnf->p_kont[k]) {   
   ...
   lo_uloz_kontejner(p_cnf->p_mat,MAX_EDIT_MATERIALU,p_cnf->p_kont[k],p_cnf->p_kont[k]->jmeno,FALSE,NULL);
...
void oe_prikaz_uloz_kontejner_jako(K_EDITOR *p_cnf)
{
 ...
 if(!p_cnf->p_kont[k])
   ...
   if(doe_dialog_save(pom, 99, "Ulozit jako:",MASKA_MESH,"Mesh soubory")) {
     ...
     lo_uloz_kontejner(p_cnf->p_mat,MAX_EDIT_MATERIALU,p_cnf->p_kont[k],pom,FALSE,NULL);
   }     
 }
}
...
void oe_prikaz_uloz_vybrane_kontejnery_jako(K_EDITOR *p_cnf)
{
 ...
 if(doe_dialog_save(pom, 99, "Ulozit jako:",MASKA_SCENY,"Vybrane kontejnery")) {
   if(p_cnf->groupfacenum && p_cnf->groupflag&GROUP_KONT && !(p_cnf->groupflag&(GROUP_OBJ|GROUP_FACE))) {
     for(i = 0; i < p_cnf->groupfacenum; i++) {
       ...
       if(p_cnf->p_kont[k]) {
         ...
         vycisti_kontejner(p_cnf->p_kont[k]);
         updatuj_kontejner_statistika(p_cnf->p_kont[k],TRUE);
         f = lo_uloz_kontejner(p_cnf->p_mat,MAX_EDIT_MATERIALU,p_cnf->p_kont[k],pom,TRUE,f);
       }
     }   
     ffclose(f);
   }
...
int oe_uloz_projekt(K_EDITOR *p_cnf, byte *p_file, byte *p_dir, int level, int transf_stat)
{ 
...
int oe_nahraj_projekt_out(K_EDITOR *p_cnf, byte *p_file, byte volny_load)
{
  ...
  while(fread(&slot,sizeof(slot),1,f)) {
    if(strstr(slot.jmeno,KONCOVKA_MESH) || strstr(slot.jmeno,KONCOVKA_MESH_OLD)) {
      chdir(p_cnf->dir.scene_dir);
...
void oe_recalc_zrcadla(EDIT_KONTEJNER **p_kont, int kontnum, ZDRCADLO_DESC *p_zrcadlo)
{
...
int oe_nahraj_projekt(K_EDITOR *p_cnf, byte *p_file, byte *p_dir, byte volny_load, int load_textury)
{
  LEVEL_KONFIG lc;
  byte zaloha[200];
  byte file[50];
  ...
  if(efile(file)) {
    ...
    kprintf(TRUE,"Soubor %s v adresari %s neexistuje!",file,p_cnf->dir.scene_dir);
  }
...
void oe_prikaz_nahraj_projekt(K_EDITOR *p_cnf)
{
  byte file[50];
  chdir(p_cnf->dir.scene_dir);
  if(doe_dialog_load(hwnd_aplikace,file,49,"Nahrat scenu",MASKA_SCENY,"Scena")) {
...
void oe_prikaz_uloz_materialy(K_EDITOR *p_cnf)
{
 ...
 if(!doe_dialog_save(file, 99, "Ulozeni materialu", MASKA_MATERIAL, "Materialove soubory")) {
   return;
 } else {
   lo_uloz_materialy(p_cnf->p_mat,MAX_EDIT_MATERIALU,file,p_cnf->dir.material_dir);
 }
}
...
void oe_prikaz_nahraj_materialy(K_EDITOR *p_cnf, int maz)
{
 ...
 chdir(p_cnf->dir.material_dir); 
 if(doe_dialog_load(hwnd_aplikace, file, 99, "Nahrani materialu", MASKA_MATERIAL, "Materialove soubory")) {
   if(maz)
     lo_vymaz_materialy(p_cnf->p_mat,MAX_EDIT_MATERIALU,p_cnf->p_text,MAX_EDIT_TEXTUR);   
   if(lo_nahraj_materialy(p_cnf->p_mat,MAX_EDIT_MATERIALU,p_cnf->p_text,MAX_EDIT_TEXTUR,file)) {
     oe_reload_textur(p_cnf);
   }
 }
}
...
void oe_uloz_berusci_level(K_EDITOR *p_cnf, byte *p_level)
{
  ...
  */  
  lo_uloz_poly_list(zamen_koncovku(levelname,".ply"), p_cnf->p_poly, p_cnf->polynum, p_cnf->p_lightmap, p_cnf->p_mat, MAX_EDIT_MATERIALU);
  ...
  */
  be_save_level(&p_cnf->bnf,zamen_koncovku(levelname,KONCOVKA_LEVELU),dirname);
...
void oe_oznac_pouzite_textury(K_EDITOR *p_cnf)
{
  ...
  for(m = 0; m < MAX_CELKEM_MATERIALU; m++) {
    if(p_cnf->p_mat[m] && 
       p_cnf->p_mat[m]->flag&MAT_POUZITY && 
       p_cnf->p_mat[m]->p_text[0])      
      p_cnf->p_mat[m]->p_text[0]->flag2 |= TEXT2_POUZITA;
  }
}

void oe_oznac_pouzite_materialy_kont(K_EDITOR *p_cnf, EDIT_KONTEJNER *p_kont)
{
  int o,m;
...
int oe_oznac_dlouhe_materialy_kont(K_EDITOR *p_cnf, EDIT_KONTEJNER *p_kont)
{
  ...
  for(m = 0; m < MAX_CELKEM_MATERIALU; m++) {
    if(p_cnf->p_mat[m] && p_cnf->p_mat[m]->flag&MAT_POUZITY) {
      ...
      if(l > 8+3+1) {
        kprintf(TRUE,"DL Mat '%s' Text '%s'",p_cnf->p_mat[m]->jmeno,p_cnf->p_mat[m]->textfile[0]);
        dl++;
      }            
    }
  }  
...
void oe_prikaz_uloz_berusci_level(K_EDITOR *p_cnf)
{
  ...
  chdir(p_cnf->dir.game_level_dir);
  if(doe_dialog_save(pom,100,"Save Berusky2 game level","*.lv6","Game level")) {
...
//    ilInit();
    ...
    if(p_cnf->tdir.scene_texture_dir[0]) {
      lo_reload_textur_dir(p_cnf->p_text,MAX_EDIT_TEXTUR, p_cnf->tdir.scene_texture_dir,FALSE);
    }
...
void oe_prikaz_vypis_pouzite_materialy(K_EDITOR *p_cnf)
{
  ...
  for(i = 0; i < MAX_EDIT_MATERIALU; i++) {
    if(p_cnf->p_mat[i] && p_cnf->p_mat[i]->flag&MAT_POUZITY) {
      kprintf(TRUE,"%s : Text1 : %s",p_cnf->p_mat[i]->jmeno,p_cnf->p_mat[i]->textfile[0]);
      if(p_cnf->p_mat[i]->textfile[1][0])
        kprintf(TRUE,"%s : Text2 : %s",p_cnf->p_mat[i]->jmeno,p_cnf->p_mat[i]->textfile[1]);
    }
  }
}
...
void oe_prikaz_vypis_pouzite_materialy_kontejner(K_EDITOR *p_cnf)
{
 ...
 while(p_kont = oe_vybr_kont_get(&handle)) {
   ...
   for(i = 0; i < MAX_EDIT_MATERIALU; i++) {
     if(p_cnf->p_mat[i] && p_cnf->p_mat[i]->flag&MAT_POUZITY) {
       kprintf(TRUE,"%c %s : Text1 : %s",(p_cnf->p_mat[i]->flag&MAT_PRUHLEDNY)?'P':'N',p_cnf->p_mat[i]->jmeno,p_cnf->p_mat[i]->textfile[0]);
     }
   }
 }
