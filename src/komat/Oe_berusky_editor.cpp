...
BOOL CALLBACK be_calback_new_level( HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam )
{
  switch(msg)  {
    case WM_COMMAND:
...
void be_prikaz_novy_level(B_KONFIG *p_bnf)
{
  ...
  if(DialogBox(hinst, MAKEINTRESOURCE(IDD_BERUSKY_EDITOR_NEW_LEVEL), hwnd_aplikace,  be_calback_new_level)) {
...
int be_nahraj_level(K_EDITOR *p_cnf, byte *p_file, int rekonstrukce, int textury, int mazat_materialy)
{
...
void be_prikaz_nahraj_level(K_EDITOR *p_cnf, int rekonstrukce)
{
  ...
  chdir(p_cnf->dir.edit_level_dir);
  if(doe_dialog_load(hwnd_aplikace,file,99,"Nahrani levelu...",MASKA_LEVELU,"Levely do berusek II")) {
    be_nahraj_level(p_cnf, file, rekonstrukce,TRUE,p_cnf->cnf.mazat_materialy);
  }
}
...
void be_prikaz_uloz_level(K_EDITOR *p_cnf)
{
  ...
  if(p_cnf->cnf.druh_sceny == SCENA_POLY) {
    ddw("Level je preveden do hry a neda se editovat. Pro export levelu do hry pouzij v menu BeruskyII/export");
    return;
  }
...
void be_prikaz_uloz_level_jako(K_EDITOR *p_cnf)
{
  ...
  if(doe_dialog_save(file,99,"Ulozeni levelu...",MASKA_LEVELU,"Levely do berusek II")) {
 ...
 for(i = 0; i < MAX_PATER; i++) {
   if(p_bnf->grid.kreslit[i] && p_bnf->grid.p_v[i]) {
     ...
     for(j = 0; j < p_bnf->grid.vert; j++) {
       glColor3f(p_bnf->grid.p_v[i][j].r, p_bnf->grid.p_v[i][j].g, p_bnf->grid.p_v[i][j].b);
       glVertex3f(p_bnf->grid.p_v[i][j].x, p_bnf->grid.p_v[i][j].y, p_bnf->grid.p_v[i][j].z);
     }
     glEnd();
   }
 }
...
int be_prepni_na_patro(B_KONFIG *p_bnf, int patro)
{
  if( patro > -1 && patro < p_bnf->y_num && p_bnf->grid.akt_patro != patro) {
    byte pom[100];
    memset(p_bnf->grid.kreslit,0,sizeof(p_bnf->grid.kreslit[0])*MAX_PATER);
...
void be_kresli_level_kostku(K_EDITOR *p_cnf, byte remake, byte posun)
{
 ...
 if(remake) {
   ...
   if(p_bnf->p_lev) {
     ...
     p_kont->p_obj[0]->material = p_bnf->p_kont[KONT_NEVID_STENA]->p_obj[0]->material;
 ...
 if(p_bnf->p_lev) {
   set_matrix_world(&m);
   oe_kresli_objekt(p_cnf, p_kont, p_kont->p_obj[0], p_cnf->p_mat, FALSE, FALSE);
  ...
  if(p_bnf->cnf.kreslit_stinove_objekty) {
    for(i = 0; i < MAX_PRVKU_LEVEL; i++) {
      if(p_bnf->p_prv_lev[i]) {
        ...
        s = ber_je_objekt_staticky(p_bnf->p_prv_lev[i]->p_prv_databaze->guid) ? KONT_NEVID_STENA : KONT_NEVID_PRVEK;
...
void be_prikaz_posun_tridy_lista(K_EDITOR *p_cnf, int smer)
{
 ...
 //  1 == nahoru
  ...
  SendMessage(p_cnf->bnf.kp.hwnd_list_trida,LB_SETCURSEL,p_cnf->bnf.kp.vybr_trida,0);
  SendMessage(p_cnf->bnf.kp.hwnd_dialog, WM_COMMAND, MAKEWPARAM(IDC_LIST2,LBN_SELCHANGE), (LPARAM)p_cnf->bnf.kp.hwnd_list_trida);
}
...
void be_prikaz_posun_pod_tridy_lista(K_EDITOR *p_cnf, int smer)
{
 ...
 //  1 == nahoru
  ...
  SendMessage(p_cnf->bnf.kp.hwnd_list_pod_trida,LB_SETCURSEL,p_cnf->bnf.kp.vybr_podtrida,0);
  SendMessage(p_cnf->bnf.kp.hwnd_dialog, WM_COMMAND, MAKEWPARAM(IDC_LIST1,LBN_SELCHANGE), (LPARAM)p_cnf->bnf.kp.hwnd_list_pod_trida);
}
...
void be_prikaz_posun_varianty_lista(K_EDITOR *p_cnf, int smer)
{
  ...
  if(p_cnf->bnf.kp.lista_aktivni) {
    ...
    SendMessage(p_cnf->bnf.kp.hwnd_list_varianta,LB_SETCURSEL,p_cnf->bnf.kp.vybr_varianta,0);
    SendMessage(p_cnf->bnf.kp.hwnd_dialog, WM_COMMAND, MAKEWPARAM(IDC_LIST8,LBN_SELCHANGE), (LPARAM)p_cnf->bnf.kp.hwnd_list_varianta);
  } else {
    if(p_bnf->sad.hwnd_list_trida) {
      ...
      SendMessage(p_sad->hwnd_dialog, WM_COMMAND, MAKEWPARAM(IDC_LIST1,LBN_SELCHANGE), (LPARAM)p_sad->hwnd_list_trida);
    }
  }
}
...
{
 B_KONFIG *p_bnf = &p_cnf->bnf;
 ROVINA   r = {0.0f,1.0f,0.0f,(float)(-p_bnf->grid.akt_patro*Y_PRVEK)-Y_ZACATEK};
 BOD a,b,p;
 int x,y,z;
...
void be_prikaz_vloz_prvek(K_EDITOR *p_cnf)
{
 ...
 if(be_mys_do_policka_levelu(p_cnf, &b)) {
   ...
   if(d != -1) {
     ...
     p_bnf->p_lev[i] = be_pridej_prvek_levelu_z_databaze(p_bnf, p_bnf->p_prv_dat[d], p_bnf->p_prv_dat[d]->guid);
     be_umisti_prvek( p_bnf, p_bnf->p_lev[i], b.x, b.y, b.z);
     kprintf(1,"+ prvek guid %d [%d %d %d] lay %d",p_bnf->p_prv_dat[d]->guid,b.x,b.y,b.z,lay);
   }
 }
}
...
void be_prikaz_nahrad_prvek(K_EDITOR *p_cnf, int guid)
{
 ...
 for(p = 0; p < p_bnf->prvnum; p++) {
   if(p_bnf->p_prv_lev[p]) {
     ...
     p_bnf->p_lev[i] = be_pridej_prvek_levelu_z_databaze(p_bnf,p_prv_databaze,p_prv_databaze->guid);
     ...
     kprintf(1,"+ prvek guid %d [%d %d %d] lay %d",p_prv_databaze->guid,x,y,z,pp);
   }
 }
}
...
*/
void be_prikaz_prepis_guid(K_EDITOR *p_cnf, int guid_min, int guid_max, int guid_vysl)
{
 B_KONFIG *p_bnf = &p_cnf->bnf;
 int p; 
 int guid;
 ...
 *p_cnf->p_vzdal = oe_stredni_vzdalenost_ve_scene(p_cnf);
 ...
 kprintf(TRUE,"Posun levelu na [%.3f %.3f %.3f]",p_bnf->x_start,p_bnf->y_start,p_bnf->z_start);
}
...
{
  num *= 3;
  p[num] = p1; p[num+1] = p2; p[num+2] = p3;
}

EDIT_OBJEKT * be_vyrob_obrubu_levelu(BOD *p_add, int material, 
                                     int num_x,  int num_z, 
                                     float obruba_sirka, float obruba_vyska, float obruba_zeme,
                                     int text)
{
  OBJ_VERTEX  *p_vertex;
  EDIT_OBJEKT *p_obj;
...
{ 
  ...
  if(!fast) {
    if(!doe_obruba_levelu(p_cnf, &obruba_vyska, &obruba_zeme, &obruba_sirka, &text_x)) {
      return;
    }
  }
  ...
  p_kont->p_obj[0] = be_vyrob_obrubu_levelu(&add, material,p_bnf->x_num,1,obruba_sirka, obruba_vyska, obruba_zeme, text_x);
  updatuj_kontejner_statistika(p_kont,TRUE);  
  ...
  add.y = p_bnf->y_start;
  add.z = p_bnf->z_start;
  p_kont->p_obj[0] = be_vyrob_obrubu_levelu(&add, material,1,p_bnf->z_num,obruba_sirka, obruba_vyska, obruba_zeme, !text_x);
  updatuj_kontejner_statistika(p_kont,TRUE);
...
void be_prikaz_vyrob_podlozku_levelu(K_EDITOR *p_cnf, int material)
{
  ...
  for(x = 0; x < p_bnf->x_num; x++) {
    for(z = 0; z < p_bnf->z_num; z++) {
      ...
      p_kont->p_obj[0] = be_vyrob_obrubu_levelu(&add, material,1,1,X_PRVEK,vyska_podlozky,0,FALSE);
      updatuj_kontejner_statistika(p_kont,TRUE);
    }
  }
}
...
void be_prikaz_vyrezani_levelu(K_EDITOR *p_cnf)
{
  ...
  p_cnf->elm.flag = ELIM_MAZAT|ELIM_STATIC|ELIM_PRVKY|ELIM_NORMAL|ELIM_INTELIGENT_2|ELIM_INTELIGENT_2_BRUTAL;
  oe_prikaz_decimace_sceny(p_cnf);
  if(p_cnf->groupfacenum) {
    oe_prikaz_smaz_face(p_cnf,TRUE);
  }

  p_cnf->elm.flag = ELIM_MAZAT|ELIM_STATIC|ELIM_DYNAMIC|ELIM_PRVKY|ELIM_NORMAL;
  oe_prikaz_decimace_sceny(p_cnf);
  if(p_cnf->groupfacenum) {
    oe_prikaz_smaz_face(p_cnf,TRUE);
  }
...
void be_prikaz_nahrad_aktualni_prvek(K_EDITOR *p_cnf)
{
 ...
 if(p_bnf->p_lev && be_mys_do_policka_levelu(p_cnf, &b)) {
   ...
   for(i = 0; i < MAX_PRVKU_LEVEL; i++) {
     if(p_bnf->p_prv_lev[i] && p_bnf->p_prv_lev[i]->p_prv_databaze->guid == guid) {
       j = p_bnf->p_prv_lev[i]->lev_index;
       be_pozice_na_disku(j,x,y,z,lay,p_bnf->x_num,p_bnf->z_num);       
       be_zrus_prvek_levelu(p_bnf, x, y, z, lay);
       p_bnf->p_lev[j] = be_pridej_prvek_levelu_z_databaze(p_bnf, p_bnf->p_prv_dat[d], p_bnf->p_prv_dat[d]->guid);
