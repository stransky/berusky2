...
void kom_init_level(LEVEL_HEADER * p_head)      // level header
{
  ...
  p_ber->p_hit_list =
    (int *) mmalloc(sizeof(p_ber->p_hit_list[0]) * p_ber->hitnum);
  ...
  int z)
{
  ...
  if (!p_ber->p_prv_lev[handle]->staticky && (mesh == K_CHYBA
      || !p_ber->p_mesh[mesh])) {
  ...
  int z, int rotace, int guid)
{
  ...
  lo_kopiruj_svetla_do_sceny_mesh(p_data, p_ber->p_dlight, MAX_FLARE_SVETEL,
...
void kom_zrus_level(int restart)
{
  ...
  memset(p_ber->p_lightmap, 0,
    sizeof(p_ber->p_lightmap[0]) * MAX_CELKEM_TEXTUR);
  ...
  memset(p_ber->p_mesh_renderlist, 0,
    sizeof(p_ber->p_mesh_renderlist[0]) * MAX_BERUSKY_KONTEJNERU);
...
int ber_vyber_berusky(BUNKA_LEVELU_DISK * p_disk, int num,
  BUNKA_LEVELU_DISK * p_berusky)
{
  int i, berusek = 0;
  ...
  BUNKA_LEVELU_DISK * p_bunka, int bunek)
{
  ...
  if (!ber_nahraj_materialy(p_ber, file, dir)) {
    kprintf(TRUE,
      "Error loading level materials %s from directory %s, stop loading..",
      file, dir);
    konec(TRUE);
  }
  ...
  if (!ret) {
    kprintf(TRUE, "Error loading scene %s from directory %s, stop loading...",
      file, dir);
    konec(TRUE);
  }
  else {
  ...
  time = timeGetTime();
  if (!restart) {
    int bump = hwconf.bump_mapping && p_ber->conf_dyn_light
  ...
  vertexu = 0;
  vel3 =
    lo_velikost_meshu(p_ber->p_mesh, p_ber->meshnum, &facu, &vertexu) / 1000;
  vel4 =
    lo_velikost_poly(p_ber->p_poly, p_ber->polynum, &facu, &vertexu) / 1000;
...
{
  if (p_ber->p_mat[mh]
    && (strlen(p_ber->p_mat[mh]->jmeno) < (dword) max_znaku)) {
    ...
    if (p_prv && p_prv->guid && p_prv->guid / 1000 != itrida) {
      if (p_prv->mesh != K_CHYBA) {
        ...
        kd_bunka_expanze3(p_prv->x, p_prv->y, p_prv->z,
          X_PRVEK2, Y_PRVEK2, Z_PRVEK2, &min, &max);
        p_min = &min;
        p_max = &max;
      }
...
MeshHandle kom_mesh_set_mesh(MeshHandle mh, int ID, int rotace)
{
  ...
  int mesh = p_lev->mesh;
  GAME_MESH_OLD *p_prev = NULL,
    *p_mesh = p_ber->p_mesh[mesh], *p_last = p_mesh;
