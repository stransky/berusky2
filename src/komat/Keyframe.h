...
float  key_track_interpolace_float(float *p_vysl, float *p_body, KEY_POINT_BRS *p_keys, int time, int endtime, int keynum, int loop);
BOD  * key_track_interpolace_bod(BOD *p_vysl, BOD *p_body, KEY_POINT_BRS *p_keys, int time, int endtime, int keynum, int loop);
QUAT * key_track_interpolace_quat(QUAT *p_quat, QUAT *p_at, KEY_POINT_BRS *p_rkeys, int time, int endtime, int keynum, int loop);
WBOD * key_track_interpolace_wbod(WBOD *p_vysl, WBOD *p_body, KEY_POINT_BRS *p_keys, int time, int endtime, int keynum, int loop);

float  key_track_interpolace_float_linear(float *p_vysl, float *p_body, KEY_POINT_BRS *p_keys, int time, int endtime, int keynum, int loop);
BOD  * key_track_interpolace_bod_linear(BOD *p_vysl, BOD *p_body, KEY_POINT_BRS *p_keys, int time, int endtime, int keynum, int loop);
QUAT * key_track_interpolace_quat_linear(QUAT *p_quat, QUAT *p_at, KEY_POINT_BRS *p_rkeys, int time, int endtime, int keynum, int loop);
WBOD * key_track_interpolace_wbod_linear(WBOD *p_vysl, WBOD *p_body, KEY_POINT_BRS *p_keys, int time, int endtime, int keynum, int loop);
...
inline void key_mesh_recalc_normal_anim(GAME_MESH *p_mesh)
{
  ...
  if(kflag&KONT_NORMAL_ANIM) {
    ...
    for(o = 0; o < p_mesh->objektu; o++) {
      if((last = p_mesh->p_normal_scale_last[o]) == (akt = p_mesh->p_normal_scale[o]))
        continue;
      ...
      for(i = 0; i < num; i++, p_vertex++, p_vertex_norm++) {
        vektor_add(p_vertex,vektor_mult_skalar(p_vertex_norm,(last-akt),&n),p_vertex);
        if(vr) {
          vektor_add(p_vertex,vektor_mult_skalar(&up,(akt-last),&n),p_vertex);
        }
      }
...
{  
  int i; 
  for(i = 0; i < keynum; i++) {
    if(p_quat[i].x == 0 && p_quat[i].y == 0 && p_quat[i].z == 0 && p_quat[i].w == 0)
      p_quat[i].w = 1;
  }
}
