...
int lo_chunk_save_kamset(FFILE f, NASTAVENI_KAMER * p_kam)
{
  ...
  ch.velikost = sizeof(ch) +
    ...
    sizeof(p_kam->min_vzdal) + sizeof(p_kam->max_vzdal);
...
int lo_chunk_save_kamset_2(FFILE f, NASTAVENI_KAMER * p_kam)
{
  ...
  ch.velikost = sizeof(ch) + sizeof(p_kam->max) + sizeof(p_kam->min);
...
int lo_chunk_save_dyn_light_anim(FFILE f, DYN_LIGHT_ANIM * p_an, int extra)
{
  ...
  if (!extra) {
    ...
    p_an->p_diff =
      (BARVA_RGBA *) mmalloc(sizeof(p_an->p_diff[0]) * p_an->alfa_keys);
    for (i = 0; i < p_an->alfa_keys; i++) {
      p_an->p_diff[i].r = p_an->p_diff[i].g = p_an->p_diff[i].b =
        p_an->p_diff[i].a = p_an->p_alfa[i];
    }
...
int lo_chunk_save_dyn_light(FFILE f, DYN_LIGHT * p_light)
{
  ...
  ch.velikost =
    sizeof(ch) + sizeof(p_light->akt) + sizeof(byte) + sizeof(p_light->np) +
    sizeof(p_light->dir) + sizeof(p_light->look_max) +
    sizeof(p_light->dosah) + sizeof(p_light->min) + sizeof(p_light->max) +
    sizeof(p_light->theta) + sizeof(p_light->dr) + sizeof(p_light->dg) +
    sizeof(p_light->db) + sizeof(p_light->an_flag) + sizeof(p_light->flag);
...
void lo_chunk_save_mlhokostku(FFILE f, MLZNA_KOSTKA * p_top, int kont)
{
  ...
  while (p_top) {
    ...
    ch.velikost = sizeof(ch) +
      ...
      sizeof(float) * 4 + sizeof(int) + sizeof(int);
...
/* **********************************************************************
   Ulozeni flare
*/
void lo_chunk_save_flare(FFILE f, LENS_FLARE * p_flare,
  EDIT_MATERIAL ** p_mat, int light)
{
  OUT_CHUNK ch;
  char pom[200];
  int nic = K_CHYBA;
  ...
  ch.velikost = sizeof(ch) +
    ...
    sizeof(float) * 4 +
    sizeof(float) * 2 +
    sizeof(int) + sizeof(int) + sizeof(int) + strlen(pom) + 1;
...
void lo_chunk_save_flare_mesh(FFILE f, LENS_FLARE * p_flare,
  EDIT_MATERIAL ** p_mat)
{
  OUT_CHUNK ch;
  char pom[200];
...
void lo_chunk_save_flare_slozeny(FFILE f, LENS_FLARE * p_flare,
  EDIT_MATERIAL ** p_mat)
{
  OUT_CHUNK ch;
  ...
  ch.velikost = sizeof(ch) +
    ...
    sizeof(float) * 4 * SLOZ_FLARE + sizeof(float) * 2 * SLOZ_FLARE + d;
...
void lo_chunk_save_lc_ext(FFILE f, LEVEL_KONFIG * p_lc)
{
  ...
  ch.velikost = sizeof(ch) +
    ...
    sizeof(p_lc->zm.h_stop) + sizeof(p_lc->zm.barva);

  ffwrite(&ch, sizeof(ch), 1, f);
  ffwrite(&p_lc->barva_pozadi, sizeof(p_lc->barva_pozadi), 1, f);
  ffwrite(&p_lc->barva_pozadi_aktivni, sizeof(p_lc->barva_pozadi_aktivni), 1,
    f);
...
/* **********************************************************************
   Ulozeni zrcadla
*/
void lo_chunk_preved_zrcadlo(EDIT_KONTEJNER ** p_kont, int kontnum,
  ZDRCADLO_DESC * p_desc)
{
  ...
  EDIT_MATERIAL ** p_mat)
{
  ...
  ch.velikost =
    sizeof(ch) + sizeof(p_light->akt) + strlen(p_light->jmeno) + 1 +
...
int lo_chunk_save_static_light_mesh(FFILE f, STATIC_LIGHT * p_light,
  EDIT_MATERIAL ** p_mat)
{
  LENS_FLARE *p_flare;
  OUT_CHUNK ch;
...
int lo_chunk_save_mat_matanim_frame(FFILE f, ANIM_FRAME * p_fram, int frame)
{
  ...
  if (p_fram) {
    ...
    ch.velikost =
      sizeof(ch) + strlen(p_text->jmeno) + 1 + sizeof(p_text->flag)
...
int lo_chunk_save_mat_matanim(FFILE f, EDIT_MATERIAL * p_mat)
{
  ...
  if (p_mat->flag & MAT_ANIM_FRAME) {
    ...
    ch.velikost =
      sizeof(ch) + strlen(p_mat->anim.jmeno) + 1 + sizeof(p_anim->framenum);
...
int lo_chunk_save_mat_matanim_text_dal(FFILE f, ANIM_TEXT * p_track, int typ)
{
  ...
  if (p_track->pos_keys && p_track->p_pos) {
    ...
    ffwrite(p_track->p_pkeys, sizeof(p_track->p_pkeys[0]), p_track->pos_keys,
      f);
  }
  if (p_track->piv_keys && p_track->p_piv) {
    ffwrite(p_track->p_piv, sizeof(p_track->p_piv[0]), p_track->piv_keys, f);
    ffwrite(p_track->p_vkeys, sizeof(p_track->p_vkeys[0]), p_track->piv_keys,
      f);
  }
  if (p_track->rot_keys && p_track->p_rot) {
    ffwrite(p_track->p_rot, sizeof(p_track->p_rot[0]), p_track->rot_keys, f);
    ffwrite(p_track->p_rkeys, sizeof(p_track->p_rkeys[0]), p_track->rot_keys,
      f);
  }
  if (p_track->scs_keys && p_track->p_scale) {
    ffwrite(p_track->p_scale, sizeof(p_track->p_scale[0]), p_track->scs_keys,
      f);
    ffwrite(p_track->p_skeys, sizeof(p_track->p_skeys[0]), p_track->scs_keys,
      f);
  }
  return (TRUE);
}
    ...
    if (p_mat->flag & glstav_posun_flagy2[j]) {
      for (i = 0; i < MAX_TEXT_ANIM; i++) {
        if (p_mat->p_atext[j][i]) {
          lo_chunk_save_mat_matanim_text_dal(f, p_mat->p_atext[j][i],
            CHUNK_MATANIM_T1 + j);
        }
      }
    }
...
int lo_chunk_save_mat_env_spec(FFILE f, EDIT_MATERIAL * p_mat)
{
  ...
  ch.velikost = sizeof(ch) +
    ...
    sizeof(p_mat->env_r) + sizeof(p_mat->env_g) + sizeof(p_mat->env_b);
...
int lo_chunk_save_mat_text_blend_ext(FFILE f, EDIT_MATERIAL * p_mat, int num)
{
  ...
  ch.velikost =
    sizeof(ch) + sizeof(int) * 1 + sizeof(int) * 4 + sizeof(int) * 4 +
    sizeof(int) * 4 +
    sizeof(p_mat->text_state[num].text_ati_bump_matrix[0]) * 4;
  ffwrite(&ch, sizeof(ch), 1, f);
...
int lo_chunk_save_mat_text(FFILE f, EDIT_MATERIAL * p_mat, int num, int typ)
{
  ...
  if (p_mat->textfile[num][0]) {
    ...
    ffwrite(&ch, sizeof(ch), 1, f);
    ffwrite(p_mat->textfile[num], sizeof(char),
      strlen(p_mat->textfile[num]) + 1, f);
    return (TRUE);
  }
  else
    return (FALSE);
...
int lo_chunk_save_mat_text_flag(FFILE f, EDIT_MATERIAL * p_mat, int num,
  int typ)
{
  OUT_CHUNK ch;
...
int lo_chunk_save_keyframe_node(FFILE f, HIERARCHY_TRACK_INFO * p_node)
{
  ...
  if (p_node) {
    ...
    ch.velikost = sizeof(ch) +
      sizeof(HIERARCHY_TRACK_INFO) + strlen(jmeno) + 1 + strlen(otec) + 1;
    ffwrite(&ch, sizeof(ch), 1, f);
...
int lo_chunk_save_sim_root(FFILE f, HIERARCHY_SIM * p_sim)
{
  ...
  if (p_sim) {
    ...
    ch.velikost = sizeof(ch) +
      sizeof(p_sim->flag) +
      sizeof(p_sim->keynum) +
      sizeof(p_sim->jmeno[0]) * MAX_JMENO + sizeof(p_sim->childnum);
...
int lo_chunk_save_sim_node_ext(FFILE f, SIMPLE_TRACK_INFO * p_sim)
{
  ...
  if (p_sim) {
    ...
    ch.velikost = sizeof(ch) +
      ...
      sizeof(klicu) * 3 +
      (p_sim->p_pos ? sizeof(p_sim->p_pos[0]) * p_sim->keynum : 0)
...
int lo_chunk_save_kont_jmeno(FFILE f, EDIT_KONTEJNER * p_kont, int pozice)
{
  ...
  if (p_kont) {
    ...
    ch.velikost =
      sizeof(ch) + sizeof(p_kont->prvek) + strlen(p_kont->jmeno) + 1;
    ffwrite(&ch, sizeof(ch), 1, f);
...
int lo_chunk_save_kont_flag_ext(FFILE f, EDIT_KONTEJNER * p_kont)
{
  ...
  if (p_kont) {
    ...
    ch.velikost =
      sizeof(ch) + sizeof(p_kont->m1flag) + sizeof(p_kont->m2flag);
    ffwrite(&ch, sizeof(ch), 1, f);
...
int lo_chunk_save_kont_wmatrix(FFILE f, EDIT_KONTEJNER * p_kont,
  GLMATRIX * p_matrix)
{
  OUT_CHUNK ch;
...
int lo_chunk_save_kont_mpmatrix(FFILE f, EDIT_KONTEJNER * p_kont,
  GLMATRIX * p_matrix)
{
  OUT_CHUNK ch;
...
int lo_chunk_save_obj_vertex(FFILE f, EDIT_OBJEKT * p_obj,
  GLMATRIX * p_transf)
{
  OUT_CHUNK ch;
  dword pocet, i;
  ...
  if (p_obj) {
    ...
    ch.velikost = sizeof(ch) +
      sizeof(dword) + sizeof(float) * 3 * p_obj->vertexnum;
    ffwrite(&ch, sizeof(ch), 1, f);
...
int lo_chunk_save_obj_face(FFILE f, EDIT_OBJEKT * p_obj)
{
  ...
  if (p_obj) {
    ...
    ch.velikost = sizeof(ch) + sizeof(dword) + sizeof(word) * p_obj->facenum;
    ffwrite(&ch, sizeof(ch), 1, f);
    pocet = p_obj->facenum;
...
int lo_chunk_save_obj_mapy_1(FFILE f, EDIT_OBJEKT * p_obj)
{
  ...
  if (p_obj) {
    ...
    ch.velikost = sizeof(ch) +
      sizeof(dword) + sizeof(float) * 2 * p_obj->vertexnum;
    ffwrite(&ch, sizeof(ch), 1, f);
    pocet = p_obj->vertexnum;
    if (!ffwrite(&pocet, sizeof(pocet), 1, f))
      chyba("Zapis");
...
int lo_chunk_save_obj_barvy(FFILE f, EDIT_OBJEKT * p_obj)
{
  ...
  if (p_obj) {
    ...
    ch.velikost = sizeof(ch) +
      sizeof(dword) + sizeof(float) * 4 * p_obj->vertexnum;
    ffwrite(&ch, sizeof(ch), 1, f);
...
int lo_chunk_save_obj_normal(FFILE f, EDIT_OBJEKT * p_obj)
{
  ...
  if (p_obj) {
    ...
    ch.velikost =
      sizeof(ch) + sizeof(pocet) + sizeof(float) * 3 * p_obj->vertexnum;
...
int lo_chunk_save_obj_barvy_ext(FFILE f, EDIT_OBJEKT * p_obj)
{
  ...
  if (p_obj) {
    ...
    ch.velikost = sizeof(ch) +
      sizeof(dword) + sizeof(float) * 4 * p_obj->vertexnum * 4;
    ffwrite(&ch, sizeof(ch), 1, f);
...
int lo_chunk_save_obj_joint_spline(FFILE f, JOINT_KEYS * p_keys)
{
  ...
  if (p_keys->p_pos) {
    ...
    ch.velikost =
      sizeof(ch) + sizeof(p_keys->pos_keys) +
      p_keys->pos_keys * sizeof(p_keys->p_pos[0]) +
      p_keys->pos_keys * sizeof(p_keys->p_pkeys[0]);
    ffwrite(&ch, sizeof(ch), 1, f);
  ...
  if (p_keys->p_rot) {
    ...
    ch.velikost =
      sizeof(ch) + sizeof(p_keys->rot_keys) +
      p_keys->rot_keys * sizeof(p_keys->p_rot[0]) +
      p_keys->rot_keys * sizeof(p_keys->p_rkeys[0]);
    ffwrite(&ch, sizeof(ch), 1, f);
  ...
  if (p_keys->p_scale) {
    ...
    ch.velikost =
      sizeof(ch) + sizeof(p_keys->scs_keys) +
      p_keys->scs_keys * sizeof(p_keys->p_scale[0]) +
      p_keys->scs_keys * sizeof(p_keys->p_skeys[0]);
    ffwrite(&ch, sizeof(ch), 1, f);
...
int lo_chunk_save_obj_joint(FFILE f, JOINT * p_joint, int id_prev)
{
  ...
  if (p_joint->p_vertexlist) {
    ...
    ch.velikost =
      ...
      p_joint->vertexnum * sizeof(p_joint->p_vertexlist[0]);
    ffwrite(&ch, sizeof(ch), 1, f);
    ffwrite(&p_joint->vertexnum, sizeof(p_joint->vertexnum), 1, f);
    ffwrite(p_joint->p_vertexlist, sizeof(p_joint->p_vertexlist[0]),
      p_joint->vertexnum, f);
  }
  ...
  if (p_joint->p_pos) {
    ...
    ch.velikost =
      sizeof(ch) + sizeof(p_joint->pos_keys) +
      p_joint->pos_keys * sizeof(p_joint->p_pos[0]);
    ffwrite(&ch, sizeof(ch), 1, f);
  ...
  if (p_joint->p_rot) {
    ...
    ch.velikost =
      sizeof(ch) + sizeof(p_joint->rot_keys) +
      p_joint->rot_keys * sizeof(p_joint->p_rot[0]);
    ffwrite(&ch, sizeof(ch), 1, f);
  ...
  if (p_joint->p_scs) {
    ...
    ch.velikost =
      sizeof(ch) + sizeof(p_joint->scs_keys) +
      p_joint->scs_keys * sizeof(p_joint->p_scs[0]);
    ffwrite(&ch, sizeof(ch), 1, f);
...
int lo_chunk_save_kont_mesh(FFILE f, EDIT_KONTEJNER * p_kont,
  EDIT_MATERIAL ** p_mat, int max_mat, GLMATRIX * p_world)
{
  int i, j;
...
int lo_chunk_save_kontejner(FFILE f, EDIT_KONTEJNER * p_kont,
  EDIT_MATERIAL ** p_mat, int max_mat, int low, int kont, int kont_id,
  int transf_stat)
{
  int i;
...
int lo_uloz_materialy_chunk_kont(FFILE f, EDIT_MATERIAL ** p_mat, int max_mat,
  EDIT_KONTEJNER * p_kont, int kontrola)
{
  int i, j;
...
int lo_uloz_materialy_chunk_pouzite(FFILE f, EDIT_MATERIAL ** p_mat,
  int max_mat)
{
  int i;
  ...
  EDIT_KONTEJNER * p_kont, int low)
{
  ...
  /* Uloz kontejner
   */
  lo_chunk_save_kontejner(f, p_kont, p_mat, max_mat, low, K_CHYBA, K_CHYBA,
    FALSE);
...
int lo_uloz_projekt(EDIT_MATERIAL ** p_mat, int max_mat,
  EDIT_KONTEJNER ** p_kontlist, int kontnum, char *p_jmeno,
  STATIC_LIGHT * p_light, int lightnum, ZDRCADLO_DESC * p_zrcadlo,
  LENS_FLARE * p_flarr, int flaremax, dword ambient, MLZNA_KOSTKA * p_mlha,
  DYN_LIGHT * p_ddlight, int ddlightnum, EXTRA_DYN_LIGHT * p_eelight,
  int eelightnum, LEVEL_ENVIRONMENT * p_env, NASTAVENI_KAMER * p_kam,
  LEVEL_KONFIG * p_lc, int level, int transf_static)
{
  ...
  for (j = 0; j < kontnum; j++) {
    if ((p_kont = p_kontlist[j])) {
      ...
      lo_chunk_save_kontejner(f, p_kont, p_mat, max_mat, FALSE, K_CHYBA, kID,
        transf_static);

      p_kont = p_kont->p_next;
      while (p_kont) {
        lo_uloz_materialy_chunk_kont(f, p_mat, max_mat, p_kont, TRUE);
        lo_chunk_save_kontejner(f, p_kont, p_mat, max_mat, TRUE, K_CHYBA, kID,
          transf_static);
        p_kont = p_kont->p_next;
      }
    }
...
int lo_chunk_load_dyn_light_anim(FFILE f, DYN_LIGHT_ANIM * p_an, int extra)
{
  ...
  if (p_an->pos_keys) {
    p_an->p_pkeys =
      (KEY_POINT_BRS *) mmalloc(sizeof(p_an->p_pkeys[0]) * p_an->pos_keys);
  ...
  if (p_an->trg_keys) {
    p_an->p_tkeys =
      (KEY_POINT_BRS *) mmalloc(sizeof(p_an->p_tkeys[0]) * p_an->trg_keys);
  ...
  if (p_an->dos_keys) {
    p_an->p_dskeys =
      (KEY_POINT_BRS *) mmalloc(sizeof(p_an->p_dskeys[0]) * p_an->dos_keys);
    ffread(p_an->p_dskeys, sizeof(p_an->p_dskeys[0]), p_an->dos_keys, f);

    p_an->p_dosah =
      (BOD *) mmalloc(sizeof(p_an->p_dosah[0]) * p_an->dos_keys);
  ...
  if (extra) {
    ...
    if (p_an->alfa_keys) {
      ...
      p_an->p_akeys =
        (KEY_POINT_BRS *) mmalloc(sizeof(p_an->p_akeys[0]) * p_an->alfa_keys);
      ...
      p_an->p_alfa =
        (float *) mmalloc(sizeof(p_an->p_alfa[0]) * p_an->alfa_keys);
      for (i = 0; i < p_an->alfa_keys; i++) {
    ...
    if (p_an->diff_keys) {
      p_an->p_dfkeys =
        (KEY_POINT_BRS *) mmalloc(sizeof(p_an->p_dfkeys[0]) *
        p_an->diff_keys);
      ffread(p_an->p_dfkeys, sizeof(p_an->p_dfkeys[0]), p_an->diff_keys, f);

      p_an->p_diff =
        (BARVA_RGBA *) mmalloc(sizeof(p_an->p_diff[0]) * p_an->diff_keys);
  ...
  if (p_an->spec_keys) {
    p_an->p_spkeys =
      (KEY_POINT_BRS *) mmalloc(sizeof(p_an->p_spkeys[0]) * p_an->spec_keys);
    ffread(p_an->p_spkeys, sizeof(p_an->p_spkeys[0]), p_an->spec_keys, f);

    p_an->p_spec =
      (BARVA_RGBA *) mmalloc(sizeof(p_an->p_spec[0]) * p_an->spec_keys);
...
int lo_chunk_load_flare(FFILE f, OUT_CHUNK * p_ch)
{
  ...
  if (p_flarelist && p_ch->typ == CHUNK_FLARE) {
    ...
    str_read(pom, f);
    if ((p_flare->material =
        lo_najdi_material(p_matlist, matlistnum, pom)) == K_CHYBA) {
      ...
      p_flare->p_mat =
        (p_flare->material <
        matlistnum) ? p_matlist[p_flare->material] : NULL;
    return (TRUE);
  }
  else
    return (FALSE);
...
int lo_chunk_load_flare_slozeny(FFILE f, OUT_CHUNK * p_ch)
{
  ...
  if (p_flare && p_ch->typ == CHUNK_FLARE_SLOZENY) {
    p_flare->p_sloz =
      (LENS_FLARE_SLOZ *) mmalloc(sizeof(p_flare->p_sloz[0]) * SLOZ_FLARE);
    ...
    for (s = 0; s < SLOZ_FLARE; s++) {
      ...
      str_read(pom, f);
      if (p_flare->p_sloz[s].vzdal > 0.0f) {
        if ((p_flare->p_sloz[s].material =
            lo_najdi_material(p_matlist, matlistnum, pom)) == K_CHYBA) {
          kprintf(1, "Cannot find material %s", pom);
          assert(0);
        }
        p_flare->p_sloz[s].p_mat =
          (p_flare->p_sloz[s].material <
          matlistnum) ? p_matlist[p_flare->p_sloz[s].material] : NULL;
      }
      else {
...
int lo_chunk_load_levelconfig(FFILE f, OUT_CHUNK * p_ch)
{
  if (p_ch->typ == CHUNK_LEVEL_KONFIG_EXT) {
    ...
    ffread(&p_lc->barva_pozadi_aktivni, sizeof(p_lc->barva_pozadi_aktivni), 1,
      f);
...
int lo_chunk_load_matanim_frame(FFILE f, OUT_CHUNK * p_ch)
{
  ...
  if (p_mat && p_fanim && p_ch->typ == CHUNK_MATANIM_FRAME) {
    ...
    if ((t =
        lo_najdi_texturu(p_textlist, textlistnum, p_fram->file,
          flag)) == K_CHYBA) {
...
int lo_chunk_load_matanim(FFILE f, OUT_CHUNK * p_ch)
{
  if (p_mat && p_ch->typ == CHUNK_MATANIM_EXT) {
    ...
    p_fanim->p_frame =
      (ANIM_FRAME *) mmalloc(sizeof(p_fanim->p_frame[0]) * p_fanim->framenum);
    return (TRUE);
  }
  else
    return (FALSE);
...
int lo_chunk_load_matanim_text_1(FFILE f, OUT_CHUNK * p_ch)
{
  ...
  if (p_mat && p_ch->typ == CHUNK_MATANIM_T1) {
    ...
    p_track = p_mat->p_atext[0][i] =
      key_vyrob_material_animace(tmp.pos_keys, tmp.rot_keys, tmp.scs_keys,
      tmp.piv_keys);
    ...
    if (p_track->pos_keys) {
      ...
      ffread(p_track->p_pkeys, sizeof(p_track->p_pkeys[0]), p_track->pos_keys,
        f);
    }

    if (p_track->piv_keys) {
      ffread(p_track->p_piv, sizeof(p_track->p_piv[0]), p_track->piv_keys, f);
      ffread(p_track->p_vkeys, sizeof(p_track->p_vkeys[0]), p_track->piv_keys,
        f);
    }

    if (p_track->rot_keys) {
      ffread(p_track->p_rot, sizeof(p_track->p_rot[0]), p_track->rot_keys, f);
      ffread(p_track->p_rkeys, sizeof(p_track->p_rkeys[0]), p_track->rot_keys,
        f);
    }

    if (p_track->scs_keys) {
      ffread(p_track->p_scale, sizeof(p_track->p_scale[0]), p_track->scs_keys,
        f);
      ffread(p_track->p_skeys, sizeof(p_track->p_skeys[0]), p_track->scs_keys,
        f);
    }
    return (TRUE);
  }
...
int lo_chunk_load_text(FFILE f, OUT_CHUNK * p_ch)
{
  if (p_mat) {
    ...
    if ((t =
        lo_najdi_texturu(p_textlist, textlistnum, p_mat->textfile[num],
          p_mat->textflag[num])) == K_CHYBA) {
      if ((t = lo_najdi_volnou_texturu(p_textlist, textlistnum)) == K_CHYBA)
        chyba("Neni volna textura !");
      // Zaloz novou texturu      
...
int lo_chunk_load_kont_jmeno(FFILE f, OUT_CHUNK * p_ch)
{
  ...
  if (p_ch->typ == CHUNK_KONT_JMENO) {
    ...
    if (p_kontlist) {
      if (pozice == K_CHYBA) {
        if ((i =
            lo_najdi_volny_kontejner(p_kontlist, kontlistnum)) == K_CHYBA) {
...
int lo_chunk_load_obj_material(FFILE f, OUT_CHUNK * p_ch)
{
  ...
  if (p_obj && p_ch->typ == CHUNK_OBJ_MATERIAL) {
    ...
    if ((p_obj->material =
        lo_najdi_material(p_matlist, matlistnum,
          jmeno_materialu)) == K_CHYBA) {
...
int lo_chunk_load_obj_vertex(FFILE f, OUT_CHUNK * p_ch)
{
  ...
  if (p_obj && p_ch->typ == CHUNK_OBJ_VERTEXY) {
    ...
    if (!(p_obj->p_vertex =
        (OBJEKT_BOD *) mmalloc(sizeof(OBJEKT_BOD) * pocet)))
...
int lo_chunk_load_obj_joint_vertexy(FFILE f, OUT_CHUNK * p_ch)
{
  if (p_joint && p_ch->typ == CHUNK_OBJ_JNT_VERT) {
    ...
    p_joint->p_vertexlist =
      (int *) mmalloc(sizeof(p_joint->p_vertexlist[0]) * p_joint->vertexnum);
    ffread(p_joint->p_vertexlist, sizeof(p_joint->p_vertexlist[0]),
      p_joint->vertexnum, f);
  }
  return (TRUE);
}
...
int lo_chunk_load_obj_joint_pos(FFILE f, OUT_CHUNK * p_ch)
{
  if (p_joint && p_ch->typ == CHUNK_OBJ_JNT_POS) {
    ...
    p_joint->p_pos =
      (BOD *) mmalloc(sizeof(p_joint->p_pos[0]) * p_joint->pos_keys);
...
int lo_chunk_load_obj_joint_rot(FFILE f, OUT_CHUNK * p_ch)
{
  if (p_joint && p_ch->typ == CHUNK_OBJ_JNT_ROT) {
    ...
    p_joint->p_rot =
      (QUAT *) mmalloc(sizeof(p_joint->p_rot[0]) * p_joint->rot_keys);
...
int lo_chunk_load_obj_joint_scs(FFILE f, OUT_CHUNK * p_ch)
{
  if (p_joint && p_ch->typ == CHUNK_OBJ_JNT_SCS) {
    ...
    p_joint->p_scs =
      (BOD *) mmalloc(sizeof(p_joint->p_scs[0]) * p_joint->scs_keys);
  ...
  if (p_joint && p_ch->typ == CHUNK_OBJ_JNT_KEYPOS) {
    if (!p_joint->p_keys) {
      p_key = p_joint->p_keys =
        (JOINT_KEYS *) mmalloc(sizeof(p_joint->p_keys[0]));
    }
    p_key = p_joint->p_keys;
    ...
    p_key->p_pkeys =
      (KEY_POINT_BRS *) mmalloc(sizeof(p_key->p_pkeys[0]) * p_key->pos_keys);
...
int lo_chunk_load_obj_joint_rot_spline(FFILE f, OUT_CHUNK * p_ch)
{
  ...
  if (p_joint && p_ch->typ == CHUNK_OBJ_JNT_KEYROT) {
    ...
    p_key->p_rot =
      (QUAT *) mmalloc(sizeof(p_key->p_rot[0]) * p_key->rot_keys);
    ffread(p_key->p_rot, sizeof(p_key->p_rot[0]), p_key->rot_keys, f);

    p_key->p_rkeys =
      (KEY_POINT_BRS *) mmalloc(sizeof(p_key->p_rkeys[0]) * p_key->rot_keys);
...
int lo_chunk_load_obj_joint_scs_spline(FFILE f, OUT_CHUNK * p_ch)
{
  ...
  if (p_joint && p_ch->typ == CHUNK_OBJ_JNT_KEYSCS) {
    ...
    p_key->p_scale =
      (BOD *) mmalloc(sizeof(p_key->p_scale[0]) * p_key->scs_keys);
    ffread(p_key->p_scale, sizeof(p_key->p_scale[0]), p_key->scs_keys, f);

    p_key->p_skeys =
      (KEY_POINT_BRS *) mmalloc(sizeof(p_key->p_skeys[0]) * p_key->scs_keys);
...
int lo_chunk_load_obj_joint_framy(FFILE f, OUT_CHUNK * p_ch)
{
  if (p_joint_animace && p_ch->typ == CHUNK_OBJ_JA_FRAMU) {
    ffread(&p_joint_animace->framenum, sizeof(p_joint_animace->framenum), 1,
      f);
  }
  return (TRUE);
}
...
int lo_chunk_load_sim_node(FFILE f, SIMPLE_TRACK_INFO * p_sim)
{
  ...
  if (p_sim && (ch.typ == CHUNK_SIM_NODE || ch.typ == CHUNK_SIM_NODE_EXT
    ...
    if (ch.typ == CHUNK_SIM_NODE_EXT2) {
      p_sim->p_all_matrix =
        (GLMATRIX *) mmalloc(sizeof(p_sim->p_all_matrix[0]));
...
int lo_chunk_load_sim_root(FFILE f, OUT_CHUNK * p_ch)
{
  ...
  if (p_kont && p_ch->typ == CHUNK_SIM_ROOT) {
    ...
    if (p_sim->childnum) {
      p_sim->p_child =
        (SIMPLE_TRACK_INFO *) malloc(sizeof(p_sim->p_child[0]) *
        p_sim->childnum);
      for (i = 0; i < p_sim->childnum; i++)
        lo_chunk_load_sim_node(f, p_sim->p_child + i);
    }
...
int lo_nahraj_projekt(EDIT_MATERIAL ** p_mat, int max_mat,
  ...
  NASTAVENI_KAMER * p_kam, LEVEL_KONFIG * p_lev_konfig, int flag, int normal)
{
  FFILE f;
  int i;
  ...
  for (i = 0; i < kontnum; i++) {
    if (p_kontls[i] && p_kontls[i]->k2flag & KONT2_LOADED) {
      ...
      lo_preved_svetla_do_sceny(p_kontls[i], p_light, lightnum, p_ddlight,
        ddlightnum, p_eelight, eelightnum);
...
int lo_nahraj_materialy_chunk(EDIT_MATERIAL ** p_mat, int max_mat,
  EDIT_TEXT * p_text, int max_text, char *p_file, int save)
{
  FFILE f;
