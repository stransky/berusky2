...
{
 p_ber->x_num = x; p_ber->y_num = y; p_ber->z_num = z;
 p_ber->prvnum = 0; 
}
  ...
  for(i = 0; i < MAX_PRVKU_LEVEL; i++) {
    if(!p_ber->p_prv_lev[i]) {
      ...
      if(p_lev->staticky && !ber_je_objekt_voda(p_disk->guid)) {
        assert(ber_pozice_v_levelu(x,y>>1,z,p_ber->x_num,p_ber->z_num) < p_ber->hitnum);
        p_ber->p_hit_list[ber_pozice_v_levelu(x,y>>1,z,p_ber->x_num,p_ber->z_num)] = TRUE;
      }
    ...
    for(i = 0; i < p_mesh->objektu; i++) {
      ...
      if(p_mat->flag&MAT_ANIM_FRAME) {
        if(restart) {
          ...
          mnew = lo_najdi_prazdny_material(p_ber->p_mat, MAX_CELKEM_MATERIALU);
          assert(mnew != K_CHYBA);
...
{    
  ...
  */
  ...
  p_mat->text_state[AUTO_STAGE].textury[DOT3_TEXT] = mat_text = state_stary.textury[0];
  ...
  if((btext = lo_najdi_texturu(p_text,textnum,p_mat->textfile[mat_text],TEXT_GEN_DOT3)) == K_CHYBA) {
...
int ber_mat_vloz_dot3_env(EDIT_TEXT *p_text, int textnum, EDIT_MATERIAL *p_mat)
{
  ...
  */
  ...
  if((btext = lo_najdi_texturu(p_text,textnum,BUMP_ENV_TEXT,FALSE)) == K_CHYBA) {
    if((btext = lo_najdi_volnou_texturu(p_text,textnum)) == K_CHYBA)
      chyba("Textury");
...
{ 
  ...
  if(lo_nahraj_projekt(p_ber->p_mat,MAX_CELKEM_MATERIALU,
    ...
    p_ber->conf_barva_pozadi_pouzit = p_ber->conf_barva_pozadi_pouzit_default ? TRUE : lc.barva_pozadi_aktivni;
    p_ber->zm = lc.zm;
...
inline void bod_min_max(BOD *p_ref, BOD *p_min, BOD *p_max)
{
  ...
  if(p_ref->z < p_min->z) p_min->z = p_ref->z;

  if(p_ref->x > p_max->x) p_max->x = p_ref->x;
  if(p_ref->y > p_max->y) p_max->y = p_ref->y;
  if(p_ref->z > p_max->z) p_max->z = p_ref->z;
}
