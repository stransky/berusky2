...
// save demo
//------------------------------------------------------------------------------------------------
int demo_Save(DEMOSTRUCTURE *p_Demo, char *p_File_Name, char Ovladani, char *cLevelName)
{
	DEMOFILEHEADER	FileHeader;
	FILE	*file;
	int		i;
...
int demo_SaveWC(DEMOSTRUCTURE *p_Demo, WCHAR *wcName, char Ovladani, char *cLevelName, char *cLoadSignature)
{
	char	cText[1024];
	char	cFile[1024];
	char	pom[128];
...
// load demo
//------------------------------------------------------------------------------------------------
int demo_Load(DEMOSTRUCTURE *p_Demo, char *p_File_Name, char *bOvladani, char *cLevel_Name, char *cLoadedSignature)
{
	DEMOFILEHEADER	FileHeader;
...
int demo_Create_Sequence(int *pKeyLine, int iKLSize, DEMOKEYFRAME	*pFrame, char *pBerusky1, 
						  int Rotation, CONTROL_KEYS *p_cont, DEMOSTRUCTURE	*pDemo, int *pos_a)
{
	int i, c = 0;
	...
	{
		int rot = demo_Rotation(Rotation, demo_Get_Move(pFrame->pos_o, pFrame->pos_n));
		*pBerusky1 = 1;
		...
		*pBerusky1 = 0;
		...
		if(!rot)
			...
			if(abs(rot) == 2)
			{
				...
				if(Rotation - 1 ==  move || (!Rotation &&  move == 3))
				{
					pKeyLine[0] = p_cont->turn_left;
					pKeyLine[1] = p_cont->move_forward;
				}
