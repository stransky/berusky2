...
void RunMenuSceneMap(char *p_File_Name, HWND hWnd, AUDIO_DATA *p_ad, int cpu, char *cSceneBmp, 
					 char *cSceneAnim, int iScene, int iLevelStart, int iNumOfLevels, char *cLevelList,
					 int xTV, int yTV, char bLoadGame, char *cSaveFile, char bTutorial, int xBack, int yBack);
...
int NextStep(CMD_LINE *cmd)
{
	if((rand()%100) < 60)
		...
		return cmd->iAnim[cmd->iLastfrm][11 + rand()%(cmd->iAnim[cmd->iLastfrm][17])];
}
...
// provede animace
//------------------------------------------------------------------------------------------------
void AnimationEvent(HWND hwnd, UINT uMsg, UINT idEvent, DWORD dwTime, AUDIO_DATA *p_ad)
{
	DRAW_RECT *dr;
	int i, j;
	char bAnim = 0;
...
// prida animaci
//------------------------------------------------------------------------------------------------
int AddAnimation(CMD_LINE *cmd, AUDIO_DATA *p_ad, char bOnlyOnes, char bButton)
{
	int i,r;
...
void StretchAnimation(RECT *rStart, RECT *rFinish, int iSurface, int iSpeed, AUDIO_DATA *p_ad)
{
	DWORD	dwS, dwF, dwE;
	char	done = 0;
	int		max = 0;
...
void SetCharMenu(LIST_VIEW_CONTROL *p_li)
{
	...
	ddxBitBlt(HDC2DD, p_li->rectList.left + TAB_X, p_li->rectList.top + TAB_Y + y - p_li->dx + xp, 
	...
	ddxBitBlt(HDC2DD, p_li->rectList.left + TAB_X, p_li->rectList.top + TAB_Y, ddxGetWidth(p_li->bDCn), 
...
void SetMenuSettingsS(ROZLISENI *roz, CONTROL_LIST_ITEM *citem, int *hdcTabUse)
{
	if(hdcTabUse[2])
	{
	setup.soundvolume = co_Progres_Get(citem, CLIST_ITEMC, 4);
...
void InitTab3d(CONTROL_LIST_ITEM *citem, int *hdcTab, ROZLISENI	*roz, int roz_size)
{
	...
	DrawClock(&iClock, 0);
		...
		co_Combo_Set_Sel(hdcTab[1], citem[8].p_combo, GetResCombobox(roz, roz_size));
		...
		if(!setup.bump_mapping)
			...
			if(setup.bump_mapping && setup.text_bump2 && !setup.text_bump3)
		...
		citem[29].p_check = co_Create_CheckBox(hdcTab[1], 525, 415, "##settings_dithering", 0, 7);
		citem[29].iTab = 1;
		co_Check_Set_State(citem[29].p_check, hdcTab[1], setup.ditering, 1);

		citem[11].p_check = co_Create_CheckBox(hdcTab[1], 525, 135, "##settings_draw_mirror", 0, 4);
		citem[11].iTab = 1;
		co_Check_Set_State(citem[11].p_check, hdcTab[1], setup.zrcado_aktivni, 1);

		citem[30].p_check = co_Create_CheckBox(hdcTab[1], 525, 535, "##settings_caustic", 0, 8);
		citem[30].iTab = 1;
		co_Check_Set_State(citem[30].p_check, hdcTab[1], setup.scene_materialy, 1);

		citem[31].p_check = co_Create_CheckBox(hdcTab[1], 300, 580, "##settings_vertex", 0, 9);
		citem[31].iTab = 1;
		co_Check_Set_State(citem[31].p_check, hdcTab[1], setup.vertex_arrays, 1);

		citem[44].p_check = co_Create_CheckBox(hdcTab[1], 300, 610, "##settings_load_menu_vram", 0, 10);
		citem[44].iTab = 1;
...
MENU_SETTING_BRUTAL_RESTART:
  ...
  working_file_translate(dir,256);
	...
	if(co_Load_Graphic(0))
	{
		...
		citem[4].p_check = co_Create_CheckBox(hdcTab[0], 25, 80, "##settings_b1_control", 0, 2);
		co_Check_Set_State(citem[4].p_check, hdcTab[0], setup.ovladani, 1);
		citem[4].bActive = 1;

		citem[45].p_check = co_Create_CheckBox(hdcTab[0], 25, 110, "##settings_beathe_vis_at_start", 0, 10);
		co_Check_Set_State(citem[45].p_check, hdcTab[0], setup.start_zpruhlednovani, 1);
		citem[45].bActive = 1;
		
		citem[46].p_check = co_Create_CheckBox(hdcTab[0], 25, 140, "##settings_items_vis_at_start", 0, 11);
		co_Check_Set_State(citem[46].p_check, hdcTab[0], setup.start_zvyraznovani, 1);
		citem[46].bActive = 1;

		citem[51].p_check = co_Create_CheckBox(hdcTab[0], 25, 200, "##settings_camera_intro", 0, 16);
		co_Check_Set_State(citem[51].p_check, hdcTab[0], setup.camera_intro, 1);
		citem[51].bActive = 1;

		citem[47].p_check = co_Create_CheckBox(hdcTab[0], 25, 230, "##settings_camera_rect", 0, 12);
		co_Check_Set_State(citem[47].p_check, hdcTab[0], setup.ovladani_rohy, 1);
		citem[47].bActive = 1;

		citem[48].p_check = co_Create_CheckBox(hdcTab[0], 25, 260, "##settings_implicit_move", 0, 13);
		...
		citem[49].p_check = co_Create_CheckBox(hdcTab[0], 25, 290, "##settings_imlicit_rot", 0, 14);
		...
		citem[50].p_check = co_Create_CheckBox(hdcTab[0], 25, 320, "##settings_camera_swap", 0, 15);
	...
	for(i=0;i<lastcmd;i++)
		if(res[i].iParam[0] == COM_RUNANIMATION)
		{
			...
			if(iWave != -1)
			{
				...
				anm[iWave].iWave = res[i+1].iParam[5] = mPlaySound(&res[i+1], p_ad, 2);
			}
		}
					...
					if(i != lastabv)
					{
						if(in)
						{
							...
							else
							{
								...
								ddxDrawDisplayColorKey(res[lastabv].iAnim[0], 3, TRANSCOLOR);
		...
		if(dim.t1 && !click)
		{
			...
			if(iCheck != -1)
			{
				if(!iCheck)
				{
					...
					co_Check_Disable(HDC2DD, TAB_X, TAB_Y, citem, CLIST_ITEMC, 15);
					//co_Check_Disable(HDC2DD, TAB_X, TAB_Y, citem, CLIST_ITEMC, 16);
					co_Progres_Disable(HDC2DD, TAB_X, TAB_Y, citem, CLIST_ITEMC, 0, 1, HDC2DD);
				}
				else
				{
...
void GetRunMenuNewGameSceneLoadGame(char *cscene, char *cscenemap, char *csceneanim, char	*cscenelevel, 
									int *iLevelStart, int	*iNumOfLevels, int	*xTV, int *yTV, char *bTutorial,
									int	*xBack, int *yBack)
{
	*bTutorial = 0;
...
void RunMenuNewGameSceneLoadGame(HWND hWnd, AUDIO_DATA *p_ad, int cpu, char bLoadGame, char *cSaveFile)
{
	char	cscene[64] = "";
	char	cscenemap[64] = "";
	char	csceneanim[64] = "";
...
void RunMenuNewGameScene(char *p_File_Name, HWND hWnd, AUDIO_DATA *p_ad, int cpu, char bLoadGame, char *cSaveFile, char bNewGame)
{
	DWORD	dwEplased = 0, dwStart, dwStop;
...
void LoadSceneMap(int *pBmp, char *cSceneBmp, char *cSceneAnim, int iScene, int *iClock)
{
	int i;
	char text[256];
	APAK_HANDLE *hArchive = NULL;
...
void CreateLevelButton(int x, int y, CMD_LINE *res, int *lastcmd, int *pBmp, char *cMessage, char bDone, 
					   int iDificulty, char bTuturial, int iLevel)
{
	int	ii;
	char text[1024];
	...
	if(!iDificulty)
	{
		...
		sprintf(text,"BindAnimation(118, -1, -1, -1, televize5.txt, televize.txt)");
		Parse_LineT(text, res[*lastcmd].iParam, 6, res[*lastcmd].cParam[0], res[*lastcmd].cParam[1]);
		(*lastcmd)++;

		//BindAnimation(118, -1, -1, -1, televize5.txt, televize.txt)
		sprintf(text,"BindAnimation(118, -1, -1, -1, televize6.txt, televize.txt)");
		Parse_LineT(text, res[*lastcmd].iParam, 6, res[*lastcmd].cParam[0], res[*lastcmd].cParam[1]);
		(*lastcmd)++;	
	}
	else
	...
	{
		//BindAnimation(118, -1, -1, -1, televize5.txt, televize.txt)
		sprintf(text,"BindAnimation(118, -1, -1, -1, televize7.txt, televize.txt)");
		Parse_LineT(text, res[*lastcmd].iParam, 6, res[*lastcmd].cParam[0], res[*lastcmd].cParam[1]);
		(*lastcmd)++;

		//BindAnimation(118, -1, -1, -1, televize5.txt, televize.txt)
		sprintf(text,"BindAnimation(118, -1, -1, -1, televize8.txt, televize.txt)");
		Parse_LineT(text, res[*lastcmd].iParam, 6, res[*lastcmd].cParam[0], res[*lastcmd].cParam[1]);
		(*lastcmd)++;	
	}
	else
	...
	{
		//BindAnimation(118, -1, -1, -1, televize5.txt, televize.txt)
		sprintf(text,"BindAnimation(118, -1, -1, -1, televize9.txt, televize.txt)");
		Parse_LineT(text, res[*lastcmd].iParam, 6, res[*lastcmd].cParam[0], res[*lastcmd].cParam[1]);
		(*lastcmd)++;
	}
	else
	if(iDificulty == 3)
	{
		...
		//BindAnimation(118, -1, -1, -1, televize5.txt, televize.txt)
		sprintf(text,"BindAnimation(118, -1, -1, -1, televize10.txt, televize.txt)");
		Parse_LineT(text, res[*lastcmd].iParam, 6, res[*lastcmd].cParam[0], res[*lastcmd].cParam[1]);
		(*lastcmd)++;
	}
	else
	{
		//BindAnimation(118, -1, -1, -1, televize5.txt, televize.txt)
		sprintf(text,"BindAnimation(118, -1, -1, -1, televize11.txt, televize.txt)");
		Parse_LineT(text, res[*lastcmd].iParam, 6, res[*lastcmd].cParam[0], res[*lastcmd].cParam[1]);
		(*lastcmd)++;

		//BindAnimation(118, -1, -1, -1, televize5.txt, televize.txt)
		sprintf(text,"BindAnimation(118, -1, -1, -1, televize12.txt, televize.txt)");
		Parse_LineT(text, res[*lastcmd].iParam, 6, res[*lastcmd].cParam[0], res[*lastcmd].cParam[1]);
		(*lastcmd)++;
	}
...
/*	else
	...
	//OnClick(16,661,100,748, quit_gamec.txt, EXIT)
	sprintf(text,"OnClick(%d,%d,%d,%d, NO_EXEPTION, %s)", x, y, x + ddxGetWidth(pBmp[6+bDone]), y + ddxGetHight(pBmp[6+bDone]), cMessage);
...
void CreateLevelButtons(CMD_LINE *res, int *lastcmd, int *pBmp, int iScene, int iLevelStart, int iNumOfLevels, char *cLevelList, char bTutorial)
{
	char text[256];
	int r[4];
	int i;
	...
	for(i=iLevelStart; i<iLevelStart+iNumOfLevels; i++)
	{
		...
		CreateLevelButton(r[1], r[2], res, lastcmd, pBmp, itoa(r[0], text, 10), pPlayerProfile.cLevel[i], r[3], bTutorial, i);
	}
...
void CorrectTV(CMD_LINE *res, int iAnim, int iLast, int x, int y, int idx, int iScene)
{
	int tvcoridx = -1;
	int i, d;
...
void CreateBackButton(CMD_LINE *res, int *lastcmd, int sidx1, int sidx2, int xBack, int yBack)
{
	int lcmd = *lastcmd;
...
void RunMenuSceneMap(char *p_File_Name, HWND hWnd, AUDIO_DATA *p_ad, int cpu, char *cSceneBmp, char *cSceneAnim, 
					 int iScene, int iLevelStart, int iNumOfLevels, char *cLevelList, int xTV, int yTV, 
					 char bLoadGame, char *cSaveFile, char bTutorial, int xBack, int yBack)
{
	DWORD	dwEplased = 0, dwStart, dwStop;
...
BRUTAL_RESTART_SCENE_MAP_MENU:
	...
	GetRunMenuNewGameSceneLoadGame( csrriptname, cscenemap, csceneanim, cscenelevels, &iLevelStart, &iNumOfLevels, 
									&xTV, &yTV, &bTutorial, &xBack, &yBack);
  ...
  working_file_translate(dir,256);
	...
	CreateLevelButtons(res, &lastcmd, iBmp, iActualScene, iLevelStart, iNumOfLevels, cscenelevels, bTutorial);
			...
			case COM_DRAW:
				{
					if(!c)
					{
						...
						ddxTransparentBlt(BackDC,res[i].iParam[2], res[i].iParam[3], ddxGetWidth(res[i].iParam[1]), 
...
void RunMenuLoadGameLoad(char *p_File_Name, HWND hWnd, AUDIO_DATA *p_ad, int cpu, int LoadGame)
{
	DWORD	dwEplased = 0, dwStart, dwStop;
...
__QUIT:
	...
	if(!cBrutalRestart)
	{
		...
		ddxTransparentBlt(BackDC, res[idx].iParam[2], res[idx].iParam[3], 
						  ddxGetWidth(res[idx].iParam[1]), 
						  ddxGetHight(res[idx].iParam[1]),
						  res[idx].iParam[1], 0, 0, ddxGetWidth(res[idx].iParam[1]), ddxGetHight(res[idx].iParam[1]), TRANSCOLOR);
...
//	ddxCleareSurface(BackDC);
  ...
  working_file_translate(dir,256);
	...
	iNadpisDC = ddxCreateSurface(r.right - r.left, r.bottom - r.top, ddxFindFreeSurface());

	ddxBitBlt(iNadpisDC, 0, 0, ddxGetWidth(iNadpisDC), ddxGetHight(iNadpisDC), BackDC, r.left, r.top);
	...
	while(!key[K_ESC])
	{
		...
		if(!anm[anmid].cmd)
		{
			...
			if(!cBrutalRestart)
			{
				...
				ddxTransparentBltDisplay(r.left, r.top, ddxGetWidth(iNadpisDC), ddxGetHight(iNadpisDC), 
										 iNadpisDC, 0, 0, ddxGetWidth(iNadpisDC), ddxGetHight(iNadpisDC), TRANSCOLOR);

				ddxTransparentBlt(BackDC, r.left, r.top, ddxGetWidth(iNadpisDC), ddxGetHight(iNadpisDC), 
								  iNadpisDC, 0, 0, ddxGetWidth(iNadpisDC), ddxGetHight(iNadpisDC), TRANSCOLOR);
			}
...
void RunMenuChildGame(char *p_File_Name, HWND hWnd, AUDIO_DATA *p_ad, int cpu)
{
	DWORD	dwEplased = 0, dwStart, dwStop;
...
int LoadCList(char *cFile, int *p_count, int *p_bmp, int *iClock, APAK_HANDLE *hArchive)
{
	int c = 0, i = -1, t;
	char text[256];
	FILE *file;
