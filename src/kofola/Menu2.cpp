...
int NextStep2(CMD_LINE *cmd)
{
	if((rand()%100) < 60)
		...
		return cmd->iAnim[cmd->iLastfrm][6 + rand()%(cmd->iAnim[cmd->iLastfrm][12])];
}
...
*/
			...
			else
			{
				...
				if(anm[i].cmd->iCounter >= anm[i].cmd->iAnim[anm[i].cmd->iLastfrm][4])
				{
	...
	if(bAnim)
	{
		if(bBackDC)
		{
			for(i=0;i<rline.rlast;i++)
			{
				...
				ddx2TransparentBlt(BackDC, dr->rect.left,  dr->rect.top,  dr->rect.right, dr->rect.bottom, 
			...
			for(i=0;i<rline.rlast;i++)
			{
				...
				ddx2TransparentBlt(CompositDC, dr->rect.left,  dr->rect.top,  dr->rect.right, dr->rect.bottom,
...
//				ddx2UpdateMouse();
				...
				ddx2TransparentBltDisplay(dr->rect.left,  dr->rect.top,  dr->rect.right, dr->rect.bottom, 
...
int CreateFontAnimations2(CMD_LINE *res, int *lastcmd, int ycor, char bcor)
{
	...
	for(i=0;i<lcmd;i++)
		if(res[i].iParam[0] == COM_CREATEBUTTON)
			if(fn2_Gen_Menu_Text(0, HDC2DD, res[i].cParam[0], &sidx1, &sidx2) != -1)
				if(sidx1 != -1 && sidx2 != -1)
				{
					int ii;
					int oy = res[i].iParam[2];
					//int x = r.left + ftoi((r.right - r.left - _2dd.bitmap[sidx1].bitmap.bmWidth) / (float)2.0f);
					int x = r.left + ftoi((r.right - r.left - ddx2GetWidth(sidx1)) / (float)2.0f);
					...
					*/
					...
					sprintf(text,"OnClick(%d,%d,%d,%d, NO_EXEPTION, %s)", x, oy, x + ddx2GetWidth(sidx1), 
							y, res[i].cParam[1]);
...
void SetTab2(int iTab, int iLTab, CONTROL_LIST_ITEM2 *p_list, int lsize, int	*hdcTab)
{
	int i;
...
void SetCharMenu2(LIST_VIEW_CONTROL2 *p_li, LEVELINFO *p_Level, ANIMATION_MODULE *p_am, int mix, int miy)
{
	char text[128];
	...
	if((p_li->rectList.top + TAB_Y + y - p_li->dx + 30) > p_li->rectList.bottom + TAB_Y)
		xt = (p_li->rectList.top + TAB_Y + y - p_li->dx + 30) - (p_li->rectList.bottom + TAB_Y);
				
	ddx2BitBlt(HDC2DD, p_li->rectList.left + TAB_X, p_li->rectList.top + TAB_Y + y - p_li->dx + xp, 
		       ddx2GetWidth(p_li->bDCs), 30 - xp - xt, p_li->bDCs, 0, y + xp);
	...
	while(!key[0])
	{
		...
		if(key[K_F1] || key[K_F2] || key[K_F3] || key[K_F10] || key[K_PLUS] || key[K_MINUS])
		{
			key[0] = 0;
			key[K_F1] = 0;
			key[K_F2] = 0;
	...
	ddx2BitBlt(HDC2DD, p_li->rectList.left + TAB_X, p_li->rectList.top + TAB_Y, ddx2GetWidth(p_li->bDCn), 
...
void SetMenuSettings2(CONTROL_LIST_ITEM2 *citem, int *hdcTabUse)
{
	...
	setup.ovladani_rohy_rychlost = co2_Progres_Get(citem, CLIST_ITEMC, 0) / 10.0f;
...
void InitTab3d2(CONTROL_LIST_ITEM2 *citem, int *hdcTab)
{
	...
	citem[29].p_check = co2_Create_CheckBox(hdcTab[1], 300, 150, "##settings_dithering", 0, 7);
	citem[29].iTab = 1;
	co2_Check_Set_State(citem[29].p_check, hdcTab[1], setup.ditering, 1);

	//kprintf(1, "##settings_animations");

	citem[30].p_check = co2_Create_CheckBox(hdcTab[1], 300, 180, "##settings_animations", 0, 8);
	citem[30].iTab = 1;
...
//	ZeroMemory(roz, POCETROZLICENI * sizeof(ROZLISENI));
	...
	else
	{
		...
		citem[3].p_check = co2_Create_CheckBox(hdcTab[0], 25, 50, "##settings_camera_mov", 0, 1);
		co2_Check_Set_State(citem[3].p_check, hdcTab[0], setup.posouvat_kameru, 1);
		citem[3].bActive = 1;

		citem[4].p_check = co2_Create_CheckBox(hdcTab[0], 25, 80, "##settings_b1_control", 0, 2);
		...
		co2_Check_Set_State(citem[36].p_check, hdcTab[0], setup.start_zvyraznovani, 1);
		citem[36].bActive = 1;

		citem[41].p_check = co2_Create_CheckBox(hdcTab[0], 25, 200, "##settings_camera_intro", 0, 16);
		co2_Check_Set_State(citem[41].p_check, hdcTab[0], setup.camera_intro, 1);
		citem[41].bActive = 1;

		citem[37].p_check = co2_Create_CheckBox(hdcTab[0], 25, 230, "##settings_camera_rect", 0, 12);
		co2_Check_Set_State(citem[37].p_check, hdcTab[0], setup.ovladani_rohy, 1);
		citem[37].bActive = 1;

		citem[38].p_check = co2_Create_CheckBox(hdcTab[0], 25, 260, "##settings_implicit_move", 0, 13);
		...
		citem[39].p_check = co2_Create_CheckBox(hdcTab[0], 25, 290, "##settings_imlicit_rot", 0, 14);
		...
		citem[40].p_check = co2_Create_CheckBox(hdcTab[0], 25, 320, "##settings_camera_swap", 0, 15);
		co2_Check_Set_State(citem[40].p_check, hdcTab[0], setup.ovladani_rohy_smer, 1);
		citem[40].bActive = 1;
		...
		co2_Progres_Set(citem[42].p_prog, hdcTab[0], ftoi(setup.ovladani_rohy_rychlost * 10));
		citem[42].bActive = 1;

		co2_Set_Text_Right(hdcTab[0], "##settings_trans_radius", 0, 450, 425);
		citem[7].p_prog = co2_Create_Progres(hdcTab[0], 500, 412, 1, 20, 1);
		co2_Progres_Set(citem[7].p_prog, hdcTab[0], ftoi(setup.p_kamera_radius * 2));
		citem[7].bActive = 1;
		...
		if(!setup.ovladani_rohy)
		{
			...
			co2_Progres_Disable(hdcTab[0], 0, 0, citem, CLIST_ITEMC, 0, 1, hdcTab[0]);
		}
		...
		if(!setup.ovladani_rohy)
		{
			...
			co2_Progres_Disable(HDC2DD, TAB_X, TAB_Y, citem, CLIST_ITEMC, 0, 0, HDC2DD);
		}
		...
		if(mi.t1 && !click)
		{
			...
			if(iCheck != -1)
			{
				if(!iCheck)
				{
					...
					co2_Check_Disable(HDC2DD, TAB_X, TAB_Y, citem, CLIST_ITEMC, 15);
					//co2_Check_Disable(HDC2DD, TAB_X, TAB_Y, citem, CLIST_ITEMC, 16);
					co2_Progres_Disable(HDC2DD, TAB_X, TAB_Y, citem, CLIST_ITEMC, 0, 1, HDC2DD);
				}
				else
				{
...
//	char *t;
	...
	for(c=0;c<isize;c++)
	{
		...
		if(bLoad == 2)
		{
			...
			if(!check_Save_Owner(list[c].text, wFile))
				continue;
...
//	FILE	*file;
	...
	ddx2DeviceSetScreenRec((int)ceil(s_bfactor[0] * 299), (int)ceil(s_bfactor[1] * 209), 
						   (int)ceil(s_bfactor[0] * 444), (int)ceil(s_bfactor[1] * 415));
        ...
        */
		...
		if((p_Level->bRestart || p_Level->bReturnToMenu) && mi.t1)
		{
			if(!p_Level->iMessageBoxReturn)
			{
				...
				if(p_Level->iMessageBoxReturn == 1)
				{
					iReturn = 1;
					key[K_ESC] = 1;
					p_Level->bRestart = 0;
...
int DrawHelp(int iBmp, int iSurface, int *iHelpIndex)
{
	...
	y = co2_Set_Text_Formated_Rect(iSurface, 10, 60, "##help_introduction_h", 0, &rect) + 50;
	...
	ddx2BitBlt(iSurface, 0, y, ddx2GetWidth(iBmp+3), ddx2GetHeight(iBmp+3), iBmp+3, 0, 0);
	rect.left = 10 + ddx2GetWidth(iBmp+3);

	y = co2_Set_Text_Formated_Rect(iSurface, rect.left, y, "##help_beatle_h", 0, &rect) + 50;
	...
	ddx2BitBlt(iSurface, 0, y, ddx2GetWidth(iBmp+4), ddx2GetHeight(iBmp+4), iBmp+4, 0, 0);
	rect.left = 10 + ddx2GetWidth(iBmp+4);

	co2_Set_Text_Formated_Rect(iSurface, rect.left, y, "##help_exit_h", 0, &rect);
	...
	ddx2BitBlt(iSurface, 0, y, ddx2GetWidth(iBmp+15), ddx2GetHeight(iBmp+15), iBmp+15, 0, 0);
	rect.left = 10 + ddx2GetWidth(iBmp+15);

	co2_Set_Text_Formated_Rect(iSurface, rect.left, y, "##help_wall_h", 0, &rect);
	...
	ddx2BitBlt(iSurface, 0, y, ddx2GetWidth(iBmp+8), ddx2GetHeight(iBmp+8), iBmp+8, 0, 0);
	...
	ddx2BitBlt(iSurface, 0, y, ddx2GetWidth(iBmp+1), ddx2GetHeight(iBmp+1), iBmp+1, 0, 0);
	rect.left = 10 + ddx2GetWidth(iBmp+1);

	co2_Set_Text_Formated_Rect(iSurface, rect.left, y, "##help_empty_box_h", 0, &rect);
	...
	ddx2BitBlt(iSurface, 0, y, ddx2GetWidth(iBmp+2), ddx2GetHeight(iBmp+2), iBmp+2, 0, 0);
	rect.left = 10 + ddx2GetWidth(iBmp+2);

	co2_Set_Text_Formated_Rect(iSurface, rect.left, y, "##help_full_box_h", 0, &rect);
	...
	ddx2BitBlt(iSurface, 0, y, ddx2GetWidth(iBmp+11), ddx2GetHeight(iBmp+11), iBmp+11, 0, 0);
	rect.left = 10 + ddx2GetWidth(iBmp+11);

	co2_Set_Text_Formated_Rect(iSurface, rect.left, y, "##help_tnt_box_h", 0, &rect);
	...
	ddx2BitBlt(iSurface, 0, y, ddx2GetWidth(iBmp+5), ddx2GetHeight(iBmp+5), iBmp+5, 0, 0);
	rect.left = 10 + ddx2GetWidth(iBmp+5);

	co2_Set_Text_Formated_Rect(iSurface, rect.left, y, "##help_rock_h", 0, &rect);
	...
	ddx2BitBlt(iSurface, 0, y, ddx2GetWidth(iBmp+10), ddx2GetHeight(iBmp+10), iBmp+10, 0, 0);
	rect.left = 10 + ddx2GetWidth(iBmp+10);

	co2_Set_Text_Formated_Rect(iSurface, rect.left, y, "##help_button_h", 0, &rect);
	...
	ddx2BitBlt(iSurface, 0, y, ddx2GetWidth(iBmp+9), ddx2GetHeight(iBmp+9), iBmp+9, 0, 0);
	
	rect.left = 10 + ddx2GetWidth(iBmp+9);

	ty = y + ddx2GetHeight(iBmp+9) + 50;

	y = co2_Set_Text_Formated_Rect(iSurface, rect.left, y, "##help_teleport_h", 0, &rect) + 50;
	...
	y = co2_Set_Text_Formated_Rect(iSurface, rect.left, y, "##help_teleport_button_h", 0, &rect);
	...
	ddx2BitBlt(iSurface, 0, y, ddx2GetWidth(iBmp+12), ddx2GetHeight(iBmp+12), iBmp+12, 0, 0);
	rect.left = 10 + ddx2GetWidth(iBmp+12);

	co2_Set_Text_Formated_Rect(iSurface, rect.left, y, "##help_lift_h", 0, &rect);
	...
	ddx2BitBlt(iSurface, 0, y, ddx2GetWidth(iBmp+17), ddx2GetHeight(iBmp+17), iBmp+17, 0, 0);
	rect.left = 10 + ddx2GetWidth(iBmp+17);
	ty = y + ddx2GetHeight(iBmp+17) + 50;

	y = co2_Set_Text_Formated_Rect(iSurface, rect.left, y, "##help_lift_weight_h", 0, &rect);
	...
	ddx2BitBlt(iSurface, 0, y, ddx2GetWidth(iBmp+18), ddx2GetHeight(iBmp+18), iBmp+18, 0, 0);
	rect.left = 10 + ddx2GetWidth(iBmp+18);

	ty = y + ddx2GetHeight(iBmp+18) + 50;

	y = co2_Set_Text_Formated_Rect(iSurface, rect.left, y, "##help_lift_button_h", 0, &rect);
	y = ty;
	...
	y = co2_Set_Text_Formated_Rect(iSurface, rect.left, y, "##help_water_h", 0, &rect);
	y += 50;
	...
	ddx2BitBlt(iSurface, 0, y, ddx2GetWidth(iBmp+7), ddx2GetHeight(iBmp+7), iBmp+7, 0, 0);
	rect.left = 10 + ddx2GetWidth(iBmp+7);

	co2_Set_Text_Formated_Rect(iSurface, rect.left, y, "##help_trap_door_h", 0, &rect);
	...
	ddx2BitBlt(iSurface, 0, y, ddx2GetWidth(iBmp+16), ddx2GetHeight(iBmp+16), iBmp+16, 0, 0);
	rect.left = 10 + ddx2GetWidth(iBmp+16);

	co2_Set_Text_Formated_Rect(iSurface, rect.left, y, "##help_water_pipe_h", 0, &rect);
	...
	ddx2BitBlt(iSurface, 0, y, ddx2GetWidth(iBmp+13), ddx2GetHeight(iBmp+13), iBmp+13, 0, 0);
	rect.left = 10 + ddx2GetWidth(iBmp+13);

	co2_Set_Text_Formated_Rect(iSurface, rect.left, y, "##help_hormonal_vitamine_h", 0, &rect);
	...
	ddx2BitBlt(iSurface, 0, y, ddx2GetWidth(iBmp+14), ddx2GetHeight(iBmp+14), iBmp+14, 0, 0);
	rect.left = 10 + ddx2GetWidth(iBmp+14);

	co2_Set_Text_Formated_Rect(iSurface, rect.left, y, "##help_iron_h", 0, &rect);
	...
	ddx2BitBlt(iSurface, 0, y, ddx2GetWidth(iBmp+6), ddx2GetHeight(iBmp+6), iBmp+6, 0, 0);
	rect.left = 10 + ddx2GetWidth(iBmp+6);

	co2_Set_Text_Formated_Rect(iSurface, rect.left, y, "##help_rock_axe_h", 0, &rect);
...
/*				else
	...
	else
	{
		...
		co2_Combo_Add_StringWC(citem[0].p_combo, "##help_hormonal_vitamine_mainc");
		...
		citem[1].p_list_v = co2_Create_List_View(HDC2DD, 100, 107, 828, 622, 0, 6800);
		citem[1].bActive = 1;
	...
	while(!key[K_ESC])
	{
		...
		{
			co2_Set_List_View_List_Pos(citem[1].p_list_v, iHelpIndex[iComboActSel], HDC2DD, 0, 0);
...
/*
	...
	ddx2DeviceSetScreenRec(ftoi(10 * s_bfactor[0]), ftoi(200 * s_bfactor[1]), ftoi(900 * s_bfactor[0]),ftoi(450 * s_bfactor[1]));
	//ddx2DeviceSetScreenRec(0, 0, hwconf.xres,hwconf.yres);
...
//	sh = ddx2LoadBitmap("brouk.bmp", pBmpArchive);
	...
	while(!key[K_ESC] && !key[K_ENTER])
	{
		...
		am_Flip(p_Level, p_am, 0, &rline, CLIST_ITEMC, 1, ttext.text, ycorrection);
	}
