...
BOOL CALLBACK ko_MainDlgProc( HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam);
...
BOOL CheckGuid(INT g)
{
	...
	for (j=0;j<NUMOFOBJECTS;j++)
		if ((Object[j].GUID == g) && (Object[j].Class != 0)) return TRUE;
return FALSE;
}
...
void NaplnListBox(HWND hWnd, int Class)
{
	...
	if ((Object[i].GUID >= Class) && (Object[i].GUID<(Class+1000)))
		{
			WordString = SendMessage( hCombo, LB_ADDSTRING, 0, (LPARAM) (LPCTSTR) Object[i].Name );
