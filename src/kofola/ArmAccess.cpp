...
int Expire(HWND hWnd)
{
  ...
  if (!libInst)
    exit(0);                    // Couldn't load library

  ExpireCurrentKey =
    (ExpireCurrentKeyFn) GetProcAddress(libInst, "ExpireCurrentKey");
  if (ExpireCurrentKey == 0)
    exit(0);                    // Couldn't find function

  ShowEnterKeyDialog =
    (ShowEnterKeyDialogFn) GetProcAddress(libInst, "ShowEnterKeyDialog");
  if (ShowEnterKeyDialog == 0)
    exit(0);                    // Couldn't find function
