...
void adas_Set_Source_Propertiesl(ADAS_SOUND_SOURCE *p_ss, ALuint uiValue, long lValue);
void adas_Set_Source_Propertiesf(ADAS_SOUND_SOURCE *p_ss, ALuint uiValue, float fValue);
void adas_Set_Source_Propertiesdw(ADAS_SOUND_SOURCE *p_ss, ALuint uiValue, unsigned long dwValue);
...
unsigned long adas_Load_First(char *p_Index_File, char *p_File_Name)
{
	...
	if (file)
	{
		...
		alutLoadWAVFile(p_File_Name,&pSound->Format, &pSound->Data, &pSound->Size,
...
int adas_Load_Wave(ADAS_SOUND_SOURCE *p_ss)
{
	...
	alBufferData(p_ss->Buffer[p_ss->Buffer_Pointer], Format, Data, Size, Frequece);
...
int adas_Create_Source(ADAS_SOUND_SOURCE_DATA *p_ssd, void **p_callback)
{
	...
	if(p_ssd->Bounding_Object.Type == BOUNDING_BOX)
	{
		...
		memcpy(&p_ss->Bounding_Object, &p_ssd->Bounding_Object, sizeof(BOUNDING_OBJECT));
	...
	if(p_ssd->Bounding_Object.Type == BOUNDING_SPHERE)
		p_ss->Bounding_Object.Radius = p_ssd->Bounding_Object.Radius * ADAS_data.Scale_Factor;
...
// Set cube position
//------------------------------------------------------------------------------------------------
void adas_Set_Bounding_Object_PositionA(ADAS_SOUND_SOURCE *p_source, float *v)
{
	float size[3];
	int	i;
	...
	{
		for(i=0;i<3;i++)
			size[i] = (p_source->Bounding_Object.RTB_point[i] - p_source->Bounding_Object.LBF_point[i]) / 2.0f;
...
void adas_Set_Bounding_Object_Position(int Owner, int Type, int Index, float *v)
{
if(!bDevice) return;
...
void adas_Update_Cube_Pos(ADAS_SOUND_SOURCE	*p_s, int index, float *v)
{
	...
	for(j=0;j<3;j++)
	{
		if(v[j] < p_s->Bounding_Object.LBF_point[j])
			...
			if(v[j] > p_s->Bounding_Object.RTB_point[j])
				pos[j] = p_s->Bounding_Object.RTB_point[j];
			else
				pos[j] = v[j];
	}
				
	adas_Set_Source_Position(PARTICULAR_SOUND_SOURCE, UNDEFINED_VALUE, index, pos);
}
...
void adas_Update_Sphere_Pos(ADAS_SOUND_SOURCE	*p_s, int index, float *v)
{
	...
	{
		adas_Set_Source_Position(PARTICULAR_SOUND_SOURCE, UNDEFINED_VALUE, index, v);
	}
	...
	adas_Set_Source_Position(PARTICULAR_SOUND_SOURCE, UNDEFINED_VALUE, index, SourcePos);
}
	...
	for (i=0;i<ADAS_data.Channels;i++)
		if (SoundSource[i].Owner == Owner)
		{
			if ((Type == ALL_TYPES) && SoundSource[i].Source)
				...
				if((SoundSource[i].Type == Type) && (SoundSource[i].Source != UNDEFINED_VALUE))
						alSourcePause(SoundSource[i].Source);
		}
}
	...
	for (i=0;i<ADAS_data.Channels;i++)
		if (SoundSource[i].Owner == Owner)
		{
			if ((Type == ALL_TYPES) && SoundSource[i].Source)
				...
				if((SoundSource[i].Type == Type) && SoundSource[i].Source)
						alSourcePlay(SoundSource[i].Source);
		}
}
...
int adas_Is_Sound_Processed(int Wave_Index, int Owner)
{
	...
	for(i=0;i<ADAS_data.Channels;i++)
		if ((SoundSource[i].Owner == Owner) && (SoundSource[i].Wave_Index[0] == Wave_Index))
			return 1;
...
void adas_Set_All_Source_Volume(float fValue)
{
	...
	if(!bDevice) return;
	if (fValue < 0) fValue = 0.001f;
	if (fValue > 1) fValue = 1.0f;
...
//------------------------------------------------------------------------------------------------
void adas_Set_Source_All_Properties(int Owner, int Type, int Index, LPEAXBUFFERPROPERTIES lpData)
{
if(!bDevice) return;
...
// set source properties
//------------------------------------------------------------------------------------------------
void adas_Set_Source_lEAXproperty(int Owner, int Type, int Index, long lValue, ALuint Property)
{
...
void adas_Set_Source_fEAXproperty(int Owner, int Type, int Index, float fValue, ALuint Property)
{
	ADAS_SOUND_SOURCE	*p_Source;
...
void adas_Set_Source_dwEAXproperty(int Owner, int Type, int Index, unsigned long dwValue, ALuint Property)
{
	ADAS_SOUND_SOURCE	*p_Source;
...
//------------------------------------------------------------------------------------------------
void adas_Set_Source_fOPENALproperty(int Owner, int Type, int Index, float fValue, ALenum Property)
{
	ADAS_SOUND_SOURCE	*p_Source;
...
void adas_Set_Source_vOPENALproperty(int Owner, int Type, int Index, float *vValue, ALenum Property)
{
	ADAS_SOUND_SOURCE	*p_Source;
...
void adas_Set_Source_dwOPENALproperty(int Owner, int Type, int Index, unsigned long dwValue, ALenum Property)
{
	ADAS_SOUND_SOURCE	*p_Source;
...
//------------------------------------------------------------------------------------------------
void adas_Set_Source_Cone_Outside_Volume(int Owner, int Type, int Index, long lValue)
{
	ADAS_SOUND_SOURCE	*p_Source;
...
if (Owner == PARTICULAR_SOUND_SOURCE)
    alSourcef(SoundSource[Index].Source, AL_CONE_OUTER_GAIN, (float)pow(10, (float)lValue/2000.0f));
...
if (Owner < PARTICULAR_SOUND_SOURCE)
{
	...
	for (i=0;i<ADAS_data.Channels;i++)
	{
		...
		{
			if (Type == ALL_TYPES)
				alSourcef(p_Source->Source, AL_CONE_OUTER_GAIN, (float)pow(10, (float)lValue/2000.0f));
			else 
				if(p_Source->Type == Type)
					alSourcef(p_Source->Source, AL_CONE_OUTER_GAIN, (float)pow(10, (float)lValue/2000.0f));
		}
...
int adas_Queue_Sound(int Source, int Wave_Index)
{
	...
	if(p_ss->Buffer_Pointer > MAXQUEUED) return 0;
	...
	if (Mem_Index == -1)
	{
		...
		alSourceQueueBuffers(p_ss->Source,1,&p_ss->Buffer[p_ss->Buffer_Pointer]);
...
void adas_Unqueue_Sound(int Source, int Sounds)
{
	...
	for(i=0;i<Sounds; i++)
	{
		...
		if ((p_ss->Buffer_Source[i] != RAM) && (p_ss->Buffer_Source[i] != UNDEFINED_VALUE))
			alDeleteBuffers(1,&p_ss->Buffer[i]);
